...
#####################################################
# Helper functions mainly for the M33kAuras Options #
#####################################################
...
function M33kAuras.UnitExistsFixed(unit, smart)
...
function M33kAuras.split(input)
  input = input or "";
  local ret = {};
...
function M33kAuras.ScanEventsByID(event, id, ...)
...
function M33kAuras.ScanEvents(event, arg1, arg2, ...)
      ...
      for id, triggers in pairs(event_list) do
        ...
        for triggernum, data in pairs(triggers) do
          ...
          then
            ...
            if delay == 0 then
              local allStates = M33kAuras.GetTriggerStateForTrigger(id, triggernum);
...
function M33kAuras.ScanUnitEvents(event, unit, ...)
...
function M33kAuras.ScanEventsInternal(event_list, event, arg1, arg2, ... )
...
function Private.ScanEventsWatchedTrigger(id, watchedTriggernums)
  ...
  for _, watchedTrigger in ipairs(watchedTriggernums) do
    if watched_trigger_events[id] and watched_trigger_events[id][watchedTrigger] then
      local updatedTriggerStates = M33kAuras.GetTriggerStateForTrigger(id, watchedTrigger)
      for observerTrigger in pairs(watched_trigger_events[id][watchedTrigger]) do
        ...
        local allstates = M33kAuras.GetTriggerStateForTrigger(id, observerTrigger)
...
function HandleEvent(frame, event, arg1, arg2, ...)
  ...
  if not(M33kAuras.IsPaused()) then
...
---@class M33kAurasGenericTriggerFrame: FrameScriptObject
local frame = CreateFrame("Frame");
frame.unitFrames = {};
Private.frames["M33kAuras Generic Trigger Frame"] = frame;
      ...
      if(Private.category_event_prototype[triggerType] or triggerType == "custom") then
        ...
        if(Private.category_event_prototype[triggerType]) then
          if not(trigger.event) then
            error("Improper arguments to M33kAuras.Add - trigger type is \"event\" but event is not defined");
          elseif not(event_prototypes[trigger.event]) then
            if(event_prototypes["Conditions"]) then
              trigger.event = "Conditions";
            else
              error("Improper arguments to M33kAuras.Add - no event prototype can be found for event type \""..trigger.event.."\" and default prototype reset failed.");
            end
          else
            ...
            elseif prototype.timedrequired then
              ...
              M33kAuras.prettyPrint("Invalid Prototype found: " .. prototype.name)
            end
          ...
          triggerFunc = M33kAuras.LoadFunction("return "..(trigger.custom or ""), data.id);
          if (trigger.custom_type == "stateupdate") then
            tsuConditionVariables = M33kAuras.LoadFunction("return function() return \n" .. (trigger.customVariables or "") .. "\n end", data.id);
          ...
          if(trigger.custom_type == "status" or trigger.custom_type == "event" and trigger.custom_hide == "custom") then
            untriggerFunc = M33kAuras.LoadFunction("return "..(untrigger.custom or ""), data.id);
          ...
          if(trigger.custom_type ~= "stateupdate" and trigger.customDuration and trigger.customDuration ~= "") then
            durationFunc = M33kAuras.LoadFunction("return "..trigger.customDuration, data.id);
          end
          if(trigger.custom_type ~= "stateupdate") then
            overlayFuncs = {};
            for i = 1, 7 do
          ...
          if((trigger.custom_type == "status" or trigger.custom_type == "stateupdate") and trigger.check == "update") then
            ...
            local rawEvents = M33kAuras.split(trigger.events);
...
do
  ...
  local function CheckGCD()
    ...
    if(event and not M33kAuras.IsPaused()) then
  ...
  local function HandleSpell(self, id, startTime, duration, modRate, paused)
    ...
    if duration > 0 then
      if (startTime == gcdStart and duration == gcdDuration)
          or (M33kAuras.IsClassicOrWrath() and duration == shootDuration and startTime == shootStart)
      then
  ...
  local SpellDetails = {
    ...
    AddEffectiveSpellId = function(self, effectiveSpellId, userSpellId)
      ...
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
            ...
            = M33kAuras.GetSpellCooldownUnified(effectiveSpellId, GetRuneDuration());
    ...
    CheckSpellKnown = function(self)
      ...
      for effectiveSpellId, spellDetailsData in pairs(self.data) do
        local known = M33kAuras.IsSpellKnownIncludingPet(effectiveSpellId)
    ...
    CheckSpellCooldown = function(self, effectiveSpellId, runeDuration)
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
        startTimeCooldown, durationCooldown, cooldownBecauseRune, startTimeCharges, durationCharges,
        spellCount, unifiedModRate, modRate, modRateCharges, paused
        = M33kAuras.GetSpellCooldownUnified(effectiveSpellId, runeDuration);
    ...
    WatchSpellCooldown = function(self, userSpellId, ignoreRunes, useExact, followoverride)
      ...
      if ignoreRunes and M33kAuras.IsWrathOrCataOrMistsOrRetail() then
        for i = 1, 6 do
  ...
  function M33kAuras.GetRuneCooldown(id)
  ...
  local essenceCache = {{},{},{},{},{},{}}
  ---@private
  function M33kAuras.InitEssenceCooldown()
  ...
  function M33kAuras.GetEssenceCooldown(essence)
  ...
  function M33kAuras.GetSpellLossOfControlCooldown(identifier)
  ...
  function M33kAuras.GetSpellCooldown(id, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
  ...
  function M33kAuras.GetSpellCharges(id, ignoreSpellKnown)
  ...
  ---@return number gcdCooldown
  function M33kAuras.GetItemCooldown(id, showgcd)
  ...
  ---@return number duration
  function M33kAuras.gcdDuration()
    return gcdDuration or 0;
  end

  ---@return string name
  function M33kAuras.GcdSpellName()
    return gcdSpellName;
  end
  ...
  ---@return boolean enabled
  ---@return number gcdCooldown
  function M33kAuras.GetItemSlotCooldown(id, showgcd)
    local startTime, duration, enabled, gcdCooldown;
  ...
  function Private.CheckRuneCooldown()
    ...
    for id, _ in pairs(runes) do
      ...
      if(duration > 0 and duration ~= M33kAuras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
  ...
  function Private.CheckItemCooldowns()
    for id, _ in pairs(items) do
      ...
      if(duration > 0 and duration > 1.5 and duration ~= M33kAuras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
      ...
      if (itemCdEnabledChanged and not M33kAuras.IsPaused()) then
        Private.ScanEventsByID("ITEM_COOLDOWN_CHANGED", id);
      end
    end
  end
  ...
  ---@private
  function M33kAuras.WatchRuneCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
  ...
  ---@private
  function M33kAuras.WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
    SpellDetails:WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
  end

  ---@private
  function M33kAuras.WatchItemCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
    ...
    if not(items[id]) then
      ...
      itemCdEnabled[id] = enabled;
      if(duration and duration > 0 and duration > 1.5 and duration ~= M33kAuras.gcdDuration()) then
        local time = GetTime();
        local endTime = startTime + duration;
        itemCdDurs[id] = duration;
        itemCdExps[id] = endTime;
  ...
  ---@private
  function M33kAuras.WatchItemSlotCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
...
do
  ...
  ---@private
  function M33kAuras.WatchSpellActivation(id)
    if (not id) then
      return;
    end
  ...
  function M33kAuras.SpellActivationActive(id)
...
function M33kAuras.GetPlayerReaction(unit)
...
---@param unit UnitToken
function M33kAuras.WatchUnitChange(unit)
  unit = string.lower(unit)
  if not watchUnitChange then
    ...
    local function unitUpdate(unitA, eventsToSend)
      ...
      local newGUID = M33kAuras.UnitExistsFixed(unitA) and UnitGUID(unitA)
    ...
    local function reactionUpdate(unit, eventsToSend)
      ...
      local newReaction = M33kAuras.GetPlayerReaction(unit)
    ...
    local function reactionInit(unit)
      watchUnitChange.nameplateFaction[unit] = M33kAuras.GetPlayerReaction(unit)
    end
    ...
    if M33kAuras.IsClassicOrWrath() then
      function roleUpdate(unit, eventsToSend)
        ...
        local newRaidRole = M33kAuras.UnitRaidRole(unit)
    ...
    local handleEvent = {
      ...
      UNIT_PET = function(unit, eventsToSend)
        local pet = M33kAuras.unitToPetUnit[unit]
  ...
  watchUnitChange.unitIdToGUID[unit] = M33kAuras.UnitExistsFixed(unit) and UnitGUID(unit)
...
function M33kAuras.GetEquipmentSetInfo(itemSetName, partial)
...
function Private.WatchStagger()
  if not staggerWatchFrame then
    ...
    Private.frames["M33kAuras Stagger Frame"] = staggerWatchFrame
...
if M33kAuras.IsClassicOrWrath() then
  ...
  function M33kAuras.WatchForQueuedSpell()
  ...
  function M33kAuras.GetQueuedSpell()
...
-- Weapon Enchants
do
  local isCata = M33kAuras.IsCataClassic()
  ...
  function M33kAuras.TenchInit()
    if not(tenchFrame) then
      ...
      if M33kAuras.IsRetail() then
        ...
        getTenchName = function(id)
          tenchTip = M33kAuras.GetHiddenTooltip();
...
do
  ...
  ---@private
  function M33kAuras.WatchForPetDeath()
...
do
  ...
  ---@private
  function M33kAuras.WatchForCastLatency()
  ...
  function M33kAuras.GetCastLatency()
...
do
  ...
  ---@private
  function M33kAuras.WatchForNameplateTargetChange()
...
do
  ...
  ---@private
  function M33kAuras.WatchForPlayerMoving()
...
if C_AssistedCombat and C_AssistedCombat.GetNextCastSpell then
  ...
  ---@private
  function M33kAuras.WatchForAssistedCombatNextCast()
...
---@private
function M33kAuras.RegisterItemCountWatch()
...
function M33kAuras.GetUniqueCloneId()
...
local commonConditions = {
  ...
  itemInRange = {
    display = M33kAuras.newFeatureString .. L["Item in Range"],
    hidden = true,
    type = "bool",
    test = function(state, needle)
        ...
        if (enable) then
          ...
          if (result[v.name].type == "select" or result[v.name].type == "unit") then
            if (v.conditionValues) then
              result[v.name].values = Private[v.conditionValues] or M33kAuras[v.conditionValues];
            else
              if type(v.values) == "function" then
                result[v.name].values = v.values()
              else
                result[v.name].values = Private[v.values] or M33kAuras[v.values];
              end
            end
          end
...
do
  ...
  if class == "DEMONHUNTER"
    ...
    or class == "EVOKER"
  then
    function M33kAuras.CalculatedGcdDuration()
...
M33kAuras.CheckForItemBonusId = function(ids)
...
M33kAuras.GetBonusIdInfo = function(ids, specificSlot)
...
M33kAuras.CheckForItemEquipped = function(itemId, specificSlot)
...
Private.ExecEnv.GetCurrencyAccountInfo = function(currencyId)
  ...
  if M33kAuras.IsRetail() and currencyInfo and currencyInfo.isAccountTransferable then
  ...
  -- WORKAROUND https://github.com/m33shoq/M33kAuras/issues/5106
  if currencyInfo and M33kAuras.IsCataClassic() then
...
-- So for now make it simply a member of M33kAuras
M33kAuras.genericTriggerTypes = types

M33kAuras.RegisterTriggerSystem(types, GenericTrigger);
