...
function TimeMachine:StartTransaction()
  if self.transaction then
    M33kAuras.prettyPrint("If you're reading this, a time machine transaction was started, but there was already one in  progress. That's not supposed to happen. Please report this to the M33kAuras developers, thanks!")
    self:Reject()
  end
...
function TimeMachine:Commit(instant)
  if not self.transaction and not instant then
    M33kAuras.prettyPrint("If you're reading this, a time machine transaction was committed, but there was no transaction in progress. That's not supposed to happen. Please report this to the M33kAuras developers, thanks!")
    return
  end
...
function TimeMachine:DestroyTheUniverse(id)
  if self.transaction then
    M33kAuras.prettyPrint("If you're reading this, a time machine transaction was destroyed, but there was one in progress. That's not supposed to happen. Please report this to the M33kAuras developers, thanks!")
    self:Reject()
  end
