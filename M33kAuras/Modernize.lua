...
function Private.Modernize(data, oldSnapshot)
  if not data.internalVersion or data.internalVersion < 2 then
    M33kAuras.prettyPrint(string.format("Data for '%s' is too old, can't modernize.", data.id))
  ...
  -- Version 10 is skipped, due to a bad migration script (see https://github.com/m33shoq/M33kAuras/pull/1091)
    ...
    for triggerId, triggerData in ipairs(data.triggers) do
      if triggerData.trigger.type == "status" or triggerData.trigger.type == "event" then
        ...
        if newType then
          ...
          M33kAuras.prettyPrint("Unknown trigger type found in, please report: ", data.id, triggerData.trigger.event)
        end
      end
    end
  end
  ...
  if data.internalVersion == 49 then
    ...
    if broken then
      M33kAuras.prettyPrint(L["Trying to repair broken conditions in %s likely caused by a M33kAuras bug."]:format(data.id))
    end
  end
  ...
  if data.internalVersion < 75 then
    -- this commit from nov 2019 https://github.com/m33shoq/M33kAuras/commit/6d8f11c17422aeffdb82a0aa05181edfdd137896
    ...
    -- this commit https://github.com/m33shoq/M33kAuras/commit/dbcb70b1e4df262af82f63620b3b0d80741e6df2
