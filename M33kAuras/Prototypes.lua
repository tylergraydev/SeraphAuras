...
function M33kAuras.IsSpellInRange(spellId, unit)
  return SpellRange.IsSpellInRange(spellId, unit)
end

local LibRangeCheck = LibStub("LibRangeCheck-3.0")

function M33kAuras.GetRange(unit, checkVisible)
  return LibRangeCheck:GetRange(unit, checkVisible);
end

function M33kAuras.CheckRange(unit, range, operator)
...
function M33kAuras.UnitDetailedThreatSituation(unit1, unit2)
...
M33kAuras.UnitCastingInfo = UnitCastingInfo
...
function M33kAuras.SpellSchool(school)
  return Private.combatlog_spell_school_types[school] or ""
end

---@param flag integer
---@return integer index
function M33kAuras.RaidFlagToIndex(flag)
...
---@return number b
---@return number a
function M33kAuras.GetHSVTransition(perc, r1, g1, b1, a1, r2, g2, b2, a2)
...
straightHSV = [[
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    return M33kAuras.GetHSVTransition(progress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    local angle = (progress * 2 * math.pi) - (math.pi / 2)
    local newProgress = ((math.sin(angle) + 1)/2);
    return M33kAuras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
M33kAuras.class_ids = {}
M33kAuras.classes_sorted = {}
for classID = 1, 20 do -- GetNumClasses not supported by wow classic
  local classInfo = C_CreatureInfo.GetClassInfo(classID)
  if classInfo then
    M33kAuras.class_ids[classInfo.classFile] = classInfo.classID
    tinsert(M33kAuras.classes_sorted, classInfo.classFile)
  end
end
table.sort(M33kAuras.classes_sorted)

if M33kAuras.IsClassicOrWrathOrCata() then
  ---@param index integer
  ---@param extraOption boolean?
  ---@return boolean? hasTalent
  function M33kAuras.CheckTalentByIndex(index, extraOption)
...
if M33kAuras.IsRetail() then
  ...
  ---@return number? rank
  function M33kAuras.GetTalentById(talentId)
  ...
  ---@param talentId integer
  ---@return boolean hasTalent
  function M33kAuras.CheckTalentId(talentId)
...
---@return boolean hasTalent
---@return number? spellid
function M33kAuras.CheckPvpTalentBySpellId(spellId)
...
function M33kAuras.CheckNumericIds(loadids, currentId)
...
function M33kAuras.ValidateNumeric(info, val)
...
function M33kAuras.ValidateTime(info, val)
...
function M33kAuras.TimeToSeconds(val)
...
---@param val string
---@return boolean result
function M33kAuras.ValidateNumericOrPercent(info, val)
  if val ~= nil and val ~= "" then
...
---@return boolean result
function M33kAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(spell)
...
---@private
function M33kAuras.IsSpellKnownForLoad(spell, exact)
  if spell == 0 or spell >= 2^31 then return false end
  ...
  if (spellName) then
    ...
    if otherSpell and otherSpell ~= spell then
      return M33kAuras.IsSpellKnownForLoad(otherSpell)
    end
  end
end
...
---@param pet boolean?
---@return boolean result
function M33kAuras.IsSpellKnown(spell, pet)
  if spell == 0 or spell >= 2^31 then return false end
  if (pet) then
...
---@param spell string|number
---@return boolean result
function M33kAuras.IsSpellKnownIncludingPet(spell)
  ...
  return M33kAuras.IsSpellKnown(spell, false) or M33kAuras.IsSpellKnown(spell, true)
end

if M33kAuras.IsMists() then
  function M33kAuras.IsGlyphActive(glyphID)
...
function M33kAuras.GetNumSetItemsEquipped(setID)
...
local function valuesForTalentFunction(trigger)
  return function()
    ...
    elseif M33kAuras.IsWrathOrCata() then
...
Private.load_prototype = {
  ...
  args = {
    ...
    },
    {
      ...
      type = "tristate",
      init = M33kAuras.IsRetail() and "arg" or nil,
      width = M33kAuras.normalWidth,
      optional = true,
    ...
    },
    {
      ...
      type = "tristate",
      init = M33kAuras.IsMistsOrRetail() and "arg" or nil,
      width = M33kAuras.normalWidth,
      optional = true,
      enable = M33kAuras.IsWrathOrMistsOrRetail(),
      hidden = not M33kAuras.IsWrathOrMistsOrRetail(),
    ...
    },
    {
      ...
      type = "tristate",
      init = M33kAuras.IsWrathOrCataOrMistsOrRetail() and "arg" or nil,
      width = M33kAuras.normalWidth,
      optional = true,
      enable = M33kAuras.IsWrathOrCataOrMistsOrRetail(),
      hidden = not M33kAuras.IsWrathOrCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      display = L["Hardcore"],
      type = "tristate",
      init = M33kAuras.IsClassicEra() and "arg" or nil,
      width = M33kAuras.normalWidth,
      enable = M33kAuras.IsClassicEra(),
      hidden = not M33kAuras.IsClassicEra(),
    },
    {
      name = "engraving",
    ...
    },
    {
      ...
      type = "multiselect",
      values = "class_types",
      init = not M33kAuras.IsRetail() and "arg" or nil,
      enable = not M33kAuras.IsRetail(),
    ...
    },
    {
      ...
      init = M33kAuras.IsCataOrMistsOrRetail() and "arg" or nil,
      enable = M33kAuras.IsCataOrMistsOrRetail(),
      hidden = not M33kAuras.IsCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      extraOption = M33kAuras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      test = "M33kAuras.CheckTalentId(%d) == (%d == 4)",
      enableTest = function(trigger, talent, arg)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
      ...
      enable = function(trigger)
        return M33kAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil
      end,
      hidden = function(trigger)
        return not (M33kAuras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
    },
    {
      name = "pvptalent",
      ...
      test = "M33kAuras.CheckPvpTalentBySpellId(%d)",
      enable = M33kAuras.IsRetail(),
      hidden = not M33kAuras.IsRetail(),
    ...
    },
    {
      ...
      test = "M33kAuras.IsGlyphActive(%s)",
      events = {"GLYPH_ADDED", "GLYPH_REMOVED", "GLYPH_UPDATED", "USE_GLYPH"},
      enable = M33kAuras.IsMists(),
      hidden = not M33kAuras.IsMists(),
    },
    {
      name = "spellknown",
      display = L["Spell Known"],
      type = "spell",
      test = "M33kAuras.IsSpellKnownForLoad(%s, %s)",
      events = M33kAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
      name = "not_spellknown",
      display = M33kAuras.newFeatureString .. L["|cFFFF0000Not|r Spell Known"],
      type = "spell",
      test = "not M33kAuras.IsSpellKnownForLoad(%s, %s)",
      events = M33kAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
    ...
    {
      name = "spec_position",
      display = M33kAuras.newFeatureString .. L["Spec Position"],
      type = "multiselect",
    ...
    {
      name = "group_leader",
      display = M33kAuras.newFeatureString .. L["Group Leader/Assist"],
      type = "multiselect",
      init = "arg",
    ...
    },
    {
      ...
      test = "M33kAuras.CheckNumericIds(%q, encounterid)",
      events = {"ENCOUNTER_START", "ENCOUNTER_END"},
      optional = true,
    },
    {
    ...
    },
    {
      ...
      init = not M33kAuras.IsClassicEra() and "arg" or nil,
      enable = not M33kAuras.IsClassicEra(),
      hidden = M33kAuras.IsClassicEra(),
    ...
    },
    {
      ...
      display = M33kAuras.newFeatureString .. L["|cFFFF0000Not|r Item Equipped"],
      type = "item",
      multiEntry = {
        operator = "or"
      },
    ...
    },
    {
      ...
      type = "string",
      test = "M33kAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    },
    {
      name = "not_item_bonusid_equipped",
      display =  M33kAuras.newFeatureString .. L["|cFFFF0000Not|r Item Bonus Id Equipped"],
      type = "string",
      test = "not M33kAuras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    }
  }
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        isPet = M33kAuras.UnitIsPet(unit)
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        ...
        if (includePets ~= nil and isPet) or (includePets ~= "PetsOnly" and not isPet) then
          if unitisunit then
            M33kAuras.WatchUnitChange(unitisunit)
          end
          M33kAuras.WatchUnitChange(unit)
        end
      end
    else
      if unitisunit then
      ...
      M33kAuras.WatchUnitChange(triggerUnit)
    end
  end
end
...
local function AddUnitSpecChangeInternalEvents(triggerUnit, t)
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      local isPet = M33kAuras.UnitIsPet(unit)
      if (not isPet) then
...
local function AddUnitRoleChangeInternalEvents(triggerUnit, t)
  ...
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      if not M33kAuras.UnitIsPet(unit) then
        tinsert(t, "UNIT_ROLE_CHANGED_" .. string.lower(unit))
      end
    end
  else
    if not M33kAuras.UnitIsPet(triggerUnit) then
...
Private.event_prototypes = {
  ["Unit Characteristics"] = {
    ...
    args = {
      ...
      },
      {
        ...
        sorted = true,
        conditionType = "select",
        enable = M33kAuras.IsMistsOrRetail(),
        hidden = not M33kAuras.IsMistsOrRetail(),
      },
      {
        name = "creatureFamily",
  ...
  ["Faction Reputation"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = M33kAuras.IsRetail() and function(trigger)
  ...
  ["Power"] = {
    ...
    internal_events = function(trigger)
      ...
      if M33kAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 4 then
    ...
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        if not M33kAuras.IsRetail() and powerType == 99 then powerType = 1 end
      ...
      elseif M33kAuras.IsMists() and powerType == 99 then
        table.insert(ret, ([[
          local power = UnitStagger(unit) or 0
          local scaleStagger = %s
          local total = math.max(1, UnitHealthMax(unit) * scaleStagger)
        ]]):format(trigger.use_scaleStagger and trigger.scaleStagger or 1))
      elseif M33kAuras.IsMists() and (powerType == 14 or powerType == 7) then
        table.insert(ret, [[
      ...
      if (canEnableShowCost and trigger.use_showCost) then
        table.insert(ret, [[
          if (event == "UNIT_DISPLAYPOWER") then
            local cost = M33kAuras.GetSpellCost(powerTypeToCheck)
    ...
    args = {
      ...
      },
      {
        ...
        reloadOptions = true,
        enable = function(trigger)
          return M33kAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4
        end,
        hidden = not M33kAuras.IsRetail()
      },
      {
      ...
      },
      {
        ...
        store = true,
        enable = function(trigger)
          return M33kAuras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26
        end,
      },
      {
        name = "power",
    ...
    overlayFuncs = {
      ...
      },
      {
        ...
        enable = function(trigger)
          return M33kAuras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4 and trigger.use_showChargedComboPoints
        end,
      },
    },
    automaticrequired = true
  ...
  ["Alternate Power"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        unit = string.lower(unit)
        local unitname, realm = M33kAuras.UnitNameWithRealm(unit)
        local smart = %s
      ]=]
    ...
    args = {
      ...
      {
        hidden = true,
        test = "name and M33kAuras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ["Spell Activation Overlay"] = {
    ...
    loadFunc = function(trigger)
      if (trigger.use_exact_spellName) then
        M33kAuras.WatchSpellActivation(tonumber(trigger.spellName));
      else
        M33kAuras.WatchSpellActivation(type(trigger.spellName) == "number" and Private.ExecEnv.GetSpellName(trigger.spellName) or trigger.spellName);
      end
    end,
    init = function(trigger)
      local spellName
    ...
    args = {
      ...
      {
        hidden = true,
        test = "M33kAuras.SpellActivationActive(spellName)";
      }
    },
    automaticrequired = true,
    progressType = "static"
  ...
  ["Cooldown Progress (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      M33kAuras.WatchSpellCooldown(spellName, trigger.use_matchedRune, exactMatch, followoverride)
      if (trigger.use_showgcd) then
        M33kAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        local name, _, icon = Private.ExecEnv.GetSpellInfo(effectiveSpellId)
        local startTime, duration, gcdCooldown, readyTime, modRate, paused = M33kAuras.GetSpellCooldown(effectiveSpellId, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = M33kAuras.GetSpellCharges(effectiveSpellId, ignoreSpellKnown)
    ...
    args = {
      ...
      },
      {
        ...
        display = M33kAuras.newFeatureString .. L["Show Loss of Control"],
        type = "toggle",
        test = "true",
        collapse = "extra Cooldown Progress (Spell)",
      },
      ...
      },
      {
        ...
        conditionType = "bool",
        conditionTest = function(state, needle)
          return state and state.show and (UnitExists('target') and state.spellname and M33kAuras.IsSpellInRange(state.spellname, 'target') == needle)
        end,
        conditionEvents = AddTargetConditionEvents({
          "WA_SPELL_RANGECHECK",
        }),
      ...
      },
      {
        ...
        init = 'M33kAuras.IsSpellReadyFromDuration(effectiveSpellId)',
        store = true,
        test = "true",
        conditionType = "bool",
      },
  ...
  ["Cooldown Ready (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      local followoverride = not trigger.use_ignoreoverride
      M33kAuras.WatchSpellCooldown(spellName, false, useExact, followoverride)
    end,
    init = function(trigger)
      local spellName = type(trigger.spellName) ~= "table" and trigger.spellName or 0
      if type(trigger.spellName) == "string" then
  ...
  ["Cooldown Progress (Item)"] = {
    ...
    loadFunc = function(trigger)
      M33kAuras.WatchItemCooldown(trigger.itemName or 0)
      if (trigger.use_showgcd) then
        M33kAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local showgcd = %s
        local startTime, duration, enabled, gcdCooldown = M33kAuras.GetItemCooldown(itemname, showgcd);
    ...
    args = {
      ...
      {
        name = "itemId",
        display = M33kAuras.newFeatureString .. L["ItemId"],
        hidden = true,
        init = "itemId",
        test = "true",
        store = true,
  ...
  ["Cooldown Progress (Equipment Slot)"] = {
    ...
    loadFunc = function(trigger)
      M33kAuras.WatchItemSlotCooldown(trigger.itemSlot);
      if (trigger.use_showgcd) then
        M33kAuras.WatchGCD();
      end
    end,
    init = function(trigger)
      local ret = [[
        local showgcd = %s
        local itemSlot = %s
        local startTime, duration, enable, gcdCooldown = M33kAuras.GetItemSlotCooldown(itemSlot, showgcd)
        local expirationTime = startTime + duration
        local genericShowOn = %s
  ...
  ["Global Cooldown"] = {
    ...
    init = function(trigger)
      local ret = [[
        local inverse = %s;
        local duration, expirationTime, name, icon = M33kAuras.GetGCDInfo()
        local hasSpellName = M33kAuras.GcdSpellName();
      ]];
  ...
  ["Talent Known"] = {
    ...
    init = function(trigger)
      ...
      if (M33kAuras.IsTWW() and trigger.use_herotalent == false) then
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        init = "M33kAuras.IsRetail() and Private.ExecEnv.GetSpecialization()",
        required = true,
        values = function(trigger)
          return M33kAuras.spec_types_specific[trigger.class]
        end,
        enable = function(trigger)
          if M33kAuras.IsRetail() and trigger.use_class and trigger.class then
            return true
          else
            return false
          end
      ...
      },
      {
        ...
        type = "multiselect",
        values = function(trigger)
          if not M33kAuras.IsTWW() then return {} end
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
              classId = i
        ...
        enable = function(trigger)
          return M33kAuras.IsTWW() and trigger.use_class and trigger.class and trigger.use_spec and trigger.spec and true or false
        end,
        test = "heroActive",
        reloadOptions = true,
      },
    ...
    init = function(trigger)
      ...
      if (trigger.use_talent) then
        ...
        table.insert(ret, ([[
          ...
          local active, spellId = M33kAuras.CheckPvpTalentBySpellId(index)
          if spellId then
        ...
        if (trigger.talent.multi) then
          ...
          for index in pairs(trigger.talent.multi) do
            table.insert(ret, ([[
              if (not active) then
                local index = %s
                active, spellId = M33kAuras.CheckPvpTalentBySpellId(index)
  ...
  ["Item Count"] = {
    ...
    loadFunc = function(trigger)
      if(trigger.use_includeCharges) then
        M33kAuras.RegisterItemCountWatch();
      end
    end,
    init = function(trigger)
      local itemName = type(trigger.itemName) == "number" and trigger.itemName or string.format("%q", trigger.itemName or "0")
      local ret = [[
        ...
        if M33kAuras.IsRetail() and itemName then
      ...
      return ret:format(
        ...
        M33kAuras.IsRetail() and trigger.use_includeWarBandBank and "true" or "nil"
      )
    end,
    args = {
      {
  ...
  ["Stance/Form/Aura"] = {
    ...
    nameFunc = function(trigger)
      ...
      if(class == trigger.class) then
        ...
        local types = M33kAuras[class:lower().."_form_types"];
        if(types) then
  ...
  ["Weapon Enchant"] = {
    ...
    name = M33kAuras.IsRetail() and L["Weapon Enchant / Fishing Lure"] or L["Weapon Enchant"],
    init = function(trigger)
      M33kAuras.TenchInit();
      ...
      local ret = [[
        ...
        if triggerWeaponType == "main" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33kAuras.GetMHTenchInfo()
        elseif triggerWeaponType == "off" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33kAuras.GetOHTenchInfo()
        elseif triggerWeaponType == "ranged" and M33kAuras.IsCataClassic() then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33kAuras.GetRangeTenchInfo()
        end
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return not M33kAuras.IsRetail() and (not trigger.showOn or trigger.showOn == "showOnActive")
        end,
        hidden = M33kAuras.IsRetail(),
        store = true
      },
  ...
  ["Encounter Events"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        values = "difficulty_types",
        test = "%q == M33kAuras.InstanceDifficulty()",
        conditionType = "select",
        conditionTest = function(state, needle)
          return M33kAuras.InstanceDifficulty() == needle
        end,
        store = true,
        init = "arg"
      },
  ...
  ["Evoker Essence"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local genericShowOn = %s
        local duration, expirationTime, remaining, paused, essence, total = M33kAuras.GetEssenceCooldown(triggerEssence)
        local ready = paused
  ...
  ["Death Knight Rune"] = {
    ...
    loadFunc = function(trigger)
      ...
      if (trigger.use_rune) then
        M33kAuras.WatchRuneCooldown(trigger.rune);
      else
        for i = 1, 6 do
          M33kAuras.WatchRuneCooldown(i);
        end
      end
    end,
    init = function(trigger)
      ...
      if M33kAuras.IsWrathOrCataOrMists() then
        ret = [[
          ...
          local startTime, duration = M33kAuras.GetRuneCooldown(rune);
        ...
        ret = [[
          ...
          local numRunes = 0;
          for index = 1, 6 do
            local startTime = M33kAuras.GetRuneCooldown(index);
            if startTime == 0 then
              numRunes = numRunes + 1;
            end
          end
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        conditionType = "bool",
        enable = function(trigger) return M33kAuras.IsWrathOrCataOrMists() and trigger.use_rune end,
        hidden = not M33kAuras.IsWrathOrCataOrMists()
      },
      {
        name = "remaining",
        display = L["Remaining Time"],
      ...
      },
      {
        ...
        store = true,
        conditionType = "number",
        enable = function(trigger) return M33kAuras.IsWrathOrCataOrMists() and not trigger.use_rune end,
        hidden = not M33kAuras.IsWrathOrCataOrMists()
      },
      {
        name = "frostRunes",
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = function(trigger) return M33kAuras.IsWrathOrCataOrMists() and trigger.use_bloodRunes or trigger.use_unholyRunes or trigger.use_frostRunes end,
        hidden = not M33kAuras.IsWrathOrCataOrMists()
      },
    },
    durationFunc = function(trigger)
      if trigger.use_rune then
        local startTime, duration = M33kAuras.GetRuneCooldown(trigger.rune)
  ...
  ["Item Equipped"] = {
    ...
    init = function(trigger)
      ...
      ret = ret ..[[
        ...
        local equipped = M33kAuras.CheckForItemEquipped(triggerItemName, itemSlot)
      ]]
    ...
    args = {
      ...
      {
        name = "itemSlot",
        display = M33kAuras.newFeatureString .. L["Item Slot"],
        type = "select",
        values = "item_slot_types",
        test = "true",
      },
  ...
  ["Item Bonus Id Equipped"] = {
    ...
    init = function(trigger)
      local ret = [=[
        ...
        local itemBonusId, itemId, itemName, icon, itemSlot, itemSlotString = M33kAuras.GetBonusIdInfo(item, useItemSlot and slotSelected)
        local itemBonusId = tonumber(itemBonusId)
        if fetchLegendaryPower then
          itemName, icon = M33kAuras.GetLegendaryData(itemBonusId or item)
        end
  ...
  ["Item Set"] = {
    ...
    init = function(trigger)
      local ret = [[
        local setid = %s
        local value, total, name = M33kAuras.GetNumSetItemsEquipped(setid)
      ]]
      return string.format(ret, trigger.itemSetId and tonumber(trigger.itemSetId) or "0");
    end,
    GetNameAndIcon = function(trigger)
      local name = select(3, M33kAuras.GetNumSetItemsEquipped(trigger.itemSetId and tonumber(trigger.itemSetId) or 0))
      return name, nil
    end,
    statesParameter = "one",
    args = {
  ...
  ["Equipment Set"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local itemSetName, icon, numEquipped, numItems = M33kAuras.GetEquipmentSetInfo(useItemSetName and triggerItemSetName or nil, partial);
      ]];
    ...
    GetNameAndIcon = function(trigger)
      local name, icon = M33kAuras.GetEquipmentSetInfo(trigger.use_itemSetName and trigger.itemSetName or nil, true)
      return name, icon
    end,
    statesParameter = "one",
    args = {
  ...
  ["Cast"] = {
    ...
    loadFunc = function(trigger)
      if trigger.use_showLatency and trigger.unit == "player" then
        M33kAuras.WatchForCastLatency()
      end
      if trigger.unit == "nameplate" and trigger.use_onUpdateUnitTarget then
        M33kAuras.WatchForNameplateTargetChange()
      end
      local includePets = trigger.use_includePets == true and trigger.includePets or nil
      AddWatchedUnits(trigger.unit, includePets)
    end,
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local sourceName, sourceRealm = M33kAuras.UnitNameWithRealm(unit)
        local destName, destRealm = M33kAuras.UnitNameWithRealm(destUnit)
    ...
    args = {
      ...
      },
      {
        ...
        display = M33kAuras.newFeatureString .. L["Advanced Caster's Target Check"],
    ...
    overlayFuncs = {
      {
        name = L["Latency"],
        func = function(trigger, state)
          local latency = M33kAuras.GetCastLatency()
    ...
    GetNameAndIcon = function(trigger)
      ...
      if trigger.use_spellNames and type(trigger.spellNames) == "table" then
        for _, spellName in ipairs(trigger.spellNames) do
          spellId = M33kAuras.SafeToNumber(spellName)
          if spellId then
      ...
      if trigger.use_spellIds and type(trigger.spellIds) == "table" then
        for _, spellIdString in ipairs(trigger.spellIds) do
          spellId = M33kAuras.SafeToNumber(spellIdString)
          if spellId then
  ...
  ["Character Stats"] = {
    ...
    loadFunc = function(trigger)
      if trigger.use_moveSpeed then
        M33kAuras.WatchForPlayerMoving()
      end
    end,
    init = function()
      local ret = [[
    ...
    args = {
      ...
      },
      {
        ...
        enable = M33kAuras.IsClassicOrCataOrMists(),
        hidden = not M33kAuras.IsClassicOrCataOrMists(),
      },
      {
        type = "header",
  ...
  ["Conditions"] = {
    ...
    args = {
      ...
      {
        name = "vehicle",
        display = not M33kAuras.IsWrathOrCataOrMistsOrRetail() and L["On Taxi"] or L["In Vehicle"],
        type = "tristate",
        init = not M33kAuras.IsWrathOrCataOrMistsOrRetail() and "UnitOnTaxi('player')" or "UnitInVehicle('player')",
      },
      {
        name = "resting",
        display = L["Resting"],
      ...
      },
      {
        ...
        type = "multiselect",
        values = "difficulty_types",
        init = "M33kAuras.InstanceDifficulty()",
        enable = M33kAuras.IsRetail(),
        hidden = not M33kAuras.IsRetail(),
      },
      {
        ...
        type = "multiselect",
        values = "instance_difficulty_types",
        init = "M33kAuras.InstanceTypeRaw()",
        enable = M33kAuras.IsRetail(),
        hidden = not M33kAuras.IsRetail(),
      },
      {
        ...
        display = L["Secret Restrictions Active"],
        type = "tristate",
        init = "M33kAuras.IsSecretStateActive()",
      },
    },
    automaticrequired = true,
    progressType = "none"
  ...
  ["Spell Known"] = {
    type = "spell",
    events = {
      ["events"] = M33kAuras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED","PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED"},
      ["unit_events"] = {
        ["player"] = {"UNIT_PET"}
      }
    },
    ...
    init = function(trigger)
      ...
      if (trigger.use_inverse) then
        table.insert(ret, ([[
          local usePet = %s;
          local active = not spellName or not M33kAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      else
        table.insert(ret, ([[
          local usePet = %s;
          local active = spellName and M33kAuras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      end
      return table.concat(ret)
    end,
    ...
    args = {
      ...
      {
        name = "inverse",
        display = M33kAuras.newFeatureString .. L["Inverse"],
        type = "toggle",
        test = "true",
      },
      {
  ...
  ["Money"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = M33kAuras.newFeatureString..L["Player Money"],
    init = function()
      return [=[
  ...
  ["Currency"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = M33kAuras.newFeatureString..L["Currency"],
    triggerFunction = function(trigger)
  ...
  ["Location"] = {
    ...
    name = M33kAuras.newFeatureString..L["Location"],
    init = function(trigger)
      local ret = [=[
