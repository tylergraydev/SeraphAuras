...
if LibSpec then
  ...
  frame:SetScript("OnEvent", function()
    ...
    if IsInRaid() then
      ...
      for i = 1, max do
        local name = GetUnitName(M33kAuras.raidUnits[i], true)
        nameToUnitMap[name] = M33kAuras.raidUnits[i]
      end
    else
      local max = GetNumSubgroupMembers()
      for i = 1, max do
        local name = GetUnitName(M33kAuras.partyUnits[i], true)
        nameToUnitMap[name] = M33kAuras.partyUnits[i]
      end
    end
...
M33kAuras.SpecForUnit = Private.LibSpecWrapper.SpecForUnit
M33kAuras.SpecRolePositionForUnit = Private.LibSpecWrapper.SpecRolePositionForUnit
M33kAuras.CheckTalentForUnit = Private.LibSpecWrapper.CheckTalentForUnit

if Private.LibSpecWrapper.CheckGlyphForUnit then
  M33kAuras.CheckGlyphForUnit = Private.LibSpecWrapper.CheckGlyphForUnit
else
  M33kAuras.CheckGlyphForUnit = function() return nil end
end
