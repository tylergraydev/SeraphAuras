...
Private.time_format_types = {
  [0] = L["M33kAuras Built-In (63:42 | 3:07 | 10 | 2.4)"],
...
Private.format_types = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      if abbreviate and pad then
        return function(input)
          return M33kAuras.PadString(M33kAuras.WA_Utf8Sub(input, abbreviateMax), padMode, padLength)
        end
      elseif pad then
        return function(input)
          return M33kAuras.PadString(input, padMode, padLength)
        end
      elseif abbreviate then
        return function(input)
          return M33kAuras.WA_Utf8Sub(input, abbreviateMax)
        end
      end
      return nil
    end
  ...
  BigNumber = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      if pad then
        return function(input)
          return M33kAuras.PadString(formatterFunc(input), padMode, padLength)
        end
      end
    ...
    CreateFormatter = function(symbol, get, withoutColor)
      ...
      if realm == "never" then
        nameFunc = function(unit)
          return unit and M33kAuras.UnitName(unit) or ""
        end
      elseif realm == "star" then
        nameFunc = function(unit)
          ...
          local name, realm = M33kAuras.UnitName(unit)
          if realm then
            return name .. "*"
          end
          return name or ""
      ...
      elseif realm == "always" then
        nameFunc = function(unit)
          ...
          local name, realm = M33kAuras.UnitNameWithRealmCustomName(unit)
          return name .. "-" .. realm
        end
      end
    ...
    CreateFormatter = function(symbol, get, withoutColor)
      ...
      if realm == "never" then
        nameFunc = function(name, realm)
          return M33kAuras.GetName(name)
        end
      elseif realm == "star" then
        nameFunc = function(name, realm)
          name = M33kAuras.GetName(name)
          if realm ~= "" then
            return name .. "*"
          end
          return name
      ...
      elseif realm == "always" then
        nameFunc = function(name, realm)
          name = M33kAuras.GetName(name)
          if realm == "" then
            realm = select(2, M33kAuras.UnitNameWithRealm("player"))
          end
          return name .. "-" .. realm
        end
      end
  ...
  GCDTime = {
    ...
    CreateFormatter = function(symbol, get)
      ...
      return function(value, state)
        ...
        local now = GetTime()
        if gcd then
          local gcdDuration, gcdExpirationTime = M33kAuras.GetGCDInfo()
        ...
        if cast then
          local _, _, _, _, endTime = M33kAuras.UnitCastingInfo("player")
        ...
        if channel then
          local _, _, _, _, endTime = M33kAuras.UnitChannelInfo("player")
          local castExpirationTime = endTime and endTime > 0 and (endTime / 1000) or 0
          if castExpirationTime > 0 then
            result = min(result, now + value - castExpirationTime)
          end
        ...
        return numberToStringFunc(result / M33kAuras.CalculatedGcdDuration())
      end
    end
  }
}
...
for classID = 1, 20 do -- 20 is for GetNumClasses() but that function doesn't exists on Classic
  ...
  if classInfo and classID ~=14 then -- 14 == Adventurer
    M33kAuras.class_types[classInfo.classFile] = WrapTextInColorCode(classInfo.className, WA_GetClassColor(classInfo.classFile))
  end
end


M33kAuras.race_types = {}
do
  local races = {
    [1] = true,
    [2] = true,
    ...
    [7] = true,
    [8] = true,
    [9] = not M33kAuras.IsClassicOrWrath() and true or nil, -- Goblin
    [10] = true,
    [11] = true,
    [22] = true,
    [24] = true,
  ...
  for raceId, enabled in pairs(races) do
    ...
    if raceInfo then
      M33kAuras.race_types[raceInfo.clientFileString] = raceInfo.raceName
    end
  end
end
...
elseif M33kAuras.IsWrathOrCataOrMists() then
...
M33kAuras.spec_types_specific = {}

---@type table<number, string>
Private.spec_types_all = {}
Private.specs_sorted = {}
local function update_specs()
  for _, classFileName in pairs(M33kAuras.classes_sorted) do
    local classID = M33kAuras.class_ids[classFileName]
    M33kAuras.spec_types_specific[classFileName] = {}
    local numSpecs = M33kAuras.IsCataClassic() and 3 or Private.ExecEnv.GetNumSpecializationsForClassID(classID) -- see https://github.com/Stanzilla/WoWUIBugs/issues/559
    for i = 1, numSpecs do
      local specId, tabName, _, icon = Private.ExecEnv.GetSpecializationInfoForClassID(classID, i);
      if tabName then
        tinsert(M33kAuras.spec_types_specific[classFileName], "|T"..(icon or "error")..":0|t "..(tabName or "error"));
...
Private.texture_types = {
  ...
  ["Icons"] = {
    ...
    ["240972"] = "Poison Skull",
    ["241049"] = "Star",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\cancel-icon.tga"] = "Cancel Icon",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\cancel-mark.tga"] = "Cancel Mark",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\emoji.tga"] = "Emoji",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\exclamation-mark.tga"] = "Exclamation Mark",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\eyes.tga"] = "Eyes",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\ok-icon.tga"] = "Ok Icon",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\targeting-mark.tga"] = "Targeting Mark",
  ...
  ["Beams"] = {
    ...
    ["369750"] = "Shadow Beam",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\rainbowbar"] = "Rainbow Bar",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\StripedTexture"] = "Striped Bar",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\stripe-bar.tga"] = "Striped Bar 2",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\stripe-rainbow-bar.tga"] = "Rainbow Bar 2",
  },
  ["Shapes"] = {
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_Smooth"] = "Smooth Circle",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_Smooth_Border"] = "Smooth Circle with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_Squirrel"] = "Spiralled Circle",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_Squirrel_Border"] = "Spiralled Circle with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_White"] = "Circle",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_White_Border"] = "Circle with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_Smooth"] = "Smooth Square",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_Smooth_Border"] = "Smooth Square with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_Smooth_Border2"] = "Smooth Square with Border 2",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_Squirrel"] = "Spiralled Square",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_Squirrel_Border"] = "Spiralled Square with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_White"] = "Square",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_White_Border"] = "Square with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_FullWhite"] = "Full White Square",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Triangle45"] = "45° Triangle",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Trapezoid"] = "Trapezoid",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\triangle-border.tga"] = "Triangle with Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\triangle.tga"] = "Triangle",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_Smooth2.tga"] = "Smooth Circle Small",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\circle_border5.tga"] = "Circle Border",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\ring_glow3.tga"] = "Circle Border Glow",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\square_mini.tga"] = "Small Square",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\target_indicator.tga"] = "Target Indicator",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\target_indicator_glow.tga"] = "Target Indicator Glow",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\arrows_target.tga"] = "Arrows Target",

    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_AlphaGradient_In.tga"] = "Circle Alpha Gradient In",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Circle_AlphaGradient_Out.tga"] = "Circle Alpha Gradient Out",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Ring_10px.tga"] = "Ring 10px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Ring_20px.tga"] = "Ring 20px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Ring_30px.tga"] = "Ring 30px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Ring_40px.tga"] = "Ring 40px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_AlphaGradient.tga"] = "Square Alpha Gradient",

    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\square_border_1px.tga"] = "Square Border 1px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\square_border_5px.tga"] = "Square Border 5px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\square_border_10px.tga"] = "Square Border 10px",
  },
  ["Sparks"] = {
  ...
  ["M33kAuras"] = {
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\logo_64.tga"] = "M33kAuras logo 64px",
    ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\logo_256.tga"] = "M33kAuras logo 256px"
  }
}

if C_Texture and C_Texture.GetAtlasElements then
  if M33kAuras.buildType == "dev" and Private.AtlasList then
    M33kAuras.prettyPrint("Private.AtlasList can be removed now.")
  end
...
if M33kAuras.IsClassicEra() then -- Classic
...
local PowerAurasPath = "Interface\\Addons\\M33kAuras\\PowerAurasMedia\\Auras\\"
...
Private.texture_types["PowerAuras Words"] = {
  ...
  [PowerAurasPath.."Aura44"] = "Buff",
  ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\interrupt"] = "Interrupt"
}
...
if M33kAuras.IsCataClassic() then
  ...
  for i = 1, 240 do
    ...
    if name then
      if instance_difficulty_names[i] then
        ...
        M33kAuras.prettyPrint(string.format("Unknown difficulty id found. You are probably running an outdated version. Debug Information: %s %s %s", i, name, type))
      end
    end
  end
end
...
if M33kAuras.IsWrathOrMistsOrRetail() then
  ---@type table<number, string>
...
-- register sounds
LSM:Register("sound", "Heartbeat Single", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\HeartbeatSingle.ogg")
LSM:Register("sound", "Batman Punch", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\BatmanPunch.ogg")
...
LSM:Register("sound", "Sharp Punch", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\SharpPunch.ogg")
LSM:Register("sound", "Water Drop", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\WaterDrop.ogg")
LSM:Register("sound", "Air Horn", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\AirHorn.ogg")
LSM:Register("sound", "Applause", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Applause.ogg")
LSM:Register("sound", "Banana Peel Slip", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\BananaPeelSlip.ogg")
LSM:Register("sound", "Blast", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Blast.ogg")
LSM:Register("sound", "Cartoon Voice Baritone", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\CartoonVoiceBaritone.ogg")
LSM:Register("sound", "Cartoon Walking", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\CartoonWalking.ogg")
LSM:Register("sound", "Cow Mooing", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\CowMooing.ogg")
LSM:Register("sound", "Ringing Phone", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\RingingPhone.ogg")
LSM:Register("sound", "Roaring Lion", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\RoaringLion.ogg")
LSM:Register("sound", "Shotgun", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Shotgun.ogg")
LSM:Register("sound", "Squish Fart", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\SquishFart.ogg")
LSM:Register("sound", "Temple Bell", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\TempleBellHuge.ogg")
LSM:Register("sound", "Torch", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Torch.ogg")
LSM:Register("sound", "Warning Siren", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\WarningSiren.ogg")
...
LSM:Register("sound", "Sheep Blerping", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\SheepBleat.ogg")
LSM:Register("sound", "Rooster Chicken Call", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\RoosterChickenCalls.ogg")
LSM:Register("sound", "Goat Bleeting", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\GoatBleating.ogg")
LSM:Register("sound", "Acoustic Guitar", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\AcousticGuitar.ogg")
LSM:Register("sound", "Synth Chord", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\SynthChord.ogg")
LSM:Register("sound", "Chicken Alarm", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\ChickenAlarm.ogg")
LSM:Register("sound", "Xylophone", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Xylophone.ogg")
LSM:Register("sound", "Drums", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Drums.ogg")
LSM:Register("sound", "Tada Fanfare", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\TadaFanfare.ogg")
LSM:Register("sound", "Squeaky Toy Short", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\SqueakyToyShort.ogg")
LSM:Register("sound", "Error Beep", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\ErrorBeep.ogg")
LSM:Register("sound", "Oh No", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\OhNo.ogg")
LSM:Register("sound", "Double Whoosh", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\DoubleWhoosh.ogg")
LSM:Register("sound", "Brass", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Brass.mp3")
LSM:Register("sound", "Glass", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Glass.mp3")

LSM:Register("sound", "Voice: Adds", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Adds.ogg")
LSM:Register("sound", "Voice: Boss", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Boss.ogg")
LSM:Register("sound", "Voice: Circle", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Circle.ogg")
LSM:Register("sound", "Voice: Cross", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Cross.ogg")
LSM:Register("sound", "Voice: Diamond", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Diamond.ogg")
LSM:Register("sound", "Voice: Don't Release", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\DontRelease.ogg")
LSM:Register("sound", "Voice: Empowered", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Empowered.ogg")
LSM:Register("sound", "Voice: Focus", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Focus.ogg")
LSM:Register("sound", "Voice: Idiot", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Idiot.ogg")
LSM:Register("sound", "Voice: Left", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Left.ogg")
LSM:Register("sound", "Voice: Moon", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Moon.ogg")
LSM:Register("sound", "Voice: Next", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Next.ogg")
LSM:Register("sound", "Voice: Portal", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Portal.ogg")
LSM:Register("sound", "Voice: Protected", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Protected.ogg")
LSM:Register("sound", "Voice: Release", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Release.ogg")
LSM:Register("sound", "Voice: Right", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Right.ogg")
LSM:Register("sound", "Voice: Run Away", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\RunAway.ogg")
LSM:Register("sound", "Voice: Skull", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Skull.ogg")
LSM:Register("sound", "Voice: Spread", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Spread.ogg")
LSM:Register("sound", "Voice: Square", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Square.ogg")
LSM:Register("sound", "Voice: Stack", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Stack.ogg")
LSM:Register("sound", "Voice: Star", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Star.ogg")
LSM:Register("sound", "Voice: Switch", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Switch.ogg")
LSM:Register("sound", "Voice: Taunt", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Taunt.ogg")
LSM:Register("sound", "Voice: Triangle", "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\Triangle.ogg")

local PowerAurasSoundPath = "Interface\\Addons\\M33kAuras\\PowerAurasMedia\\Sounds\\"
...
LSM.RegisterCallback(M33kAuras, "LibSharedMedia_Registered", function(_, mediatype, key)
...
LSM:Register("font", "Fira Mono Medium", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\FiraMono-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
-- Other Fira fonts
LSM:Register("font", "Fira Sans Black", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\FiraSans-Heavy.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Condensed Black", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\FiraSansCondensed-Heavy.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Condensed Medium", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\FiraSansCondensed-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "Fira Sans Medium", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\FiraSans-Medium.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "PT Sans Narrow Regular", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\PTSansNarrow-Regular.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)
LSM:Register("font", "PT Sans Narrow Bold", "Interface\\Addons\\M33kAuras\\Media\\Fonts\\PTSansNarrow-Bold.ttf", LSM.LOCALE_BIT_western + LSM.LOCALE_BIT_ruRU)

-- register plain white border
LSM:Register("border", "Square Full White", [[Interface\AddOns\M33kAuras\Media\Textures\Square_FullWhite.tga]])

LSM:Register("statusbar", "Clean", [[Interface\AddOns\M33kAuras\Media\Textures\Statusbar_Clean]])
LSM:Register("statusbar", "Stripes", [[Interface\AddOns\M33kAuras\Media\Textures\Statusbar_Stripes]])
LSM:Register("statusbar", "Thick Stripes", [[Interface\AddOns\M33kAuras\Media\Textures\Statusbar_Stripes_Thick]])
LSM:Register("statusbar", "Thin Stripes", [[Interface\AddOns\M33kAuras\Media\Textures\Statusbar_Stripes_Thin]])
LSM:Register("border", "Drop Shadow", [[Interface\AddOns\M33kAuras\Media\Textures\Border_DropShadow]])
...
Private.author_option_fields = {
  ...
  media = {
    mediaType = "sound",
    media = "Interface\\AddOns\\M33kAuras\\Media\\Sounds\\AirHorn.ogg"
  },
  multiselect = {
    default = {true},
    values = {"val1"},
...
M33kAuras.item_weapon_types = Private.item_weapon_types

M33kAuras.StopMotion = M33kAuras.StopMotion or {}
M33kAuras.StopMotion.texture_types = M33kAuras.StopMotion.texture_types or {}
M33kAuras.StopMotion.texture_data = M33kAuras.StopMotion.texture_data or {}

M33kAuras.StopMotion.texture_types.Basic = {
  ["Interface\\AddOns\\M33kAuras\\Media\\Textures\\stopmotion"] = "Example",
}

M33kAuras.StopMotion.texture_data["Interface\\AddOns\\M33kAuras\\Media\\Textures\\stopmotion"] = { count = 64, rows = 8, columns = 8 }


M33kAuras.StopMotion.animation_types = {
  loop = L["Loop"],
...
if M33kAuras.IsWrathOrCata() then
  for slot = 20, 28 do
    Private.item_slot_types[slot] = nil
  end
  Private.talent_extra_option_types[0] = nil
