...
local newAPI = M33kAuras.IsRetail()
...
local function UnitInSubgroupOrPlayer(unit, includePets)
  ...
  elseif includePets == "PlayersAndPets" then
    return UnitInSubgroup(M33kAuras.petUnitToUnit[unit] or unit) or UnitIsUnit("player", unit) or UnitIsUnit("pet", unit)
  elseif includePets == "PetsOnly" then
    return UnitInSubgroup(M33kAuras.petUnitToUnit[unit]) or UnitIsUnit("pet", unit)
  end
end
...
local function UpdateToolTipDataInMatchData(matchData, time)
  ...
  if matchData.unit and matchData.auraInstanceID then
    local dataInstanceID, tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = M33kAuras.GetAuraInstanceTooltipInfo(matchData.unit, matchData.auraInstanceID, matchData.filter)
  ...
  elseif matchData.unit and matchData.index and matchData.filter then
    local tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = M33kAuras.GetAuraTooltipInfo(matchData.unit, matchData.index, matchData.filter)
    changed = matchData.tooltip ~= tooltip or matchData.tooltip1 ~= tooltip1
      or matchData.tooltip2 ~= tooltip2 or matchData.tooltip3 ~= tooltip3 or matchData.tooltip4 ~= tooltip4
    matchData.tooltip, matchData.tooltip1, matchData.tooltip2, matchData.tooltip3, matchData.tooltip4 = tooltip, tooltip1, tooltip2, tooltip3, tooltip4
  end
...
local function UpdateMatchData(time, matchDataChanged, unit, index, auraInstanceID, filter, name, icon, stacks, debuffClass, duration, expirationTime, unitCaster, isStealable, isBossDebuff, isCastByPlayer, spellId, modRate, points)
  ...
  local debuffClassIcon = M33kAuras.EJIcons[debuffClass]
...
local function UpdateStateWithMatch(time, bestMatch, triggerStates, cloneId, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local debuffClassIcon = M33kAuras.EJIcons[bestMatch.debuffClass]
...
local function UpdateStateWithNoMatch(time, triggerStates, triggerInfo, cloneId, unit, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local fallbackName, fallbackIcon = BuffTrigger.GetNameAndIconSimple(M33kAuras.GetData(triggerInfo.id), triggerInfo.triggernum)
  if not triggerStates[cloneId] then
    triggerStates[cloneId] = {
      show = true,
      changed = true,
    ...
    if allUnits then
      ...
      return function()
        if raid then
          if i <= 40 then
            ...
            if includePets == "PlayersAndPets" then
              ret = pets and M33kAuras.raidpetUnits[i] or M33kAuras.raidUnits[i]
              pets = not pets
              if pets then
                i = i + 1
              end
            elseif includePets == "PetsOnly" then
              ret = M33kAuras.raidpetUnits[i]
              i = i + 1
            else -- raid
              ret = M33kAuras.raidUnits[i]
              i = i + 1
            end
            return ret
          end
        ...
        if i <= 4 then
          local ret
          if includePets == "PlayersAndPets" then
            ret = pets and M33kAuras.partypetUnits[i] or M33kAuras.partyUnits[i]
            pets = not pets
            if pets then
              i = i + 1
            end
          elseif includePets == "PetsOnly" then
            ret = M33kAuras.partypetUnits[i]
            i = i + 1
          else -- group
            ret = M33kAuras.partyUnits[i]
            i = i + 1
          end
          return ret
        end
...
local function TriggerInfoApplies(triggerInfo, unit)
  ...
  if M33kAuras.UnitIsPet(unit) then
    controllingUnit = M33kAuras.petUnitToUnit[unit]
  end
  ...
  if triggerInfo.raidRole then
    local raidRole = M33kAuras.UnitRaidRole(controllingUnit)
  ...
  if triggerInfo.hostility and M33kAuras.GetPlayerReaction(unit) ~= triggerInfo.hostility then
    return false
  end
  ...
  if triggerInfo.nameChecker and not triggerInfo.nameChecker:Check(M33kAuras.UnitNameWithRealm(unit)) then
    return false
  end
...
Private.frames["M33kAuras Buff2 Frame"] = Buff2Frame
...
local function EventHandler(frame, event, arg1, arg2, ...)
  ...
  elseif event == "UNIT_PET" then
    local pet = M33kAuras.unitToPetUnit[arg1]
    if pet then
...
if M33kAuras.IsCataOrMistsOrRetail() then
  Private.LibSpecWrapper.Register(function(unit)
    ...
    local deactivatedTriggerInfos = {}
    RecheckActiveForUnitType("group", unit, deactivatedTriggerInfos)
    RecheckActiveForUnitType("group", M33kAuras.unitToPetUnit[unit], deactivatedTriggerInfos)
    DeactivateScanFuncs(deactivatedTriggerInfos)
...
local function createScanFunc(trigger)
  ...
  if use_ignore_name then
    ...
    for index, spellName in ipairs(trigger.ignoreAuraNames) do
      local spellId = M33kAuras.SafeToNumber(spellName)
  ...
  if use_ignore_spellId then
    ...
    for index, spellId in ipairs(trigger.ignoreAuraSpellids) do
      local spell = M33kAuras.SafeToNumber(spellId)
      if spell then
  ...
  return {
    untrackableSoftTarget = {
      ...
      check = function(trigger)
        return M33kAuras.IsUntrackableSoftTarget(trigger.unit)
      end
    }
  }
end
...
function BuffTrigger.GetNameAndIcon(data, triggernum)
  ...
  if (not name or not icon and M33kAuras.spellCache) then
    local trigger = data.triggers[triggernum].trigger
    if trigger.useName and trigger.auranames then
      for index, spellName in ipairs(trigger.auranames) do
        icon = M33kAuras.spellCache.GetIcon(spellName)
        if icon then
...
function BuffTrigger.GetTriggerConditions(data, triggernum)
  ...
  result["nameCaster"] = {
    ...
    test = function(state, needle, op, preamble)
      return state.unitCaster and preamble:Check(M33kAuras.UnitNameWithRealm(state.unitCaster))
    end,
    operator_types = "none",
  }
...
function BuffTrigger.GetTriggerDescription(data, triggernum, namestable)
  ...
  if trigger.useName and trigger.auranames then
    for index, name in pairs(trigger.auranames) do
      ...
      local icon
      local spellId = M33kAuras.SafeToNumber(name)
      if spellId then
        icon = Private.ExecEnv.GetSpellIcon(spellId)
      else
        icon = M33kAuras.spellCache.GetIcon(name)
      end
...
M33kAuras.RegisterTriggerSystem({"aura2"}, BuffTrigger)
