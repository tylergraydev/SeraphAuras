BackdropTemplateM33kAurasMixin = { };
...
function BackdropTemplateM33kAurasMixin:OnBackdropLoaded()
...
function BackdropTemplateM33kAurasMixin:OnBackdropSizeChanged()
	if self.backdropInfo then
		self:SetupTextureCoordinates();
	end
end

function BackdropTemplateM33kAurasMixin:GetEdgeSize()
...
function BackdropTemplateM33kAurasMixin:SetupTextureCoordinates()
...
function BackdropTemplateM33kAurasMixin:SetupPieceVisuals(piece, setupInfo, pieceLayout)
...
function BackdropTemplateM33kAurasMixin:SetBorderBlendMode(blendMode)
...
function BackdropTemplateM33kAurasMixin:HasBackdropInfo(backdropInfo)
	return self.backdropInfo == backdropInfo;
end

function BackdropTemplateM33kAurasMixin:ClearBackdrop()
	if self.backdropInfo then
		for pieceName in pairs(textureUVs) do
			local region = self[pieceName];
			if region then
...
function BackdropTemplateM33kAurasMixin:ApplyBackdrop()
	...
	local layout = {
		...
		setupPieceVisualsFunction = BackdropTemplateM33kAurasMixin.SetupPieceVisuals,
	};
...
function BackdropTemplateM33kAurasMixin:SetBackdrop(backdropInfo)
...
function BackdropTemplateM33kAurasMixin:GetBackdrop()
	if self.backdropInfo then
...
function BackdropTemplateM33kAurasMixin:GetBackdropColor()
	if not self.backdropInfo then
		return;
	end
	if self.Center then
...
function BackdropTemplateM33kAurasMixin:SetBackdropColor(r, g, b, a)
	if not self.backdropInfo then
...
function BackdropTemplateM33kAurasMixin:GetBackdropBorderColor()
	if not self.backdropInfo then
		return
	end
...
function BackdropTemplateM33kAurasMixin:SetBackdropBorderColor(r, g, b, a)
	if not self.backdropInfo then
		-- Ideally this would throw an error here but the old API just failed silently
		return;
	end
