...
local function RunAnimation(key, anim, elapsed, time)
  ...
  if(anim.translateFunc) then
    local errorHandler = M33kAuras.IsOptionsOpen() and noopErrorHandler or Private.GetErrorHandlerUid(anim.auraUID, L["Slide Animation"])
  ...
  if(anim.alphaFunc) then
    local errorHandler = M33kAuras.IsOptionsOpen() and noopErrorHandler or Private.GetErrorHandlerUid(anim.auraUID, L["Fade Animation"])
  ...
  if(anim.scaleFunc) then
    local errorHandler = M33kAuras.IsOptionsOpen() and noopErrorHandler
...
Private.frames["M33kAuras Animation Frame"] = frame
...
function Private.Animate(namespace, uid, type, anim, region, inverse, onFinished, loop, cloneId)
  ...
  if(valid) then
    ...
    if(anim.use_translate) then
      ...
      if (anim.translateFunc) then
        translateFunc = M33kAuras.LoadFunction("return " .. anim.translateFunc, uid);
      else
    ...
    if(anim.use_alpha) then
      ...
      if (anim.alphaFunc) then
        alphaFunc = M33kAuras.LoadFunction("return " .. anim.alphaFunc, uid);
      else
    ...
    if(anim.use_scale) then
      ...
      if (anim.scaleFunc) then
        scaleFunc = M33kAuras.LoadFunction("return " .. anim.scaleFunc, uid);
      else
        region:Scale(1, 1);
      end
    ...
    if(anim.use_rotate) then
      ...
      if (anim.rotateFunc) then
        rotateFunc = M33kAuras.LoadFunction("return " .. anim.rotateFunc, uid);
      else
    ...
    if(anim.use_color) then
      ...
      if (anim.colorFunc) then
        colorFunc = M33kAuras.LoadFunction("return " .. anim.colorFunc, uid);
      else
