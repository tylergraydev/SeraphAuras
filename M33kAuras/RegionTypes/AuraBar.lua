...
local funcs = {
  ...
  SetProgressSecret = function(self)
    ...
    if self.secretProgress == "duration" and M33kAuras.IsDurationObject(self.durationObject) then
...
local function create(parent)
  ...
  secretBar:SetStatusBarTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_FullWhite");
  secretBar:SetColorFill(1, 1, 1, 0);
  local secretBarMask = bar:CreateMaskTexture()
  secretBarMask:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_FullWhite",
  ...
  fgMask:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\Square_FullWhite",
                    "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE", "NEAREST")
...
-- Register new region type with M33kAuras
