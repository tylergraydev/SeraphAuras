...
local function create(parent, data)
  ...
  --The reason is so that M33kAuras cooldown frames can interact properly with OmniCC
  -- (i.e., put on its ignore list for timer overlays)
  local id = data.id;
  local frameId = id:lower():gsub(" ", "_");
  if(_G["M33kAurasCooldown"..frameId]) then
    local baseFrameId = frameId;
    local num = 2;
    while(_G["M33kAurasCooldown"..frameId]) do
  ...
  local cooldown = CreateFrame("Cooldown", "M33kAurasCooldown"..frameId, region, "CooldownFrameTemplate");
  ...
  then
    ElvUI[1]:RegisterCooldown(cooldown, "M33kAuras");
  end
...
local function modify(parent, region, data)
  ...
  if MSQ then
    ...
    if region.masqueId ~= masqueId then
      ...
      region.MSQGroup = MSQ:Group("M33kAuras", region.masqueId, data.uid);
