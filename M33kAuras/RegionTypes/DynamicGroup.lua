...
local function create(parent)
  ...
  region.sortStates = {}
  region.growStates = {}
  local background = CreateFrame("Frame", nil, region, "BackdropTemplateM33kAuras")
...
function M33kAuras.GetPolarCoordinates(x, y, originX, originY)
...
function M33kAuras.InvertSort(sortFunc)
...
function M33kAuras.SortNilLast(a, b)
...
local sortNilFirst = M33kAuras.InvertSort(M33kAuras.SortNilLast)
function M33kAuras.SortNilFirst(a, b)
  if a == nil and b == nil then
...
function M33kAuras.SortGreaterLast(a, b)
...
M33kAuras.SortGreaterFirst = M33kAuras.InvertSort(M33kAuras.SortGreaterLast)

function M33kAuras.SortRegionData(path, sortFunc)
  ...
  if type(sortFunc) ~= "function" then
    ...
    sortFunc = M33kAuras.SortGreaterLast
  end
  return function(a, b)
...
function M33kAuras.SortAscending(path)
  return M33kAuras.SortRegionData(path, M33kAuras.ComposeSorts(M33kAuras.SortNilFirst, M33kAuras.SortGreaterLast))
end

function M33kAuras.SortDescending(path)
  return M33kAuras.InvertSort(M33kAuras.SortAscending(path))
end

function M33kAuras.ComposeSorts(...)
...
local sorters = {
  none = function(data)
    return M33kAuras.ComposeSorts(
      M33kAuras.SortAscending({"dataIndex"}),
      M33kAuras.SortAscending({"region", "state", "index"})
    ), { index = true }
  end,
  hybrid = function(data)
    ...
    if hybridSortAscending then
      sortExpirationTime = M33kAuras.SortAscending({"region", "state", "expirationTime"})
    else
      sortExpirationTime = M33kAuras.SortDescending({"region", "state", "expirationTime"})
    end
    return M33kAuras.ComposeSorts(
      sortHybridStatus,
      sortExpirationTime,
  ...
  ascending = function(data)
    return M33kAuras.ComposeSorts(
      M33kAuras.SortAscending({"region", "state", "expirationTime"}),
      M33kAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  descending = function(data)
    return M33kAuras.ComposeSorts(
      M33kAuras.SortDescending({"region", "state", "expirationTime"}),
      M33kAuras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  custom = function(data)
    local sortStr = data.customSort or ""
    local sortFunc = M33kAuras.LoadFunction("return " .. sortStr, data.id) or noop
    local sortOn = nil
    local events = M33kAuras.split(data.sortOn or "")
    if #events > 0 then
      sortOn = {}
...
M33kAuras.SortFunctions = sorters
...
local anchorers = {
  ["NAMEPLATE"] = function(data)
    return function(frames, activeRegions)
      for _, regionData in ipairs(activeRegions) do
        ...
        local found
        if unit then
          local frame = M33kAuras.GetUnitNameplate(unit)
          if frame then
        ...
        if not found and M33kAuras.IsOptionsOpen() and regionData.region.state then
  ...
  ["CUSTOM"] = function(data)
    ...
    local anchorFunc = M33kAuras.LoadFunction("return " .. anchorStr, data.id) or noop

    local anchorOn = nil
    local events = M33kAuras.split(data.anchorOn or "")
    if #events > 0 then
      anchorOn = {}
      for _, event in ipairs(events) do
        anchorOn[event] = true
...
local growers = {
  ...
  CUSTOM = function(data)
    ...
    local growFunc = M33kAuras.LoadFunction("return " .. growStr, data.id) or noop
    local growOn = nil
    local events = M33kAuras.split(data.growOn or "")
    if #events > 0 then
      growOn = {}
      for _, event in ipairs(events) do
        growOn[event] = true
...
M33kAuras.GrowFunctions = growers
...
local function modify(parent, region, data)
  ...
  function region:IsSuspended()
    return not M33kAuras.IsLoginFinished() or self.suspended > 0
  end
  ...
  function region:ReloadControlledChildren()
    ...
    if not self:IsSuspended() then
      ...
      for dataIndex, childID in ipairs(data.controlledChildren) do
        local childRegion, childData = M33kAuras.GetRegion(childID), M33kAuras.GetData(childID)
  ...
  function region:AddChild(childID, cloneID)
    ...
    local childRegion = M33kAuras.GetRegion(childID, cloneID)
  ...
  function region:DoPositionChildrenPerFrame(frame, positions, handledRegionData)
    for regionData, pos in pairs(positions) do
      ...
      controlPoint:SetShown(show and frame ~= M33kAuras.HiddenFrames)
      ...
      if animate then
        ...
        if show and (abs(xDelta) > 0.01 or abs(yDelta) > 0.01) then
          ...
          if data.grow == "CIRCLE" or data.grow == "COUNTERCIRCLE" then
            ...
            local radius1, previousAngle = M33kAuras.GetPolarCoordinates(xPrev, yPrev, originX, originY)
            local radius2, newAngle = M33kAuras.GetPolarCoordinates(x, y, originX, originY)
  ...
  function region:Resize()
    ...
    if not self:IsSuspended() then
      ...
      if isRestricted and not M33kAuras.IsOptionsOpen() then
