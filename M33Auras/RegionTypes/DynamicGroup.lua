...
local function create(parent)
  ...
  region.sortStates = {}
  region.growStates = {}
  local background = CreateFrame("Frame", nil, region, "BackdropTemplateM33Auras")
...
function M33Auras.GetPolarCoordinates(x, y, originX, originY)
...
function M33Auras.InvertSort(sortFunc)
...
function M33Auras.SortNilLast(a, b)
...
local sortNilFirst = M33Auras.InvertSort(M33Auras.SortNilLast)
function M33Auras.SortNilFirst(a, b)
  if a == nil and b == nil then
...
function M33Auras.SortGreaterLast(a, b)
...
M33Auras.SortGreaterFirst = M33Auras.InvertSort(M33Auras.SortGreaterLast)

function M33Auras.SortRegionData(path, sortFunc)
  ...
  if type(sortFunc) ~= "function" then
    ...
    sortFunc = M33Auras.SortGreaterLast
  end
  return function(a, b)
...
function M33Auras.SortAscending(path)
  return M33Auras.SortRegionData(path, M33Auras.ComposeSorts(M33Auras.SortNilFirst, M33Auras.SortGreaterLast))
end

function M33Auras.SortDescending(path)
  return M33Auras.InvertSort(M33Auras.SortAscending(path))
end

function M33Auras.ComposeSorts(...)
...
local sorters = {
  none = function(data)
    return M33Auras.ComposeSorts(
      M33Auras.SortAscending({"dataIndex"}),
      M33Auras.SortAscending({"region", "state", "index"})
    ), { index = true }
  end,
  hybrid = function(data)
    ...
    if hybridSortAscending then
      sortExpirationTime = M33Auras.SortAscending({"region", "state", "expirationTime"})
    else
      sortExpirationTime = M33Auras.SortDescending({"region", "state", "expirationTime"})
    end
    return M33Auras.ComposeSorts(
      sortHybridStatus,
      sortExpirationTime,
  ...
  ascending = function(data)
    return M33Auras.ComposeSorts(
      M33Auras.SortAscending({"region", "state", "expirationTime"}),
      M33Auras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  descending = function(data)
    return M33Auras.ComposeSorts(
      M33Auras.SortDescending({"region", "state", "expirationTime"}),
      M33Auras.SortAscending({"dataIndex"})
    ), {expirationTime = true}
  end,
  custom = function(data)
    local sortStr = data.customSort or ""
    local sortFunc = M33Auras.LoadFunction("return " .. sortStr, data.id) or noop
    local sortOn = nil
    local events = M33Auras.split(data.sortOn or "")
    if #events > 0 then
      sortOn = {}
...
M33Auras.SortFunctions = sorters
...
local anchorers = {
  ["NAMEPLATE"] = function(data)
    return function(frames, activeRegions)
      for _, regionData in ipairs(activeRegions) do
        ...
        local found
        if unit then
          local frame = M33Auras.GetUnitNameplate(unit)
          if frame then
        ...
        if not found and M33Auras.IsOptionsOpen() and regionData.region.state then
  ...
  ["CUSTOM"] = function(data)
    ...
    local anchorFunc = M33Auras.LoadFunction("return " .. anchorStr, data.id) or noop

    local anchorOn = nil
    local events = M33Auras.split(data.anchorOn or "")
    if #events > 0 then
      anchorOn = {}
      for _, event in ipairs(events) do
        anchorOn[event] = true
...
local growers = {
  ...
  CUSTOM = function(data)
    ...
    local growFunc = M33Auras.LoadFunction("return " .. growStr, data.id) or noop
    local growOn = nil
    local events = M33Auras.split(data.growOn or "")
    if #events > 0 then
      growOn = {}
      for _, event in ipairs(events) do
        growOn[event] = true
...
M33Auras.GrowFunctions = growers
...
local function modify(parent, region, data)
  ...
  function region:IsSuspended()
    return not M33Auras.IsLoginFinished() or self.suspended > 0
  end
  ...
  function region:ReloadControlledChildren()
    ...
    if not self:IsSuspended() then
      ...
      for dataIndex, childID in ipairs(data.controlledChildren) do
        local childRegion, childData = M33Auras.GetRegion(childID), M33Auras.GetData(childID)
  ...
  function region:AddChild(childID, cloneID)
    ...
    local childRegion = M33Auras.GetRegion(childID, cloneID)
  ...
  function region:DoPositionChildrenPerFrame(frame, positions, handledRegionData)
    for regionData, pos in pairs(positions) do
      ...
      controlPoint:SetShown(show and frame ~= M33Auras.HiddenFrames)
      ...
      if animate then
        ...
        if show and (abs(xDelta) > 0.01 or abs(yDelta) > 0.01) then
          ...
          if data.grow == "CIRCLE" or data.grow == "COUNTERCIRCLE" then
            ...
            local radius1, previousAngle = M33Auras.GetPolarCoordinates(xPrev, yPrev, originX, originY)
            local radius2, newAngle = M33Auras.GetPolarCoordinates(x, y, originX, originY)
  ...
  function region:Resize()
    ...
    if not self:IsSuspended() then
      ...
      if isRestricted and not M33Auras.IsOptionsOpen() then
