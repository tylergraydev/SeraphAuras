...
local function create(parent)
  ...
  -- Border region
  local border = CreateFrame("Frame", nil, region, "BackdropTemplateM33Auras");
...
local function ConfigureModel(region, model, data)
  ...
  -- Adjust model
  M33Auras.SetModel(model, nil, data.model_fileId, data.modelIsUnit, data.modelDisplayInfo)
  ...
  if data.modelIsUnit then
    ...
    elseif not M33Auras.IsClassicEra() and unit == "focus" then
