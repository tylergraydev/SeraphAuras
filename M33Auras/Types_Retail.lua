...
local classIDToOffsets = {
	[1] = { extraOffsetX = M33Auras.IsTWW() and 60 or 30, extraOffsetY = 31, }, -- Warrior
...
function Private.GetTalentData(specId)
  ...
  local specDataByNodeId = {}
	...
	local subTreeIDs = M33Auras.IsTWW() and C_ClassTalents.GetHeroTalentSpecsForClassSpec(configId, specId) or {}
...
for k, v in pairs(texture_data) do
	...
	if atlasInfo then
		...
		M33Auras.StopMotion.texture_data[k] = v
		M33Auras.StopMotion.texture_types.Blizzard[k] = k
	end
end
