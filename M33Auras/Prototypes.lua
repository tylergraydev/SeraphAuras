...
function M33Auras.IsSpellInRange(spellId, unit)
  return SpellRange.IsSpellInRange(spellId, unit)
end

local LibRangeCheck = LibStub("LibRangeCheck-3.0")

function M33Auras.GetRange(unit, checkVisible)
  return LibRangeCheck:GetRange(unit, checkVisible);
end

function M33Auras.CheckRange(unit, range, operator)
...
function M33Auras.UnitDetailedThreatSituation(unit1, unit2)
...
M33Auras.UnitCastingInfo = UnitCastingInfo

if M33Auras.IsRetail() then
  local cacheEmpowered = {}
  M33Auras.UnitChannelInfo = function(unit)
...
local constants = {
  ...
  instanceFilterDeprecated = L["This filter has been moved to the Location trigger. Change your aura to use the new Location trigger or join the M33Auras Discord server for help."],
...
if M33Auras.IsClassicOrWrathOrCataOrMists() then
  ...
  ---@return string? role
  M33Auras.UnitRaidRole = function(unit)
...
function M33Auras.SpellSchool(school)
  return Private.combatlog_spell_school_types[school] or ""
end

---@param flag integer
---@return integer index
function M33Auras.RaidFlagToIndex(flag)
...
---@return number b
---@return number a
function M33Auras.GetHSVTransition(perc, r1, g1, b1, a1, r2, g2, b2, a2)
...
straightHSV = [[
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    return M33Auras.GetHSVTransition(progress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
function(progress, r1, g1, b1, a1, r2, g2, b2, a2)
    local angle = (progress * 2 * math.pi) - (math.pi / 2)
    local newProgress = ((math.sin(angle) + 1)/2);
    return M33Auras.GetHSVTransition(newProgress, r1, g1, b1, a1, r2, g2, b2, a2)
end
]],
...
M33Auras.class_ids = {}
M33Auras.classes_sorted = {}
for classID = 1, 20 do -- GetNumClasses not supported by wow classic
  local classInfo = C_CreatureInfo.GetClassInfo(classID)
  if classInfo then
    M33Auras.class_ids[classInfo.classFile] = classInfo.classID
    tinsert(M33Auras.classes_sorted, classInfo.classFile)
  end
end
table.sort(M33Auras.classes_sorted)

if M33Auras.IsClassicOrWrathOrCata() then
  ---@param index integer
  ---@param extraOption boolean?
  ---@return boolean? hasTalent
  function M33Auras.CheckTalentByIndex(index, extraOption)
...
if M33Auras.IsRetail() then
  ...
  Private.frames["M33Auras talentCheckFrame"] = talentCheckFrame
  ...
  ---@return number? rank
  function M33Auras.GetTalentById(talentId)
  ...
  ---@param talentId integer
  ---@return boolean hasTalent
  function M33Auras.CheckTalentId(talentId)
...
---@return boolean hasTalent
---@return number? spellid
function M33Auras.CheckPvpTalentBySpellId(spellId)
...
function M33Auras.CheckNumericIds(loadids, currentId)
...
function M33Auras.ValidateNumeric(info, val)
...
function M33Auras.ValidateTime(info, val)
...
function M33Auras.TimeToSeconds(val)
...
---@param val string
---@return boolean result
function M33Auras.ValidateNumericOrPercent(info, val)
  if val ~= nil and val ~= "" then
...
---@return boolean result
function M33Auras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(spell)
...
---@private
function M33Auras.IsSpellKnownForLoad(spell, exact)
  if spell == 0 or spell >= 2^31 then return false end
  ...
  if (spellName) then
    ...
    if otherSpell and otherSpell ~= spell then
      return M33Auras.IsSpellKnownForLoad(otherSpell)
    end
  end
end
...
---@param pet boolean?
---@return boolean result
function M33Auras.IsSpellKnown(spell, pet)
  if spell == 0 or spell >= 2^31 then return false end
  if (pet) then
...
---@param spell string|number
---@return boolean result
function M33Auras.IsSpellKnownIncludingPet(spell)
  ...
  return M33Auras.IsSpellKnown(spell, false) or M33Auras.IsSpellKnown(spell, true)
end

if M33Auras.IsMists() then
  function M33Auras.IsGlyphActive(glyphID)
...
function M33Auras.GetNumSetItemsEquipped(setID)
...
---@return number result
function M33Auras.GetEffectiveAttackPower()
  local base, pos, neg = UnitAttackPower("player")
  return base + pos + neg
end

--- @type fun(): number
function M33Auras.GetEffectiveSpellPower()
...
local function valuesForTalentFunction(trigger)
  return function()
    ...
    elseif M33Auras.IsWrathOrCata() then
...
Private.load_prototype = {
  ...
  args = {
    ...
    },
    {
      ...
      type = "tristate",
      init = M33Auras.IsRetail() and "arg" or nil,
      width = M33Auras.normalWidth,
      optional = true,
      enable = M33Auras.IsRetail(),
      hidden = not M33Auras.IsRetail(),
    ...
    },
    {
      ...
      type = "tristate",
      init = M33Auras.IsWrathClassic() and "arg" or nil,
      width = M33Auras.normalWidth,
      optional = true,
      enable = M33Auras.IsWrathClassic(),
      hidden = not M33Auras.IsWrathClassic(),
    ...
    },
    {
      ...
      type = "tristate",
      init = M33Auras.IsMistsOrRetail() and "arg" or nil,
      width = M33Auras.normalWidth,
      optional = true,
      enable = M33Auras.IsWrathOrMistsOrRetail(),
      hidden = not M33Auras.IsWrathOrMistsOrRetail(),
      events = {"PET_BATTLE_OPENING_START", "PET_BATTLE_CLOSE"}
    },
    {
      name = "vehicle",
      display = (M33Auras.IsClassicEra()) and L["On Taxi"] or L["In Vehicle"],
      type = "tristate",
      init = "arg",
      width = M33Auras.normalWidth,
      optional = true,
      events = (M33Auras.IsClassicEra()) and {"UNIT_FLAGS"}
    ...
    },
    {
      ...
      type = "tristate",
      init = M33Auras.IsWrathOrCataOrMistsOrRetail() and "arg" or nil,
      width = M33Auras.normalWidth,
      optional = true,
      enable = M33Auras.IsWrathOrCataOrMistsOrRetail(),
      hidden = not M33Auras.IsWrathOrCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      display = L["Hardcore"],
      type = "tristate",
      init = M33Auras.IsClassicEra() and "arg" or nil,
      width = M33Auras.normalWidth,
      enable = M33Auras.IsClassicEra(),
      hidden = not M33Auras.IsClassicEra(),
    },
    {
      name = "engraving",
    ...
    },
    {
      ...
      type = "multiselect",
      values = "class_types",
      init = not M33Auras.IsRetail() and "arg" or nil,
      enable = not M33Auras.IsRetail(),
      hidden = M33Auras.IsRetail(),
    },
    {
      name = "class_and_spec",
      ...
      init = M33Auras.IsCataOrMistsOrRetail() and "arg" or nil,
      enable = M33Auras.IsCataOrMistsOrRetail(),
      hidden = not M33Auras.IsCataOrMistsOrRetail(),
    ...
    },
    {
      ...
      test = M33Auras.IsRetail() and "M33Auras.CheckTalentId(%d) == (%d == 4)" or "M33Auras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
        if M33Auras.IsRetail() then
          ...
          return M33Auras.CheckTalentByIndex(talent, arg) ~= nil
        end
      end,
      multiConvertKey = M33Auras.IsRetail() and function(trigger, key)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
          local talentData = Private.GetTalentData(specId)
      ...
      events = (M33Auras.IsClassicOrWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE", "ACTIVE_TALENT_GROUP_CHANGED"})
        or (M33Auras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        -- Check for multi select!
        return M33Auras.IsClassicEra() and (load.talent_extraOption == 2 or load.talent_extraOption == 3)
      end,
      extraOption = M33Auras.IsClassicEra() and {
        display = "",
        values = function()
          return Private.talent_extra_option_types
        end
      },
      control = M33Auras.IsWrathOrCataOrMistsOrRetail() and "M33AurasMiniTalent" or nil,
      multiNoSingle = M33Auras.IsWrathOrCataOrMistsOrRetail(), -- no single mode
      multiTristate = M33Auras.IsWrathOrCataOrMistsOrRetail(), -- values can be true/false/nil
      multiAll = M33Auras.IsWrathOrCataOrMistsOrRetail(), -- require all tests
      orConjunctionGroup = M33Auras.IsWrathOrCataOrMistsOrRetail() and "talent",
      multiUseControlWhenFalse = M33Auras.IsWrathOrCataOrMistsOrRetail(),
      enable = function(trigger)
        return M33Auras.IsClassicEra()
            or (M33Auras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (M33Auras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
      hidden = function(trigger)
        return not (
            M33Auras.IsClassicEra()
            or (M33Auras.IsWrathOrCataOrMists() and Private.checkForSingleLoadCondition(trigger, "class") ~= nil)
            or (M33Auras.IsRetail() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil))
      end,
    },
    {
      name = "talent2",
      display = M33Auras.IsWrathOrCataOrMistsOrRetail() and L["Or Talent"] or L["And Talent"],
      type = "multiselect",
      values = valuesForTalentFunction,
      test = M33Auras.IsRetail() and "M33Auras.CheckTalentId(%d) == (%d == 4)" or "M33Auras.CheckTalentByIndex(%d, %d)",
      enableTest = function(trigger, talent, arg)
      ...
      events = (M33Auras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        or (M33Auras.IsWrathOrCataOrMists() and {"CHARACTER_POINTS_CHANGED", "PLAYER_TALENT_UPDATE"})
        or (M33Auras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return M33Auras.IsClassicEra() and (load.talent2_extraOption == 2 or load.talent2_extraOption == 3)
      end,
      extraOption = M33Auras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      events = (M33Auras.IsClassicEra() and {"CHARACTER_POINTS_CHANGED"})
        ...
        or (M33Auras.IsRetail() and {"WA_TALENT_UPDATE"}),
      inverse = function(load)
        return M33Auras.IsClassicEra() and (load.talent3_extraOption == 2 or load.talent3_extraOption == 3)
      end,
      extraOption = M33Auras.IsClassicEra() and {
        display = "",
        values = function()
    ...
    },
    {
      ...
      test = "M33Auras.CheckTalentId(%d) == (%d == 4)",
      enableTest = function(trigger, talent, arg)
        local specId = Private.checkForSingleLoadCondition(trigger, "class_and_spec")
        if specId then
      ...
      enable = function(trigger)
        return M33Auras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil
      end,
      hidden = function(trigger)
        return not (M33Auras.IsTWW() and Private.checkForSingleLoadCondition(trigger, "class_and_spec") ~= nil)
      end,
    },
    {
      name = "pvptalent",
      ...
      test = "M33Auras.CheckPvpTalentBySpellId(%d)",
      enable = M33Auras.IsRetail(),
      hidden = not M33Auras.IsRetail(),
    ...
    },
    {
      ...
      test = "M33Auras.IsGlyphActive(%s)",
      events = {"GLYPH_ADDED", "GLYPH_REMOVED", "GLYPH_UPDATED", "USE_GLYPH"},
      enable = M33Auras.IsMists(),
      hidden = not M33Auras.IsMists(),
    },
    {
      name = "spellknown",
      display = L["Spell Known"],
      type = "spell",
      test = "M33Auras.IsSpellKnownForLoad(%s, %s)",
      events = M33Auras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
      name = "not_spellknown",
      display = M33Auras.newFeatureString .. L["|cFFFF0000Not|r Spell Known"],
      type = "spell",
      test = "not M33Auras.IsSpellKnownForLoad(%s, %s)",
      events = M33Auras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED", "UNIT_PET", "PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED", "UNIT_PET"},
      showExactOption = true
    },
    {
    ...
    {
      name = "spec_position",
      display = M33Auras.newFeatureString .. L["Spec Position"],
      type = "multiselect",
    ...
    },
    {
      ...
      init = M33Auras.IsClassicOrWrathOrCataOrMists() and "arg" or nil,
      enable = M33Auras.IsClassicOrWrathOrCataOrMists(),
      hidden = M33Auras.IsRetail(),
    ...
    {
      name = "group_leader",
      display = M33Auras.newFeatureString .. L["Group Leader/Assist"],
      type = "multiselect",
      init = "arg",
    ...
    },
    {
      ...
      test = "M33Auras.CheckNumericIds(%q, encounterid)",
      events = {"ENCOUNTER_START", "ENCOUNTER_END"},
      optional = true,
    },
    {
    ...
    },
    {
      ...
      init = not M33Auras.IsClassicEra() and "arg" or nil,
      enable = not M33Auras.IsClassicEra(),
      hidden = M33Auras.IsClassicEra(),
    ...
    },
    {
      ...
      display = M33Auras.newFeatureString .. L["|cFFFF0000Not|r Item Equipped"],
      type = "item",
      multiEntry = {
        operator = "or"
      },
    ...
    },
    {
      ...
      type = "string",
      test = "M33Auras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    },
    {
      name = "not_item_bonusid_equipped",
      display =  M33Auras.newFeatureString .. L["|cFFFF0000Not|r Item Bonus Id Equipped"],
      type = "string",
      test = "not M33Auras.CheckForItemBonusId(%q)",
      events = { "UNIT_INVENTORY_CHANGED", "PLAYER_EQUIPMENT_CHANGED"},
      desc = L["Supports multiple entries, separated by commas"]
    }
  }
  ...
  elseif triggerUnit == "pet" then
    ...
    if Private.multiUnitUnits[triggerUnit] then
      ...
      for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
        ...
        if (includePets ~= nil and isPet) or (includePets ~= "PetsOnly" and not isPet) then
          if unitisunit then
            M33Auras.WatchUnitChange(unitisunit)
          end
          M33Auras.WatchUnitChange(unit)
        end
      end
    else
      if unitisunit then
      ...
      M33Auras.WatchUnitChange(triggerUnit)
    end
  end
end
...
local function AddUnitSpecChangeInternalEvents(triggerUnit, t)
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      local isPet = M33Auras.UnitIsPet(unit)
      if (not isPet) then
...
local function AddUnitRoleChangeInternalEvents(triggerUnit, t)
  ...
  if Private.multiUnitUnits[triggerUnit] then
    for unit in pairs(Private.multiUnitUnits[triggerUnit]) do
      if not M33Auras.UnitIsPet(unit) then
        tinsert(t, "UNIT_ROLE_CHANGED_" .. string.lower(unit))
      end
    end
  else
    if not M33Auras.UnitIsPet(triggerUnit) then
...
Private.event_prototypes = {
  ["Unit Characteristics"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local smart = %s
        local extraUnit = %q;
        local name, realm = M33Auras.UnitNameWithRealm(unit)
      ]=];
    ...
    args = {
      ...
      },
      {
        ...
        type = "multiselect",
        init = "M33Auras.SpecForUnit(unit)",
        values = "spec_types_all",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return M33Auras.IsCataOrMistsOrRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end,
      ...
      },
      {
        ...
        sorted = true,
        conditionType = "select",
        enable = M33Auras.IsMistsOrRetail(),
        hidden = not M33Auras.IsMistsOrRetail(),
      },
      {
        name = "creatureFamily",
      ...
      },
      {
        ...
        display = L["Raid Role"],
        type = "select",
        init = "M33Auras.UnitRaidRole(unit)",
        values = "raid_role_types",
        store = true,
        conditionType = "select",
        enable = function(trigger)
          return M33Auras.IsClassicOrWrathOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      -- {
      --   name = "raidMarkIndex",
      ...
      },
      {
        ...
        width = M33Auras.doubleWidth,
        enable = function(trigger)
          return M33Auras.IsRetail() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end,
      ...
      },
      {
        ...
        display = L["Hostility"],
        type = "select",
        init = "M33Auras.GetPlayerReaction(unit)",
      ...
      {
        hidden = true,
        test = "M33Auras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
  ...
  ["Faction Reputation"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = M33Auras.IsRetail() and function(trigger)
  ...
  ["Power"] = {
    ...
    internal_events = function(trigger)
      ...
      if M33Auras.IsClassicOrWrath() and trigger.use_showCost and trigger.unit == "player" then
        tinsert(result, "WA_UNIT_QUEUED_SPELL_CHANGED");
      end
      if M33Auras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26 then
        tinsert(result, "ECLIPSE_DIRECTION_CHANGE");
      end
      if M33Auras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 4 then
    ...
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        if not M33Auras.IsRetail() and powerType == 99 then powerType = 1 end
      ...
      elseif M33Auras.IsMists() and powerType == 99 then
        table.insert(ret, ([[
          local power = UnitStagger(unit) or 0
          local scaleStagger = %s
          local total = math.max(1, UnitHealthMax(unit) * scaleStagger)
        ]]):format(trigger.use_scaleStagger and trigger.scaleStagger or 1))
      elseif M33Auras.IsMists() and (powerType == 14 or powerType == 7) then
        table.insert(ret, [[
      ...
      if (canEnableShowCost and trigger.use_showCost) then
        table.insert(ret, [[
          if (event == "UNIT_DISPLAYPOWER") then
            local cost = M33Auras.GetSpellCost(powerTypeToCheck)
    ...
    args = {
      ...
      },
      {
        ...
        reloadOptions = true,
        enable = function(trigger)
          return M33Auras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4
        end,
        hidden = not M33Auras.IsRetail()
      },
      {
      ...
      },
      {
        ...
        store = true,
        enable = function(trigger)
          return M33Auras.IsCataOrMists() and trigger.unit == "player" and trigger.use_powertype and trigger.powertype == 26
        end,
      },
      {
        name = "power",
      ...
      {
        name = "maxpower",
        display = M33Auras.newFeatureString .. L["Max Power"],
        type = "number",
        init = "total",
        store = true,
        conditionType = "number",
      ...
      },
      {
        ...
        conditionType = "select",
        enable = function(trigger)
          return M33Auras.IsClassicOrCataOrMists() and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        end
      },
      -- {
      --   name = "raidMarkIndex",
    ...
    overlayFuncs = {
      ...
      },
      {
        ...
        enable = function(trigger)
          return M33Auras.IsRetail() and trigger.unit == 'player' and trigger.use_powertype and trigger.powertype == 4 and trigger.use_showChargedComboPoints
        end,
      },
    },
    automaticrequired = true
  ...
  ["Alternate Power"] = {
    ...
    init = function(trigger)
      ...
      local ret = [=[
        unit = string.lower(unit)
        local unitname, realm = M33Auras.UnitNameWithRealm(unit)
        local smart = %s
      ]=]
    ...
    args = {
      ...
      {
        hidden = true,
        test = "name and M33Auras.UnitExistsFixed(unit, smart) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ["Spell Activation Overlay"] = {
    ...
    loadFunc = function(trigger)
      if (trigger.use_exact_spellName) then
        M33Auras.WatchSpellActivation(tonumber(trigger.spellName));
      else
        M33Auras.WatchSpellActivation(type(trigger.spellName) == "number" and Private.ExecEnv.GetSpellName(trigger.spellName) or trigger.spellName);
      end
    end,
    init = function(trigger)
      local spellName
    ...
    args = {
      ...
      {
        hidden = true,
        test = "M33Auras.SpellActivationActive(spellName)";
      }
    },
    automaticrequired = true,
    progressType = "static"
  ...
  ["Cooldown Progress (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      M33Auras.WatchSpellCooldown(spellName, trigger.use_matchedRune, exactMatch, followoverride)
      if (trigger.use_showgcd) then
        M33Auras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      table.insert(ret, ([=[
        ...
        local name, _, icon = Private.ExecEnv.GetSpellInfo(effectiveSpellId)
        local startTime, duration, gcdCooldown, readyTime, modRate, paused = M33Auras.GetSpellCooldown(effectiveSpellId, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = M33Auras.GetSpellCharges(effectiveSpellId, ignoreSpellKnown)
    ...
    args = {
      ...
      },
      {
        ...
        display = M33Auras.newFeatureString .. L["Show Loss of Control"],
        type = "toggle",
        test = "true",
        collapse = "extra Cooldown Progress (Spell)",
      },
      ...
      },
      {
        ...
        conditionType = "bool",
        conditionTest = function(state, needle)
          return state and state.show and (UnitExists('target') and state.spellname and M33Auras.IsSpellInRange(state.spellname, 'target') == needle)
        end,
        conditionEvents = AddTargetConditionEvents({
          "WA_SPELL_RANGECHECK",
        }),
  ...
  ["Cooldown Ready (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      local followoverride = not trigger.use_ignoreoverride
      M33Auras.WatchSpellCooldown(spellName, false, useExact, followoverride)
    end,
    init = function(trigger)
      local spellName = type(trigger.spellName) ~= "table" and trigger.spellName or 0
      if type(trigger.spellName) == "string" then
  ...
  ["Cooldown Progress (Item)"] = {
    ...
    loadFunc = function(trigger)
      M33Auras.WatchItemCooldown(trigger.itemName or 0)
      if (trigger.use_showgcd) then
        M33Auras.WatchGCD();
      end
    end,
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local showgcd = %s
        local startTime, duration, enabled, gcdCooldown = M33Auras.GetItemCooldown(itemname, showgcd);
    ...
    args = {
      ...
      {
        name = "itemId",
        display = M33Auras.newFeatureString .. L["ItemId"],
        hidden = true,
        init = "itemId",
        test = "true",
        store = true,
  ...
  ["Cooldown Progress (Equipment Slot)"] = {
    ...
    loadFunc = function(trigger)
      M33Auras.WatchItemSlotCooldown(trigger.itemSlot);
      if (trigger.use_showgcd) then
        M33Auras.WatchGCD();
      end
    end,
    init = function(trigger)
      local ret = [[
        local showgcd = %s
        local itemSlot = %s
        local startTime, duration, enable, gcdCooldown = M33Auras.GetItemSlotCooldown(itemSlot, showgcd)
        local expirationTime = startTime + duration
        local genericShowOn = %s
  ...
  ["Global Cooldown"] = {
    ...
    init = function(trigger)
      local ret = [[
        local inverse = %s;
        local duration, expirationTime, name, icon = M33Auras.GetGCDInfo()
        local hasSpellName = M33Auras.GcdSpellName();
      ]];
  ...
  ["Action Usable"] = {
    ...
    init = function(trigger)
      ...
      if trigger.use_ignoreSpellCooldown then
        ...
        ret = ret .. [=[
        local startTime, duration, gcdCooldown, readyTime, paused = M33Auras.GetSpellCooldown(effectiveSpellId, nil, nil, nil, nil)
        local charges, maxCharges, spellCount, chargeGainTime, chargeLostTime = M33Auras.GetSpellCharges(effectiveSpellId, nil)
      ...
      if(trigger.use_targetRequired) then
        ret = ret.."active = active and M33Auras.IsSpellInRange(spellName or '', 'target')\n";
      end
  ...
  ["Talent Known"] = {
    ...
    events = function()
      ...
      elseif M33Auras.IsWrathOrCataOrMists() then
        events = {
          "CHARACTER_POINTS_CHANGED",
          "SPELLS_CHANGED",
    ...
    internal_events = M33Auras.IsRetail() and  {"WA_TALENT_UPDATE"} or nil,
    force_events = (M33Auras.IsRetail() and "TRAIT_CONFIG_UPDATED") or "CHARACTER_POINTS_CHANGED",
    name = L["Talent Known"],
    init = function(trigger)
      local inverse = trigger.use_inverse and not M33Auras.IsMistsOrRetail()
      local ret = {}
          ...
          for index, value in pairs(trigger.talent.multi) do
            ...
            elseif M33Auras.IsRetail() then
              table.insert(ret, ([[
                ...
                local shouldBeActive = %s
                if talentId then
                  activeName, activeIcon, _, rank = M33Auras.GetTalentById(talentId)
      ...
      if (M33Auras.IsTWW() and trigger.use_herotalent == false) then
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        init = "M33Auras.IsRetail() and Private.ExecEnv.GetSpecialization()",
        required = true,
        values = function(trigger)
          return M33Auras.spec_types_specific[trigger.class]
        end,
        enable = function(trigger)
          if M33Auras.IsRetail() and trigger.use_class and trigger.class then
            return true
          else
            return false
          end
      ...
      },
      {
        ...
        multiUseControlWhenFalse = M33Auras.IsMistsOrRetail(),
        multiAll = M33Auras.IsMistsOrRetail(),
        multiNoSingle = M33Auras.IsMistsOrRetail(),
        multiTristate = M33Auras.IsMistsOrRetail(), -- values can be true/false/nil
        control = M33Auras.IsMistsOrRetail() and "M33AurasMiniTalent" or nil,
        multiConvertKey = M33Auras.IsRetail() and function(trigger, key)
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
      ...
      },
      {
        ...
        type = "multiselect",
        values = function(trigger)
          if not M33Auras.IsTWW() then return {} end
          local classId
          for i = 1, GetNumClasses() do
            if select(2, GetClassInfo(i)) == trigger.class then
              classId = i
        ...
        enable = function(trigger)
          return M33Auras.IsTWW() and trigger.use_class and trigger.class and trigger.use_spec and trigger.spec and true or false
        end,
        test = "heroActive",
        reloadOptions = true,
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = not M33Auras.IsMistsOrRetail(),
        hidden = M33Auras.IsMistsOrRetail(),
      },
      {
        hidden = true,
        name = "icon",
    ...
    init = function(trigger)
      ...
      if (trigger.use_talent) then
        ...
        table.insert(ret, ([[
          ...
          local active, spellId = M33Auras.CheckPvpTalentBySpellId(index)
          if spellId then
        ...
        if (trigger.talent.multi) then
          ...
          for index in pairs(trigger.talent.multi) do
            table.insert(ret, ([[
              if (not active) then
                local index = %s
                active, spellId = M33Auras.CheckPvpTalentBySpellId(index)
  ...
  ["Totem"] = {
    ...
    triggerFunction = function(trigger)
      ...
      ret = ret:format(trigger.use_totemType and tonumber(trigger.totemType) or "nil",
        ...
        M33Auras.IsRetail() and trigger.use_totemSpellId and trigger.totemSpellId or "nil",
        M33Auras.IsRetail() and not trigger.use_ignoreoverride and "true" or "false",
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return trigger.use_totemSpellId and M33Auras.IsRetail()
        end,
      },
      {
        name = "icon",
  ...
  ["Item Count"] = {
    ...
    loadFunc = function(trigger)
      if(trigger.use_includeCharges) then
        M33Auras.RegisterItemCountWatch();
      end
    end,
    init = function(trigger)
      local itemName = type(trigger.itemName) == "number" and trigger.itemName or string.format("%q", trigger.itemName or "0")
      local ret = [[
        ...
        if M33Auras.IsRetail() and itemName then
      ...
      return ret:format(
        ...
        M33Auras.IsRetail() and trigger.use_includeWarBandBank and "true" or "nil"
      )
    end,
    args = {
      {
  ...
  ["Stance/Form/Aura"] = {
    ...
    nameFunc = function(trigger)
      ...
      if(class == trigger.class) then
        ...
        local types = M33Auras[class:lower().."_form_types"];
        if(types) then
  ...
  ["Weapon Enchant"] = {
    ...
    name = M33Auras.IsRetail() and L["Weapon Enchant / Fishing Lure"] or L["Weapon Enchant"],
    init = function(trigger)
      M33Auras.TenchInit();
      ...
      local ret = [[
        ...
        if triggerWeaponType == "main" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33Auras.GetMHTenchInfo()
        elseif triggerWeaponType == "off" then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33Auras.GetOHTenchInfo()
        elseif triggerWeaponType == "ranged" and M33Auras.IsCataClassic() then
          expirationTime, duration, name, shortenedName, icon, stacks, enchantID = M33Auras.GetRangeTenchInfo()
        end
    ...
    args = {
      ...
      },
      {
        ...
        test = "true",
        enable = function(trigger)
          return not M33Auras.IsRetail() and (not trigger.showOn or trigger.showOn == "showOnActive")
        end,
        hidden = M33Auras.IsRetail(),
        store = true
      },
  ...
  ["Chat Message"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        init = "use_cloneId and M33Auras.GetUniqueCloneId() or ''",
        reloadOptions = true
      },
    },
    countEvents = true,
  ...
  ["Encounter Events"] = {
    ...
    args = {
      ...
      },
      {
        ...
        type = "select",
        values = "difficulty_types",
        test = "%q == M33Auras.InstanceDifficulty()",
        conditionType = "select",
        conditionTest = function(state, needle)
          return M33Auras.InstanceDifficulty() == needle
        end,
        store = true,
        init = "arg"
      },
  ...
  ["Evoker Essence"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local genericShowOn = %s
        local duration, expirationTime, remaining, paused, essence, total = M33Auras.GetEssenceCooldown(triggerEssence)
        local ready = paused
  ...
  ["Death Knight Rune"] = {
    ...
    loadFunc = function(trigger)
      ...
      if (trigger.use_rune) then
        M33Auras.WatchRuneCooldown(trigger.rune);
      else
        for i = 1, 6 do
          M33Auras.WatchRuneCooldown(i);
        end
      end
    end,
    init = function(trigger)
      ...
      if M33Auras.IsWrathOrCataOrMists() then
        ret = [[
          ...
          local startTime, duration = M33Auras.GetRuneCooldown(rune);
        ...
        ret = [[
          ...
          local numRunes = 0;
          for index = 1, 6 do
            local startTime = M33Auras.GetRuneCooldown(index);
            if startTime == 0 then
              numRunes = numRunes + 1;
            end
          end
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        conditionType = "bool",
        enable = function(trigger) return M33Auras.IsWrathOrCataOrMists() and trigger.use_rune end,
        hidden = not M33Auras.IsWrathOrCataOrMists()
      },
      {
        name = "remaining",
        display = L["Remaining Time"],
      ...
      },
      {
        ...
        store = true,
        conditionType = "number",
        enable = function(trigger) return M33Auras.IsWrathOrCataOrMists() and not trigger.use_rune end,
        hidden = not M33Auras.IsWrathOrCataOrMists()
      },
      {
        name = "frostRunes",
      ...
      },
      {
        ...
        type = "toggle",
        test = "true",
        enable = function(trigger) return M33Auras.IsWrathOrCataOrMists() and trigger.use_bloodRunes or trigger.use_unholyRunes or trigger.use_frostRunes end,
        hidden = not M33Auras.IsWrathOrCataOrMists()
      },
    },
    durationFunc = function(trigger)
      if trigger.use_rune then
        local startTime, duration = M33Auras.GetRuneCooldown(trigger.rune)
  ...
  ["Item Equipped"] = {
    ...
    init = function(trigger)
      ...
      ret = ret ..[[
        ...
        local equipped = M33Auras.CheckForItemEquipped(triggerItemName, itemSlot)
      ]]
    ...
    args = {
      ...
      {
        name = "itemSlot",
        display = M33Auras.newFeatureString .. L["Item Slot"],
        type = "select",
        values = "item_slot_types",
        test = "true",
      },
  ...
  ["Item Bonus Id Equipped"] = {
    ...
    init = function(trigger)
      local ret = [=[
        ...
        local itemBonusId, itemId, itemName, icon, itemSlot, itemSlotString = M33Auras.GetBonusIdInfo(item, useItemSlot and slotSelected)
        local itemBonusId = tonumber(itemBonusId)
        if fetchLegendaryPower then
          itemName, icon = M33Auras.GetLegendaryData(itemBonusId or item)
        end
  ...
  ["Item Set"] = {
    ...
    init = function(trigger)
      local ret = [[
        local setid = %s
        local value, total, name = M33Auras.GetNumSetItemsEquipped(setid)
      ]]
      return string.format(ret, trigger.itemSetId and tonumber(trigger.itemSetId) or "0");
    end,
    GetNameAndIcon = function(trigger)
      local name = select(3, M33Auras.GetNumSetItemsEquipped(trigger.itemSetId and tonumber(trigger.itemSetId) or 0))
      return name, nil
    end,
    statesParameter = "one",
    args = {
      {
        ...
        desc = function()
          ...
          elseif M33Auras.IsClassicEra() then
  ...
  ["Equipment Set"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        local itemSetName, icon, numEquipped, numItems = M33Auras.GetEquipmentSetInfo(useItemSetName and triggerItemSetName or nil, partial);
      ]];
    ...
    GetNameAndIcon = function(trigger)
      local name, icon = M33Auras.GetEquipmentSetInfo(trigger.use_itemSetName and trigger.itemSetName or nil, true)
      return name, icon
    end,
    statesParameter = "one",
    args = {
  ...
  ["Threat Situation"] = {
    ...
    init = function(trigger)
      ...
      local ret = [[
        ...
        if unit and unit ~= "none" then
          aggro, status, threatpct, rawthreatpct, threatvalue = M33Auras.UnitDetailedThreatSituation('player', unit)
    ...
    args = {
      ...
      {
        hidden = true,
        test = "M33Auras.UnitExistsFixed(unit, false) and specificUnitCheck"
      }
    },
    automaticrequired = true
  },
  ...
  ["Cast"] = {
    ...
    loadFunc = function(trigger)
      if trigger.use_showLatency and trigger.unit == "player" then
        M33Auras.WatchForCastLatency()
      end
      if trigger.unit == "nameplate" and trigger.use_onUpdateUnitTarget then
        M33Auras.WatchForNameplateTargetChange()
      end
      local includePets = trigger.use_includePets == true and trigger.includePets or nil
      AddWatchedUnits(trigger.unit, includePets)
    end,
    ...
    init = function(trigger)
      ...
      local ret = [=[
        ...
        local sourceName, sourceRealm = M33Auras.UnitNameWithRealm(unit)
        local destName, destRealm = M33Auras.UnitNameWithRealm(destUnit)
        ...
        spell, _, icon, startTime, endTime, _, _, interruptible, spellId = M33Auras.UnitCastingInfo(unit)
        ...
        if spell then
          ...
          spell, _, icon, startTime, endTime, _, interruptible, spellId, _, stageTotal = M33Auras.UnitChannelInfo(unit)
           if issecretvalue(spell) then
            return false
          end
          if spell then
    ...
    args = {
      ...
      },
      {
        ...
        enable = M33Auras.IsRetail() and function(trigger) return not trigger.use_inverse end or false,
        hidden = not M33Auras.IsRetail()
      },
      {
        name = "empowered",
      ...
      },
      {
        ...
        display = M33Auras.newFeatureString .. L["Advanced Caster's Target Check"],
      ...
      {
        hidden = true,
        test = "M33Auras.UnitExistsFixed(unit, smart) and ((not inverseTrigger and spell) or (inverseTrigger and not spell)) and specificUnitCheck"
      },
      {
        name = "stagesData",
        hidden = true,
    ...
    overlayFuncs = {
      {
        name = L["Latency"],
        func = function(trigger, state)
          local latency = M33Auras.GetCastLatency()
      ...
      },
      {
        ...
        enable = function(trigger)
          return M33Auras.IsRetail() and trigger.use_showChargedDuration
        end
      }
    },
    GetNameAndIcon = function(trigger)
      ...
      if trigger.use_spellNames and type(trigger.spellNames) == "table" then
        for _, spellName in ipairs(trigger.spellNames) do
          spellId = M33Auras.SafeToNumber(spellName)
          if spellId then
      ...
      if trigger.use_spellIds and type(trigger.spellIds) == "table" then
        for _, spellIdString in ipairs(trigger.spellIds) do
          spellId = M33Auras.SafeToNumber(spellIdString)
          if spellId then
  ...
  ["Character Stats"] = {
    ...
    args = {
      ...
      },
      {
        ...
        store = true,
        enable = M33Auras.IsWrathOrCataOrMists(),
        conditionType = "number",
        hidden = not M33Auras.IsWrathOrCataOrMists(),
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        display = L["Mastery (%)"],
        type = "number",
        init = "M33Auras.IsCataOrMists() and GetMastery() or GetMasteryEffect()",
        store = true,
        enable = M33Auras.IsCataOrMistsOrRetail(),
        conditionType = "number",
        hidden = not M33Auras.IsCataOrMistsOrRetail(),
      ...
      },
      {
        ...
        type = "number",
        init = "M33Auras.GetEffectiveAttackPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
      ...
      },
      {
        ...
        type = "number",
        init = "M33Auras.GetEffectiveSpellPower()",
        store = true,
        conditionType = "number",
        multiEntry = {
          operator = "and",
        ...
        enable = M33Auras.IsClassicOrCataOrMists(),
        hidden = not M33Auras.IsClassicOrCataOrMists(),
      },
      {
        type = "header",
      ...
      },
      {
        ...
        type = "number",
        init = M33Auras.IsWrathClassic() and "GetCombatRating(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
      ...
      },
      {
        ...
        type = "number",
        init = M33Auras.IsWrathClassic() and "GetCombatRatingBonus(CR_RESILIENCE_PLAYER_DAMAGE_TAKEN)"
  ...
  ["Conditions"] = {
    ...
    args = {
      ...
      },
      {
        ...
        enable = M33Auras.IsWrathOrRetail(),
        hidden = not M33Auras.IsWrathOrRetail()
      },
      {
        name = "alive",
        display = L["Alive"],
      ...
      {
        name = "vehicle",
        display = not M33Auras.IsWrathOrCataOrMistsOrRetail() and L["On Taxi"] or L["In Vehicle"],
        type = "tristate",
        init = not M33Auras.IsWrathOrCataOrMistsOrRetail() and "UnitOnTaxi('player')" or "UnitInVehicle('player')",
      },
      {
        name = "resting",
        display = L["Resting"],
      ...
      },
      {
        ...
        type = "multiselect",
        values = "difficulty_types",
        init = "M33Auras.InstanceDifficulty()",
        enable = M33Auras.IsRetail(),
        hidden = not M33Auras.IsRetail(),
      },
      {
      ...
      },
      {
        ...
        display = L["Secret Restrictions Active"],
        type = "tristate",
        init = "M33Auras.IsSecretStateActive()",
      },
    },
    automaticrequired = true,
    progressType = "none"
  ...
  ["Spell Known"] = {
    type = "spell",
    events = {
      ["events"] = M33Auras.IsWrathOrCataOrMists() and {"SPELLS_CHANGED","PLAYER_TALENT_UPDATE"} or {"SPELLS_CHANGED"},
      ["unit_events"] = {
        ["player"] = {"UNIT_PET"}
      }
    },
    ...
    init = function(trigger)
      ...
      if (trigger.use_inverse) then
        table.insert(ret, ([[
          local usePet = %s;
          local active = not spellName or not M33Auras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      else
        table.insert(ret, ([[
          local usePet = %s;
          local active = spellName and M33Auras.IsSpellKnown(spellName, usePet)
        ]]):format(trigger.use_petspell and "true" or "false"))
      end
      return table.concat(ret)
    end,
    ...
    args = {
      ...
      {
        name = "inverse",
        display = M33Auras.newFeatureString .. L["Inverse"],
        type = "toggle",
        test = "true",
      },
      {
  ...
  ["Money"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = M33Auras.newFeatureString..L["Player Money"],
    init = function()
      return [=[
  ...
  ["Currency"] = {
    ...
    internal_events = {"WA_DELAYED_PLAYER_ENTERING_WORLD"},
    force_events = "WA_DELAYED_PLAYER_ENTERING_WORLD",
    name = M33Auras.newFeatureString..L["Currency"],
    triggerFunction = function(trigger)
  ...
  ["Location"] = {
    ...
    name = M33Auras.newFeatureString..L["Location"],
    init = function(trigger)
      local ret = [=[
...
if M33Auras.IsCataOrMists() then
