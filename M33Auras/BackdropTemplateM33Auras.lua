...
function BackdropTemplateM33AurasMixin:OnBackdropLoaded()
...
function BackdropTemplateM33AurasMixin:OnBackdropSizeChanged()
	if self.backdropInfo then
		self:SetupTextureCoordinates();
	end
end

function BackdropTemplateM33AurasMixin:GetEdgeSize()
...
function BackdropTemplateM33AurasMixin:SetupTextureCoordinates()
...
function BackdropTemplateM33AurasMixin:SetupPieceVisuals(piece, setupInfo, pieceLayout)
...
function BackdropTemplateM33AurasMixin:SetBorderBlendMode(blendMode)
...
function BackdropTemplateM33AurasMixin:HasBackdropInfo(backdropInfo)
	return self.backdropInfo == backdropInfo;
end

function BackdropTemplateM33AurasMixin:ClearBackdrop()
	if self.backdropInfo then
		for pieceName in pairs(textureUVs) do
			local region = self[pieceName];
			if region then
...
function BackdropTemplateM33AurasMixin:ApplyBackdrop()
	...
	local layout = {
		...
		setupPieceVisualsFunction = BackdropTemplateM33AurasMixin.SetupPieceVisuals,
	};
...
function BackdropTemplateM33AurasMixin:SetBackdrop(backdropInfo)
...
function BackdropTemplateM33AurasMixin:GetBackdrop()
	if self.backdropInfo then
...
function BackdropTemplateM33AurasMixin:GetBackdropColor()
	if not self.backdropInfo then
		return;
	end
	if self.Center then
...
function BackdropTemplateM33AurasMixin:SetBackdropColor(r, g, b, a)
	if not self.backdropInfo then
...
function BackdropTemplateM33AurasMixin:GetBackdropBorderColor()
	if not self.backdropInfo then
		return
	end
...
function BackdropTemplateM33AurasMixin:SetBackdropBorderColor(r, g, b, a)
	if not self.backdropInfo then
		-- Ideally this would throw an error here but the old API just failed silently
		return;
	end
