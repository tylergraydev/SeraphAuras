...
M33Auras.WA_Utf8Sub = WA_Utf8Sub
...
local WA_ClassColorName = function(unit, maxlen)
  if unit and UnitExists(unit) then
    local name = M33Auras.UnitName(unit)
...
M33Auras.WA_ClassColorName = WA_ClassColorName

M33Auras.PadString = function(input, padMode, padLength)
  input = tostring(input)
  if type(input) ~= "string" then
    return input
  end
...
M33Auras.ShowOverlayGlow = LCG.ButtonGlow_Start
M33Auras.HideOverlayGlow = LCG.ButtonGlow_Stop

local LGF = LibStub("LibGetFrame-1.0")
M33Auras.GetUnitFrame = LGF.GetUnitFrame
M33Auras.GetUnitNameplate =  function(unit)
...
local blockedTables = {
  ...
  ChatFrame1 = true,
  M33AurasSaved = true,
  M33AurasOptions = true,
  M33AurasOptionsSaved = true,
  ItemRackUser = true,
  ItemRackEvents = true
}
...
-- Stack of of aura environments/uids, allows use of recursive aura activations through calls to M33Auras.ScanEvents().
...
function Private.ActivateAuraEnvironment(id, cloneId, state, states, onlyConfig)
  local data = id and M33Auras.GetData(id)
  if not data then
    ...
    if environment_initialized[id] == 2 or (onlyConfig and environment_initialized[id] == 1) then
      local region = M33Auras.GetRegion(id, cloneId)
    ...
    elseif onlyConfig then
      ...
      -- Either this aura environment has not yet been initialized, or it was reset via an edit in M33AurasOptions
...
local FakeM33AurasMixin = {
  blockedFunctions = {
    ...
    ShowOptions = true,
    -- Note these shouldn't exist in the M33Auras namespace, but moving them takes a bit of effort,
  ...
  blockedTables = {
    ...
    -- checked for M33Auras.loaded in there
    loaded = true
  },
  override = {
    ...
    GetData = function(id)
      ...
      if getDataCallCounts[currentId] > 99 then
        Private.AuraWarnings.UpdateWarning(current_uid, "FakeM33AurasGetData", "warning",
    ...
    clones = MakeDeprecated(Private.clones, "clones",
                L["Using M33Auras.clones is deprecated. Use M33Auras.GetRegion(id, cloneId) instead."]),
    regions = MakeDeprecated(Private.regions, "regions",
                L["Using M33Auras.regions is deprecated. Use M33Auras.GetRegion(id) instead."]),
  ...
  blocked = blocked,
  setBlocked = function()
    Private.AuraWarnings.UpdateWarning(current_uid, "FakeM33AurasSet", "error",
                  L["Writing to the M33Auras table is not allowed."], true)
  end
}

local FakeM33Auras = MakeReadOnly(M33Auras, FakeM33AurasMixin)
...
local overridden = {
  ...
  ActionButton_ShowOverlayGlow = M33Auras.ShowOverlayGlow,
  ActionButton_HideOverlayGlow = M33Auras.HideOverlayGlow,
  M33Auras = FakeM33Auras,
  WeakAuras = FakeM33Auras,
}
...
local exec_env_custom = setmetatable(CopyTable(mixins),
{
  ...
  __newindex = function(table, key, value)
    if key == "aura_env" then
      ...
      -- please try harder to not hallucinate about M33Auras, thanks
...
function M33Auras.LoadFunction(string, id)
