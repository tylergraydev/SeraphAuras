...
function TimeMachine:StartTransaction()
  if self.transaction then
    M33Auras.prettyPrint("If you're reading this, a time machine transaction was started, but there was already one in  progress. That's not supposed to happen. Please report this to the M33Auras developers, thanks!")
    self:Reject()
  end
...
function TimeMachine:Commit(instant)
  if not self.transaction and not instant then
    M33Auras.prettyPrint("If you're reading this, a time machine transaction was committed, but there was no transaction in progress. That's not supposed to happen. Please report this to the M33Auras developers, thanks!")
    return
  end
...
function TimeMachine:DestroyTheUniverse(id)
  if self.transaction then
    M33Auras.prettyPrint("If you're reading this, a time machine transaction was destroyed, but there was one in progress. That's not supposed to happen. Please report this to the M33Auras developers, thanks!")
    self:Reject()
  end
