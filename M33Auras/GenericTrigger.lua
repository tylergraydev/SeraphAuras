...
#####################################################
# Helper functions mainly for the M33Auras Options #
#####################################################
...
function M33Auras.UnitExistsFixed(unit, smart)
...
function M33Auras.ScanEventsByID(event, id, ...)
...
function M33Auras.ScanEvents(event, arg1, arg2, ...)
      ...
      for id, triggers in pairs(event_list) do
        ...
        for triggernum, data in pairs(triggers) do
          ...
          then
            ...
            if delay == 0 then
              local allStates = M33Auras.GetTriggerStateForTrigger(id, triggernum);
...
function M33Auras.ScanUnitEvents(event, unit, ...)
...
function M33Auras.ScanEventsInternal(event_list, event, arg1, arg2, ... )
...
function Private.ScanEventsWatchedTrigger(id, watchedTriggernums)
  ...
  for _, watchedTrigger in ipairs(watchedTriggernums) do
    if watched_trigger_events[id] and watched_trigger_events[id][watchedTrigger] then
      local updatedTriggerStates = M33Auras.GetTriggerStateForTrigger(id, watchedTrigger)
      for observerTrigger in pairs(watched_trigger_events[id][watchedTrigger]) do
        ...
        local allstates = M33Auras.GetTriggerStateForTrigger(id, observerTrigger)
...
---@class M33AurasGenericTriggerFrame: FrameScriptObject
local frame = CreateFrame("Frame");
frame.unitFrames = {};
Private.frames["M33Auras Generic Trigger Frame"] = frame;
      ...
      if(Private.category_event_prototype[triggerType] or triggerType == "custom") then
        ...
        if(Private.category_event_prototype[triggerType]) then
          if not(trigger.event) then
            error("Improper arguments to M33Auras.Add - trigger type is \"event\" but event is not defined");
          elseif not(event_prototypes[trigger.event]) then
            if(event_prototypes["Conditions"]) then
              trigger.event = "Conditions";
            else
              error("Improper arguments to M33Auras.Add - no event prototype can be found for event type \""..trigger.event.."\" and default prototype reset failed.");
            end
          else
            ...
            elseif prototype.timedrequired then
              ...
              M33Auras.prettyPrint("Invalid Prototype found: " .. prototype.name)
            end
          ...
          triggerFunc = M33Auras.LoadFunction("return "..(trigger.custom or ""), data.id);
          if (trigger.custom_type == "stateupdate") then
            tsuConditionVariables = M33Auras.LoadFunction("return function() return \n" .. (trigger.customVariables or "") .. "\n end", data.id);
          ...
          if(trigger.custom_type == "status" or trigger.custom_type == "event" and trigger.custom_hide == "custom") then
            untriggerFunc = M33Auras.LoadFunction("return "..(untrigger.custom or ""), data.id);
          ...
          if(trigger.custom_type ~= "stateupdate" and trigger.customDuration and trigger.customDuration ~= "") then
            durationFunc = M33Auras.LoadFunction("return "..trigger.customDuration, data.id);
          end
          if(trigger.custom_type ~= "stateupdate") then
            overlayFuncs = {};
            for i = 1, 7 do
          ...
          if((trigger.custom_type == "status" or trigger.custom_type == "stateupdate") and trigger.check == "update") then
            ...
            local rawEvents = M33Auras.split(trigger.events);
...
do
  ...
  local function CheckGCD()
    ...
    if(event and not M33Auras.IsPaused()) then
  ...
  local function HandleSpell(self, id, startTime, duration, modRate, paused)
    ...
    if C_Secrets.ShouldSpellCooldownBeSecret(id) then
      local isReady = M33Auras.IsSpellReady(id)
    ...
    if duration > 0 then
      if (startTime == gcdStart and duration == gcdDuration)
          or (M33Auras.IsClassicOrWrath() and duration == shootDuration and startTime == shootStart)
      then
  ...
  local SpellDetails = {
    ...
    AddEffectiveSpellId = function(self, effectiveSpellId, userSpellId)
      ...
      spellDetail.known = M33Auras.IsSpellKnownIncludingPet(effectiveSpellId)
      ...
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
            ...
            = M33Auras.GetSpellCooldownUnified(effectiveSpellId, GetRuneDuration());
    ...
    CheckSpellKnown = function(self)
      ...
      for effectiveSpellId, spellDetailsData in pairs(self.data) do
        local known = M33Auras.IsSpellKnownIncludingPet(effectiveSpellId)
    ...
    CheckSpellCooldown = function(self, effectiveSpellId, runeDuration)
      local charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune,
        startTimeCooldown, durationCooldown, cooldownBecauseRune, startTimeCharges, durationCharges,
        spellCount, unifiedModRate, modRate, modRateCharges, paused
        = M33Auras.GetSpellCooldownUnified(effectiveSpellId, runeDuration);
    ...
    WatchSpellCooldown = function(self, userSpellId, ignoreRunes, useExact, followoverride)
      ...
      if ignoreRunes and M33Auras.IsWrathOrCataOrMistsOrRetail() then
        for i = 1, 6 do
  ...
  function M33Auras.GetRuneCooldown(id)
  ...
  local essenceCache = {{},{},{},{},{},{}}
  ---@private
  function M33Auras.InitEssenceCooldown()
  ...
  function M33Auras.GetEssenceCooldown(essence)
  ...
  function M33Auras.GetSpellLossOfControlCooldown(identifier)
    if M33Auras.IsTWW() then
  ...
  function M33Auras.GetSpellCooldown(id, ignoreRuneCD, showgcd, ignoreSpellKnown, track)
  ...
  function M33Auras.GetSpellCharges(id, ignoreSpellKnown)
  ...
  ---@return number gcdCooldown
  function M33Auras.GetItemCooldown(id, showgcd)
  ...
  ---@return number duration
  function M33Auras.gcdDuration()
    return gcdDuration or 0;
  end

  ---@return string name
  function M33Auras.GcdSpellName()
    return gcdSpellName;
  end
  ...
  ---@return boolean enabled
  ---@return number gcdCooldown
  function M33Auras.GetItemSlotCooldown(id, showgcd)
    local startTime, duration, enabled, gcdCooldown;
  ...
  function Private.CheckRuneCooldown()
    ...
    for id, _ in pairs(runes) do
      ...
      if(duration > 0 and duration ~= M33Auras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
  ...
  function M33Auras.GetSpellCooldownUnified(id, runeDuration)
  ...
  function M33Auras.IsSpellReady(id)
  ...
  function Private.CheckItemCooldowns()
    for id, _ in pairs(items) do
      ...
      if(duration > 0 and duration > 1.5 and duration ~= M33Auras.gcdDuration()) then
        -- On non-GCD cooldown
        local endTime = startTime + duration;
      ...
      if (itemCdEnabledChanged and not M33Auras.IsPaused()) then
        Private.ScanEventsByID("ITEM_COOLDOWN_CHANGED", id);
      end
    end
  end
  ...
  ---@private
  function M33Auras.WatchRuneCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
  ...
  ---@private
  function M33Auras.WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
    SpellDetails:WatchSpellCooldown(id, ignoreRunes, useExact, followoverride)
  end

  ---@private
  function M33Auras.WatchItemCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
    ...
    if not(items[id]) then
      ...
      itemCdEnabled[id] = enabled;
      if(duration and duration > 0 and duration > 1.5 and duration ~= M33Auras.gcdDuration()) then
        local time = GetTime();
        local endTime = startTime + duration;
        itemCdDurs[id] = duration;
        itemCdExps[id] = endTime;
  ...
  ---@private
  function M33Auras.WatchItemSlotCooldown(id)
    if not(cdReadyFrame) then
      Private.InitCooldownReady();
    end
...
do
  ...
  ---@private
  function M33Auras.WatchSpellActivation(id)
    if (not id) then
      return;
    end
  ...
  function M33Auras.SpellActivationActive(id)
...
function M33Auras.GetPlayerReaction(unit)
...
---@param unit UnitToken
function M33Auras.WatchUnitChange(unit)
  unit = string.lower(unit)
  if not watchUnitChange then
    ...
    local function unitUpdate(unitA, eventsToSend)
      ...
      local newGUID = M33Auras.UnitExistsFixed(unitA) and UnitGUID(unitA)
    ...
    local function reactionUpdate(unit, eventsToSend)
      ...
      local newReaction = M33Auras.GetPlayerReaction(unit)
    ...
    local function reactionInit(unit)
      watchUnitChange.nameplateFaction[unit] = M33Auras.GetPlayerReaction(unit)
    end
    ...
    if M33Auras.IsClassicOrWrath() then
      function roleUpdate(unit, eventsToSend)
        ...
        local newRaidRole = M33Auras.UnitRaidRole(unit)
    ...
    local handleEvent = {
      ...
      UNIT_PET = function(unit, eventsToSend)
        local pet = M33Auras.unitToPetUnit[unit]
  ...
  watchUnitChange.unitIdToGUID[unit] = M33Auras.UnitExistsFixed(unit) and UnitGUID(unit)
...
function M33Auras.GetEquipmentSetInfo(itemSetName, partial)
...
function Private.WatchStagger()
  if not staggerWatchFrame then
    ...
    Private.frames["M33Auras Stagger Frame"] = staggerWatchFrame
...
if M33Auras.IsClassicOrWrath() then
  ...
  function M33Auras.WatchForQueuedSpell()
  ...
  function M33Auras.GetQueuedSpell()
...
---@return number? cost
function M33Auras.GetSpellCost(powerTypeToCheck)
  local spellID = select(9, M33Auras.UnitCastingInfo("player"))
  if M33Auras.IsClassicOrWrath() and not spellID then
    spellID = M33Auras.GetQueuedSpell()
  end
  if spellID then
...
-- Weapon Enchants
do
  local isCata = M33Auras.IsCataClassic()
  ...
  function M33Auras.TenchInit()
    if not(tenchFrame) then
      ...
      if M33Auras.IsRetail() then
        ...
        getTenchName = function(id)
          tenchTip = M33Auras.GetHiddenTooltip();
...
do
  ...
  ---@private
  function M33Auras.WatchForPetDeath()
...
do
  ...
  ---@private
  function M33Auras.WatchForCastLatency()
    if not castLatencyFrame then
      ...
      -- triggered from cacheEmpoweredFrame after updating cache use by M33Auras.UnitChannelInfo
  ...
  function M33Auras.GetCastLatency()
...
do
  ...
  ---@private
  function M33Auras.WatchForNameplateTargetChange()
...
do
  ...
  ---@private
  function M33Auras.WatchForPlayerMoving()
...
if C_AssistedCombat and C_AssistedCombat.GetNextCastSpell then
  ...
  ---@private
  function M33Auras.WatchForAssistedCombatNextCast()
...
---@private
function M33Auras.RegisterItemCountWatch()
...
function M33Auras.GetUniqueCloneId()
...
local commonConditions = {
  ...
  itemInRange = {
    display = M33Auras.newFeatureString .. L["Item in Range"],
    hidden = true,
    type = "bool",
    test = function(state, needle)
        ...
        if (enable) then
          ...
          if (result[v.name].type == "select" or result[v.name].type == "unit") then
            if (v.conditionValues) then
              result[v.name].values = Private[v.conditionValues] or M33Auras[v.conditionValues];
            else
              if type(v.values) == "function" then
                result[v.name].values = v.values()
              else
                result[v.name].values = Private[v.values] or M33Auras[v.values];
              end
            end
          end
...
do
  ...
  if class == "DEMONHUNTER"
    ...
    or class == "EVOKER"
  then
    function M33Auras.CalculatedGcdDuration()
...
M33Auras.CheckForItemBonusId = function(ids)
...
M33Auras.GetBonusIdInfo = function(ids, specificSlot)
...
M33Auras.CheckForItemEquipped = function(itemId, specificSlot)
...
M33Auras.GetCritChance = function()
...
M33Auras.GetHitChance = function()
...
Private.ExecEnv.GetCurrencyAccountInfo = function(currencyId)
  ...
  if M33Auras.IsRetail() and currencyInfo and currencyInfo.isAccountTransferable then
  ...
  -- WORKAROUND https://github.com/m33shoq/M33Auras/issues/5106
  if currencyInfo and M33Auras.IsCataClassic() then
...
-- So for now make it simply a member of M33Auras
M33Auras.genericTriggerTypes = types

M33Auras.RegisterTriggerSystem(types, GenericTrigger);
