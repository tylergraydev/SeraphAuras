...
local newAPI = M33Auras.IsRetail()
...
local function UnitInSubgroupOrPlayer(unit, includePets)
  ...
  elseif includePets == "PlayersAndPets" then
    return UnitInSubgroup(M33Auras.petUnitToUnit[unit] or unit) or UnitIsUnit("player", unit) or UnitIsUnit("pet", unit)
  elseif includePets == "PetsOnly" then
    return UnitInSubgroup(M33Auras.petUnitToUnit[unit]) or UnitIsUnit("pet", unit)
  end
end
...
local function UpdateToolTipDataInMatchData(matchData, time)
  ...
  if matchData.unit and matchData.auraInstanceID then
    local dataInstanceID, tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = M33Auras.GetAuraInstanceTooltipInfo(matchData.unit, matchData.auraInstanceID, matchData.filter)
  ...
  elseif matchData.unit and matchData.index and matchData.filter then
    local tooltip, _, tooltip1, tooltip2, tooltip3, tooltip4 = M33Auras.GetAuraTooltipInfo(matchData.unit, matchData.index, matchData.filter)
    changed = matchData.tooltip ~= tooltip or matchData.tooltip1 ~= tooltip1
      or matchData.tooltip2 ~= tooltip2 or matchData.tooltip3 ~= tooltip3 or matchData.tooltip4 ~= tooltip4
    matchData.tooltip, matchData.tooltip1, matchData.tooltip2, matchData.tooltip3, matchData.tooltip4 = tooltip, tooltip1, tooltip2, tooltip3, tooltip4
  end
...
local function UpdateMatchData(time, matchDataChanged, unit, index, auraInstanceID, filter, name, icon, stacks, debuffClass, duration, expirationTime, unitCaster, isStealable, isBossDebuff, isCastByPlayer, spellId, modRate, points)
  ...
  local debuffClassIcon = M33Auras.EJIcons[debuffClass]
...
local function UpdateStateWithMatch(time, bestMatch, triggerStates, cloneId, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local debuffClassIcon = M33Auras.EJIcons[bestMatch.debuffClass]
...
local function UpdateStateWithNoMatch(time, triggerStates, triggerInfo, cloneId, unit, matchCount, unitCount, maxUnitCount, matchCountPerUnit, totalStacks, affected, affectedUnits, unaffected, unaffectedUnits, role, raidMark)
  local fallbackName, fallbackIcon = BuffTrigger.GetNameAndIconSimple(M33Auras.GetData(triggerInfo.id), triggerInfo.triggernum)
  if not triggerStates[cloneId] then
    triggerStates[cloneId] = {
      show = true,
      changed = true,
    ...
    if allUnits then
      ...
      return function()
        if raid then
          if i <= 40 then
            ...
            if includePets == "PlayersAndPets" then
              ret = pets and M33Auras.raidpetUnits[i] or M33Auras.raidUnits[i]
              pets = not pets
              if pets then
                i = i + 1
              end
            elseif includePets == "PetsOnly" then
              ret = M33Auras.raidpetUnits[i]
              i = i + 1
            else -- raid
              ret = M33Auras.raidUnits[i]
              i = i + 1
            end
            return ret
          end
        ...
        if i <= 4 then
          local ret
          if includePets == "PlayersAndPets" then
            ret = pets and M33Auras.partypetUnits[i] or M33Auras.partyUnits[i]
            pets = not pets
            if pets then
              i = i + 1
            end
          elseif includePets == "PetsOnly" then
            ret = M33Auras.partypetUnits[i]
            i = i + 1
          else -- group
            ret = M33Auras.partyUnits[i]
            i = i + 1
          end
          return ret
        end
...
local function TriggerInfoApplies(triggerInfo, unit)
  ...
  if M33Auras.UnitIsPet(unit) then
    controllingUnit = M33Auras.petUnitToUnit[unit]
  end
  ...
  if triggerInfo.raidRole then
    local raidRole = M33Auras.UnitRaidRole(controllingUnit)
  ...
  if triggerInfo.hostility and M33Auras.GetPlayerReaction(unit) ~= triggerInfo.hostility then
    return false
  end
  ...
  if triggerInfo.nameChecker and not triggerInfo.nameChecker:Check(M33Auras.UnitNameWithRealm(unit)) then
    return false
  end
...
Private.frames["M33Auras Buff2 Frame"] = Buff2Frame
...
local function EventHandler(frame, event, arg1, arg2, ...)
  ...
  elseif event == "UNIT_PET" then
    local pet = M33Auras.unitToPetUnit[arg1]
    if pet then
...
if M33Auras.IsCataOrMistsOrRetail() then
  Private.LibSpecWrapper.Register(function(unit)
    ...
    local deactivatedTriggerInfos = {}
    RecheckActiveForUnitType("group", unit, deactivatedTriggerInfos)
    RecheckActiveForUnitType("group", M33Auras.unitToPetUnit[unit], deactivatedTriggerInfos)
    DeactivateScanFuncs(deactivatedTriggerInfos)
...
local function createScanFunc(trigger)
  ...
  if use_ignore_name then
    ...
    for index, spellName in ipairs(trigger.ignoreAuraNames) do
      local spellId = M33Auras.SafeToNumber(spellName)
  ...
  if use_ignore_spellId then
    ...
    for index, spellId in ipairs(trigger.ignoreAuraSpellids) do
      local spell = M33Auras.SafeToNumber(spellId)
      if spell then
  ...
  return {
    untrackableSoftTarget = {
      ...
      check = function(trigger)
        return M33Auras.IsUntrackableSoftTarget(trigger.unit)
      end
    }
  }
end
...
function BuffTrigger.Add(data)
  ...
  for triggernum, triggerData in ipairs(data.triggers) do
    ...
    if trigger.type == "aura2" then
      ...
      local effectiveGroupRole = M33Auras.IsWrathOrCataOrMistsOrRetail() and (groupTrigger and trigger.useGroupRole and trigger.group_role) or nil
      local effectiveRaidRole = M33Auras.IsClassicOrWrathOrCataOrMists() and (groupTrigger and trigger.useRaidRole and trigger.raid_role) or nil
      local effectiveClass = groupTrigger and trigger.useClass and trigger.class
      local effectiveSpecId = M33Auras.IsCataOrMistsOrRetail() and (groupTrigger and trigger.useActualSpec and trigger.actualSpec) or nil
      local effectiveArenaSpec = M33Auras.IsRetail() and (trigger.unit == "arena" and trigger.useArenaSpec and trigger.arena_spec) or nil
      ...
      local effectiveInRange = M33Auras.IsRetail() and groupTrigger and trigger.inRange
      ...
      local triggerInformation = {
        ...
        fetchRole = M33Auras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole,
...
function BuffTrigger.GetNameAndIcon(data, triggernum)
  ...
  if (not name or not icon and M33Auras.spellCache) then
    local trigger = data.triggers[triggernum].trigger
    if trigger.useName and trigger.auranames then
      for index, spellName in ipairs(trigger.auranames) do
        icon = M33Auras.spellCache.GetIcon(spellName)
        if icon then
...
function BuffTrigger.GetAdditionalProperties(data, triggernum)
  ...
  if M33Auras.IsCataOrMistsOrRetail() and trigger.unit ~= "multi" and trigger.fetchRole then
...
function BuffTrigger.GetTriggerConditions(data, triggernum)
  ...
  result["nameCaster"] = {
    ...
    test = function(state, needle, op, preamble)
      return state.unitCaster and preamble:Check(M33Auras.UnitNameWithRealm(state.unitCaster))
    end,
    operator_types = "none",
  }
...
function BuffTrigger.GetTriggerDescription(data, triggernum, namestable)
  ...
  if trigger.useName and trigger.auranames then
    for index, name in pairs(trigger.auranames) do
      ...
      local icon
      local spellId = M33Auras.SafeToNumber(name)
      if spellId then
        icon = Private.ExecEnv.GetSpellIcon(spellId)
      else
        icon = M33Auras.spellCache.GetIcon(name)
      end
...
M33Auras.RegisterTriggerSystem({"aura2"}, BuffTrigger)
