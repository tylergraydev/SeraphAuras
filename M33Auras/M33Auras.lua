...
local ADDON_NAME = "M33Auras"
---@class M33Auras
local M33Auras = M33Auras
local L = M33Auras.L
local versionString = M33Auras.versionString
local prettyPrint = M33Auras.prettyPrint

M33AurasTimers = setmetatable({}, {__tostring=function() return "M33Auras" end})
LibStub("AceTimer-3.0"):Embed(M33AurasTimers)
...
function M33AurasTimers:ScheduleTimerFixed(func, delay, ...)
  if (delay < Private.maxTimerDuration) then
    if delay + GetTime() > maxUpTime then
      M33Auras.prettyPrint(M33Auras.L["Can't schedule timer with %i, due to a World of Warcraft bug with high computer uptime. (Uptime: %i). Please restart your computer."]:format(delay, GetTime()))
      return
    end
...
if CustomNames then
  M33Auras.GetName = CustomNames.Get
  M33Auras.UnitName = CustomNames.UnitName
  M33Auras.GetUnitName = CustomNames.GetUnitName
  M33Auras.UnitFullName = CustomNames.UnitFullName
else
  M33Auras.GetName = function(name) return name end
  M33Auras.UnitName = UnitName
  M33Auras.GetUnitName = GetUnitName
  M33Auras.UnitFullName = UnitFullName
end

local timer = M33AurasTimers
M33Auras.timer = timer

local loginQueue = {}
local queueshowooc

function M33Auras.InternalVersion()
...
do
  ...
  local function waErrorHandler(errorMessage)
    ...
    if currentErrorHandlerId then
      data = M33Auras.GetData(currentErrorHandlerId)
    ...
    table.insert(juicedMessage, L["M33Auras Version: %s"]:format(M33Auras.versionString))
...
function Private.LoadOptions(msg)
  if not(C_AddOns.IsAddOnLoaded("M33AurasOptions")) then
    if not M33Auras.IsLoginFinished() then
      prettyPrint(Private.LoginMessage())
      loginQueue[#loginQueue + 1] = M33Auras.OpenOptions
    elseif InCombatLockdown() then
      ...
      local loaded, reason = C_AddOns.LoadAddOn("M33AurasOptions");
      if not(loaded) then
        reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
        M33Auras.prettyPrint(string.format(L["Options could not be loaded, the addon is %s"], reason));
        return false;
      end
    end
  end
...
function M33Auras.OpenOptions(msg)
  if Private.NeedToRepairDatabase() then
    StaticPopup_Show("M33Auras_CONFIRM_REPAIR", nil, nil, {reason = "downgrade"})
  elseif (M33Auras.IsLoginFinished() and Private.LoadOptions(msg)) then
    M33Auras.ToggleOptions(msg, Private);
  end
end
...
function Private.PrintHelp()
  ...
  print(L["If you require additional assistance, please open a ticket on GitHub or visit our Discord at https://discord.gg/M33Auras!"])
end

SLASH_M33Auras1, SLASH_M33Auras2 = "/wa", "/m33auras";
function SlashCmdList.M33Auras(input)
  local args, msg = {}, nil
  ...
  if msg == "pstart" then
    M33Auras.StartProfile(args[1]);
  elseif msg == "pstop" then
    M33Auras.StopProfile();
  elseif msg == "pprint" then
    M33Auras.PrintProfile();
  elseif msg == "pcancel" then
    M33Auras.CancelScheduledProfile()
  ...
  elseif msg == "repair" then
    StaticPopup_Show("M33Auras_CONFIRM_REPAIR", nil, nil, {reason = "user"})
  elseif msg == "ff" or msg == "feat" or msg == "feature" then
    ...
    M33Auras.OpenOptions(msg);
  end
end

if not M33Auras.IsLibsOK() then return end

function M33Auras.ToggleMinimap()
  M33AurasSaved.minimap.hide = not M33AurasSaved.minimap.hide
  if M33AurasSaved.minimap.hide then
    LDBIcon:Hide("M33Auras");
    prettyPrint(L["Use /wa minimap to show the minimap icon again."])
  else
    LDBIcon:Show("M33Auras");
  end
end

BINDING_HEADER_M33Auras = ADDON_NAME
BINDING_NAME_M33AurasTOGGLE = L["Toggle Options Window"]
BINDING_NAME_M33AurasPROFILINGTOGGLE = L["Toggle Performance Profiling Window"]
BINDING_NAME_M33AurasPRINTPROFILING = L["Print Profiling Results"]

-- An alias for M33AurasSaved, the SavedVariables
-- Noteable properties:
...
-- While true no events are handled. E.g. M33Auras is paused while the Options dialog is open
local paused = true;
...
M33Auras.raidUnits = {};
M33Auras.raidpetUnits = {};
M33Auras.partyUnits = {};
M33Auras.partypetUnits = {};
M33Auras.petUnitToUnit = {
  pet = "player"
}
M33Auras.unitToPetUnit = {
  player = "pet"
}
do
  for i=1,40 do
    M33Auras.raidUnits[i] = "raid"..i
    M33Auras.raidpetUnits[i] = "raidpet"..i
    M33Auras.petUnitToUnit["raidpet"..i] = "raid"..i
    M33Auras.unitToPetUnit["raid"..i] = "raidpet"..i
  end
  for i=1,4 do
    M33Auras.partyUnits[i] = "party"..i
    M33Auras.partypetUnits[i] = "partypet"..i
    M33Auras.petUnitToUnit["partypet"..i] = "party"..i
    M33Auras.unitToPetUnit["party"..i] = "partypet"..i
  end
end

---@param unit UnitToken
---@return boolean isPet
M33Auras.UnitIsPet = function(unit)
  return M33Auras.petUnitToUnit[unit] ~= nil
end
...
function M33Auras.RegisterSubRegionType(name, displayName, supportFunction, createFunction, modifyFunction, onAcquire, onRelease, default, addDefaultsForNewAura, properties, supportsAdd)
  if not(name) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - name is not a string", 2);
  elseif not(displayName) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - display name is not defined".." "..name, 2);
  elseif(type(displayName) ~= "string") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - display name is not a string", 2);
  elseif not(supportFunction) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - support function is not defined", 2);
  elseif(type(supportFunction) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - support function is not a function", 2);
  elseif not(createFunction) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - creation function is not a function", 2);
  elseif not(modifyFunction) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - modification function is not defined", 2);
  elseif(type(modifyFunction) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - modification function is not a function", 2)
  elseif not(onAcquire) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - onAcquire function is not defined", 2);
  elseif(type(onAcquire) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - onAcquire function is not a function", 2)
  elseif not(onRelease) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - onRelease function is not defined", 2);
  elseif(type(onRelease) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - onRelease function is not a function", 2)
  elseif not(default) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - default options are not defined", 2);
  elseif(type(default) ~= "table" and type(default) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - default options are not a table or a function", 2);
  elseif(addDefaultsForNewAura and type(addDefaultsForNewAura) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionType - addDefaultsForNewAura function is not nil or a function", 2)
  elseif(subRegionTypes[name]) then
    error("Improper arguments to M33Auras.RegisterSubRegionType - region type \""..name.."\" already defined", 2);
  else
...
function M33Auras.RegisterSubRegionOptions(name, createFunction, description, getAnchors)
  if not(name) then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - name is not defined", 2);
  elseif(type(name) ~= "string") then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - name is not a string", 2);
  elseif not(createFunction) then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - creation function is not defined", 2);
  elseif(type(createFunction) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - creation function is not a function", 2);
  elseif(getAnchors and type(getAnchors) ~= "function") then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - getAnchors function is not a function", 2);
  elseif(subRegionOptions[name]) then
    error("Improper arguments to M33Auras.RegisterSubRegionOptions - region type \""..name.."\" already defined", 2);
  else
...
---@diagnostic disable-next-line: duplicate-set-field (it's replaced in M33AurasOptions.lua)
function M33Auras.IsOptionsOpen()
  return false;
end
...
function M33Auras.GetActiveConditions(id, cloneId)
  ...
  if (data.actions) then
    if data.actions.init then
      if data.actions.init.do_custom and data.actions.init.custom then
        local func = M33Auras.LoadFunction("return function() "..(data.actions.init.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["init"] = func
      end
      if data.actions.init.do_custom_load and data.actions.init.customOnLoad then
        local func = M33Auras.LoadFunction("return function() "..(data.actions.init.customOnLoad).."\n end", data.id);
      ...
      if data.actions.init.do_custom_unload and data.actions.init.customOnUnload then
        local func = M33Auras.LoadFunction("return function() "..(data.actions.init.customOnUnload).."\n end", data.id);
    ...
    if (data.actions.start) then
      if (data.actions.start.do_custom and data.actions.start.custom) then
        local func = M33Auras.LoadFunction("return function() "..(data.actions.start.custom).."\n end", data.id);
        Private.customActionsFunctions[id]["start"] = func;
      end

      if (data.actions.start.do_message and data.actions.start.message_custom) then
        local func = M33Auras.LoadFunction("return "..(data.actions.start.message_custom), data.id);
    ...
    if (data.actions.finish) then
      if (data.actions.finish.do_custom and data.actions.finish.custom) then
        local func = M33Auras.LoadFunction("return function() "..(data.actions.finish.custom).."\n end", data.id);
      ...
      if (data.actions.finish.do_message and data.actions.finish.message_custom) then
        local func = M33Auras.LoadFunction("return "..(data.actions.finish.message_custom), data.id);
...
local function CreateTalentCache()
  ...
  if M33Auras.IsClassicOrWrathOrCata() then
  ...
  elseif M33Auras.IsMists() then
    ...
    for tier = 1, MAX_TALENT_TIERS do
      for column = 1, NUM_TALENT_COLUMNS do
        ...
        -- Get the icon and name from the talent cache and record it in the table that will be used by M33AurasOptions
        if (talentName and talentIcon) then
...
-- when using this function, do not name your global data table "M33AurasCompanion"
function M33Auras.AddCompanionData(data)
  M33Auras.DeepMixin(Private.CompanionData, data)
end

-- add data from versions of companion compatible addon that does not use M33Auras.AddCompanionData yet
local function AddLegacyCompanionData()
  local CompanionData = M33AurasCompanion and M33AurasCompanion.M33Auras or M33AurasCompanion
  if CompanionData then
    M33Auras.AddCompanionData(CompanionData)
  end
end
...
function Private.PostAddCompanion()
  ...
  if count and count > 0 then
    M33Auras.prettyPrint(L["There are %i updates to your auras ready to be installed!"]:format(count))
  end
  -- nag if new installs
  if Private.CompanionData.stash and next(Private.CompanionData.stash) then
    M33Auras.prettyPrint(L["You have new auras ready to be installed!"])
  end
end
...
local function tooltip_draw(isAddonCompartment, blizzardTooltip)
  ...
  tooltip:ClearLines()
  tooltip:AddDoubleLine("M33Auras", versionString)
...
M33Auras.GenerateTooltip = tooltip_draw;
...
local Broker_M33Auras;
Broker_M33Auras = LDB:NewDataObject("M33Auras", {
  type = "launcher",
  text = "M33Auras",
  ...
  OnEnter = function(self)
    colorFrame:SetScript("OnUpdate", function(self, elaps)
      ...
      Broker_M33Auras.iconR = r + (r2 - r) * colorElapsed / colorDelay;
      Broker_M33Auras.iconG = g + (g2 - g) * colorElapsed / colorDelay;
      Broker_M33Auras.iconB = b + (b2 - b) * colorElapsed / colorDelay;
    end);
    local elapsed = 0;
    local delay = 1;
...
do -- Archive stuff
  ...
  local function OpenArchive()
    if Archivist:IsInitialized() then
      ...
      if not C_AddOns.IsAddOnLoaded("M33AurasArchive") then
        local ok, reason = C_AddOns.LoadAddOn("M33AurasArchive")
        if not ok then
          reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
          error(string.format(L["Could not load M33Auras Archive, the addon is %s"], reason))
        end
      end
      if type(M33AurasArchive) ~= "table" then
        M33AurasArchive = {}
      end
      Archivist:Initialize(M33AurasArchive)
    end
    return Archivist
  end

  function M33Auras.LoadFromArchive(storeType, storeID)
...
function M33Auras.IsLoginFinished()
    ...
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        ...
        if (guid and db.CurrentEncounter.boss_guids [guid]) then
          ...
          M33Auras.CurrentEncounter = db.CurrentEncounter
          return true
        end
      end
    end
...
function Private.Login(takeNewSnapshots)
  local loginFunc = (function() -- coroutine.create
    ...
    coroutine.yield(100)
    if db.history then
      local histRepo = M33Auras.LoadFromArchive("Repository", "history")
      local migrationRepo = M33Auras.LoadFromArchive("Repository", "migration")
      ...
      -- history is now in archive so we can shrink M33AurasSaved
      db.history = nil
    end
    ...
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFM33Auras|r detected a corrupt entry in M33Auras saved displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
...
local M33AurasFrame = CreateFrame("Frame", "M33AurasFrame", UIParent);
Private.frames["M33Auras Main Frame"] = M33AurasFrame;
M33AurasFrame:SetAllPoints(UIParent);
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      -- if not db.disableRuLocale and M33Auras.LoadLocale_ruRU then
      --   M33Auras.LoadLocale_ruRU()
      -- end
      ...
      LDBIcon:Register("M33Auras", Broker_M33Auras, db.minimap);
    end
...
function M33Auras.IsImporting()
  return importing;
end

function M33Auras.IsPaused()
  return paused;
end
...
function M33Auras.InLoadingScreen()
...
function Private.ResumeAllDynamicGroups(suspended)
  for _, id in ipairs(suspended) do
    local region = M33Auras.GetRegion(id)
...
local function StoreBossGUIDs()
  ...
  if (M33Auras.CurrentEncounter and M33Auras.CurrentEncounter.boss_guids) then
    for i = 1, 10 do
      if (UnitExists ("boss" .. i)) then
        local guid = UnitGUID ("boss" .. i)
        if (guid) then
          M33Auras.CurrentEncounter.boss_guids [guid] = true
        end
      end
    end
    db.CurrentEncounter = M33Auras.CurrentEncounter
  end
  Private.StopProfileSystem("boss_guids")
end

local function DestroyEncounterTable()
  if (M33Auras.CurrentEncounter) then
    wipe(M33Auras.CurrentEncounter)
  end
  M33Auras.CurrentEncounter = nil
  db.CurrentEncounter = nil
end
...
local function CreateEncounterTable(encounter_id)
  ...
  return M33Auras.CurrentEncounter
end
...
function M33Auras.InstanceType()
  return (GetInstanceTypeAndSize())
end

---@return string difficulty
function M33Auras.InstanceDifficulty()
  return select(2, GetInstanceTypeAndSize())
end

---@return number? difficultyID
function M33Auras.InstanceTypeRaw()
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local encounter_id = M33Auras.CurrentEncounter and M33Auras.CurrentEncounter.id or 0
  ...
  if M33Auras.IsClassicOrWrathOrCataOrMists() then
    local raidID = UnitInRaid("player")
    if raidID then
      raidRole = select(10, GetRaidRosterInfo(raidID))
    end
    role = "none"
    if M33Auras.IsWrathClassic() then
  ...
  if M33Auras.IsClassicEra() then
    vehicle = UnitOnTaxi('player')
  end
  if M33Auras.IsWrathOrCataOrMistsOrRetail() then
  ...
  if (M33Auras.CurrentEncounter) then
    if (instanceId ~= M33Auras.CurrentEncounter.zone_id and not inCombat) then
      encounter_id = 0
      DestroyEncounterTable()
    end
  end
  ...
  local addonRestrictionsActive = M33Auras.IsSecretStateActive()
  ...
  for id in pairs(toCheck) do
    local data = M33Auras.GetData(id)
    if (data and not data.controlledChildren) then
      ...
      elseif M33Auras.IsWrathClassic() then
      ...
      elseif M33Auras.IsCataClassic() then
...
if M33Auras.IsWrathOrCataOrMists() then
  loadFrame:RegisterEvent("VEHICLE_UPDATE");
  loadFrame:RegisterEvent("UPDATE_VEHICLE_ACTIONBAR")
  loadFrame:RegisterEvent("UPDATE_OVERRIDE_ACTIONBAR");
end

if M33Auras.IsMists() then
  loadFrame:RegisterEvent("PET_BATTLE_OPENING_START");
  loadFrame:RegisterEvent("PET_BATTLE_CLOSE");
end
...
function Private.LoadDisplays(toLoad, ...)
  for id in pairs(toLoad) do
    local uid = M33Auras.GetData(id).uid
    ...
    if Private.DebugLog.IsEnabled(uid) then
      M33Auras.prettyPrint(L["Debug Logging enabled for '%s'"]:format(id))
...
function Private.GetDataByUID(uid)
  return M33Auras.GetData(UIDtoID[uid])
end
...
function M33Auras.Rename(data, newid)
  ...
  if(data.parent) then
    ...
    local parentRegion = M33Auras.GetRegion(data.parent)
...
function M33Auras.DeepMixin(dest, source)
...
function Private.NeedToRepairDatabase()
  return db.dbVersion and db.dbVersion > M33Auras.InternalVersion()
end
...
local function RepairDatabase()
  local func = (function()
    ...
    db.dbVersion = M33Auras.InternalVersion()
...
StaticPopupDialogs["M33Auras_CONFIRM_REPAIR"] = {
  text = "",
  button1 = L["Repair"],
  button2 = L["Cancel"],
  ...
  for _, data in pairs(db.displays) do
    if type(data.uid) == "string" then
      if seenUIDs[data.uid] then
        ...
        data.uid = M33Auras.GenerateUniqueID()
...
function Private.CheckForAnchorCycle(source)
  ...
  while source do
    cycle[source] = true
    local data = M33Auras.GetData(source)
    local target
    if data then
      if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame then
        ...
        elseif data.anchorFrameFrame:sub(1, 9) == "M33Auras:" then
...
function Private.AddMany(tbl, takeSnapshots)
  ...
  for _, source in pairs(anchorTargets) do
    ...
    if Private.CheckForAnchorCycle(source) then
      M33Auras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(source))
  ...
  if takeSnapshots then
    ...
    if #order > 0 then
      Private:Async({name = "snapshot"}, function()
        prettyPrint(L["M33Auras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."])
      ...
      if next(M33Auras.LoadFromArchive("Repository", "migration").stores) ~= nil then
        C_Timer.After(1, function()
          prettyPrint(L["M33Auras has detected empty settings. If this is unexpected, ask for assitance on https://discord.gg/M33Auras."])
        end)
      end
    end
  end
  ...
  for _, data in ipairs(order) do
    if data.parent and bads[data.parent] then
      ...
      local ok = xpcall(M33Auras.PreAdd, Private.GetErrorHandlerUid(data.uid, "PreAdd"), data, oldSnapshot)
      if not ok then
    ...
    if not customOptionIsValid(option) or authorOptionKeys[option.key] then
      ...
      if optionClass == "simple" then
        if not option.key then
          option.key = M33Auras.GenerateUniqueID()
        end
...
function Private.ClearSounds(uid, severity)
  ...
  M33Auras.ClearAndUpdateOptions(data.id, true)
  M33Auras.FillOptions()
end

function M33Auras.PreAdd(data, snapshot)
    ...
    for _, subRegionData in ipairs(data.subRegions) do
      ...
      if subType and Private.subRegionTypes[subType] then
        if Private.subRegionTypes[subType].supports(data.regionType) then
          ...
          M33Auras.prettyPrint(L["ERROR in '%s' unknown or incompatible sub element type '%s'"]:format(data.id, subType))
        end
      end
    end
  end
...
local function cycleCheck(data)
  local id = data.id
  if data.anchorFrameType == "SELECTFRAME" and data.anchorFrameFrame and (data.anchorFrameFrame:sub(1, 10) == "WeakAuras:" or data.anchorFrameFrame:sub(1, 9) == "M33Auras:") then
    if Private.CheckForAnchorCycle(id) then
      M33Auras.prettyPrint(L["Warning: Anchoring in aura '%s' is imposssible, due to an anchoring cycle"]:format(id))
...
function pAdd(data, simpleChange)
  local id = data.id;
  if not(id) then
    error("Improper arguments to M33Auras.Add - id not defined");
    return;
  end

  data.uid = data.uid or M33Auras.GenerateUniqueID()
  if db.displays[id] and db.displays[id].uid ~= data.uid then
    print("Improper? arguments to M33Auras.Add - id", id, "is assigned to a different uid.", data.uid, db.displays[id].uid)
  end
  if UIDtoID[data.uid] and UIDtoID[data.uid] ~= id then
    print("Improper? arguments to M33Auras.Add - uid is assigned to a id. Uid:", data.uid, "assigned too:", UIDtoID[data.uid], "assigning now to", data.id)
  end
  ...
  if simpleChange then
    db.displays[id] = data
    cycleCheck(data)
    if M33Auras.GetRegion(data.id) then
    ...
    if Private.IsGroupType(data) then
      ...
      local visible
      if (M33Auras.IsOptionsOpen()) then
      ...
      if data.triggers.disjunctive == "custom" then
        triggerLogicFunc = M33Auras.LoadFunction("return "..(data.triggers.customTriggerLogic or ""), data.id);
      end
...
function M33Auras.Add(data, simpleChange)
  Private.TimeMachine:DestroyTheUniverse(data.id)
...
function Private.AddParents(data)
  ...
  if (parent) then
    local parentData = M33Auras.GetData(parent)
...
function Private.SetRegion(data, cloneId)
  ...
  if not(regionType) then
    ...
    if(not regionTypes[regionType]) then
      regionType = "fallback";
      print("Improper arguments to M33Auras.CreateRegion - regionType \""..data.regionType.."\" is not supported in ".. data.id)
    end
    ...
    if not(id) then
      ...
      if(cloneId) then
        ...
        if (not region or region.regionType ~= data.regionType) then
          ...
          if(clonePool[data.regionType] and clonePool[data.regionType][1]) then
            ...
            local clone = regionTypes[data.regionType].create(M33AurasFrame, data);
        ...
        if((not Private.regions[id]) or (not Private.regions[id].region) or Private.regions[id].regionType ~= regionType) then
          region = regionTypes[regionType].create(M33AurasFrame, data);
          Private.regions[id] = {
      ...
      local loginFinished = M33Auras.IsLoginFinished();
...
local function EnsureRegion(id)
  if not Private.regions[id] or not Private.regions[id].region then
    ...
    while(id) do
      ...
      if M33Auras.GetRegion(id) then
        break
      end
    end

    for _, toCreateId in ipairs_reverse(aurasToCreate) do
      local data = M33Auras.GetData(toCreateId)
      Private.SetRegion(data)
    end
  end
...
---@return table|nil
function M33Auras.GetRegion(id, cloneId)
  if(cloneId and cloneId ~= "") then
...
function Private.SetAllStatesHidden(id, triggernum)
  local triggerState = M33Auras.GetTriggerStateForTrigger(id, triggernum);
  local changed = false
...
function Private.ReleaseClone(id, cloneId, regionType)
  ...
  if region:IsProtected() then
    M33Auras.prettyPrint(L["Error '%s' created a secure clone. We advise deleting the aura. For more information:\nhttps://github.com/m33shoq/M33Auras/wiki/Protected-Frames"]:format(id))
  else
...
do
  local function frame_monitor_callback(event, frame, unit, previousUnit)
      ...
      for region, data in pairs(glow_frame_monitor) do
        ...
        then
          if not new_frame then
            new_frame = M33Auras.GetUnitFrame(unit)
          end
    ...
    if type(anchor_unitframe_monitor) == "table" then
      for region, data in pairs(anchor_unitframe_monitor) do
        ...
        then
          if not new_frame then
            new_frame = M33Auras.GetUnitFrame(unit) or M33Auras.HiddenFrames
          end
          if new_frame ~= data.frame then
  ...
  LGF.RegisterCallback("M33Auras", "FRAME_UNIT_UPDATE", frame_monitor_callback)
  LGF.RegisterCallback("M33Auras", "FRAME_UNIT_ADDED", frame_monitor_callback)
  LGF.RegisterCallback("M33Auras", "FRAME_UNIT_REMOVED", frame_monitor_callback)
end
  ...
  then
    ...
    if actions.glow_frame_type == "FRAMESELECTOR" then
      if actions.glow_frame:sub(1, 10) == "WeakAuras:" then
        ...
        if M33Auras.GetData(frame_name) then
      ...
      elseif actions.glow_frame:sub(1, 9) == "M33Auras:" then
        local frame_name = actions.glow_frame:sub(11)
        if M33Auras.GetData(frame_name) then
          Private.EnsureRegion(frame_name)
        end
    ...
    elseif actions.glow_frame_type == "UNITFRAME" and region.state.unit then
      glow_frame = M33Auras.GetUnitFrame(region.state.unit)
      should_glow_frame = true
    elseif actions.glow_frame_type == "NAMEPLATE" and region.state.unit then
      glow_frame = M33Auras.GetUnitNameplate(region.state.unit)
      should_glow_frame = true
...
function Private.PerformActions(data, when, region)
  if (paused or M33Auras.IsOptionsOpen()) then
    return;
  end;
  local actions;
  local formatters
...
-- This has to be in M33Auras for now, because GetNameAndIcon can be called from the options
-- before the Options has access to Private
M33Auras.GetNameAndIcon = wrapTriggerSystemFunction("GetNameAndIcon", "nameAndIcon");
...
do
  local hiddenTooltip;
  function M33Auras.GetHiddenTooltip()
    if not(hiddenTooltip) then
      hiddenTooltip = CreateFrame("GameTooltip", "M33AurasTooltip", nil, "GameTooltipTemplate");
...
function M33Auras.GetAuraInstanceTooltipInfo(unit, auraInstanceId, filter)
  if M33Auras.IsRetail() then
    local tooltipText = ""
    local tooltipData
...
function M33Auras.GetAuraTooltipInfo(unit, index, filter)
  local tooltipText = ""
  if M33Auras.IsRetail() then
    ...
    local tooltip = M33Auras.GetHiddenTooltip();
    tooltip:ClearLines();
...
local FrameTimes = {};
function M33Auras.ProfileFrames(all)
  UpdateAddOnCPUUsage();
...
local DisplayTimes = {};
function M33Auras.ProfileDisplays(all)
  UpdateAddOnCPUUsage();
...
local function FixGroupChildrenOrderImpl(data, frameLevel)
  ...
  for _, childId in ipairs(data.controlledChildren) do
    local childData = M33Auras.GetData(childId)
...
function M33Auras.EnsureString(input)
  if (input == nil) then
    return "";
  end
...
function M33Auras.RegisterTriggerSystem(types, triggerSystem)
...
function M33Auras.RegisterTriggerSystemOptions(types, func)
  for _, v in ipairs(types) do
    Private.triggerTypesOptions[v] = func;
  end
end

function M33Auras.GetTriggerStateForTrigger(id, triggernum)
...
function M33Auras.GetActiveStates(id)
  return triggerState[id].activeStates
end

function M33Auras.GetActiveTriggers(id)
...
do
  ...
  function Private.UpdateFakeStatesFor(id)
    if (M33Auras.IsOptionsOpen() and visibleFakeStates[id]) then
      local data = M33Auras.GetData(id)
      if (data) then
        ...
        Private.UpdatedTriggerState(id)
        if M33Auras.GetMoverSizerId() == id then
          M33Auras.SetMoverSizer(id)
        end
      end
    end
  end
    ...
    if (state.show) then
      ...
      for triggernum = -1, triggerState[id].numTriggers do
        ...
        if triggernum == activeTrigger then
          ...
          local triggerStates = M33Auras.GetTriggerStateForTrigger(id, triggernum)
...
function Private.UpdatedTriggerState(id)
  ...
  local activeTriggerState = M33Auras.GetTriggerStateForTrigger(id, newActiveTrigger);
...
function Private.RunCustomTextFunc(region, customFunc)
  ...
  if state then
    if state.progressType == "durationObject" and M33Auras.IsDurationObject(state.durationObject) then
...
local function ReplaceValuePlaceHolders(textStr, region, customCache, state, formatter, trigger)
  ...
  if customIndexSubStr then
    ...
    if custom then
      value = M33Auras.EnsureString(value)
    end
  else
...
function M33Auras.IsAuraActive(id)
  local active = triggerState[id]
...
local function ensureMouseFrame()
  ...
  ---@class Frame
  mouseFrame = CreateFrame("Frame", "M33AurasAttachToMouseFrame", UIParent);
  mouseFrame.attachedVisibleFrames = {};
  mouseFrame:SetWidth(1);
  mouseFrame:SetHeight(1);

  local moverFrame = CreateFrame("Frame", "M33AurasMousePointerFrame", mouseFrame);
...
function Private.ensurePRDFrame()
  ...
  personalRessourceDisplayFrame = CreateFrame("Frame", "M33AurasAttachToPRD", UIParent);
  ...
  local moverFrame = CreateFrame("Frame", "M33AurasPRDMoverFrame", personalRessourceDisplayFrame);
  ...
  texture:SetAllPoints(moverFrame);
  texture:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\PRDFrame");
  ...
  personalRessourceDisplayFrame.OptionsOpened = function()
    ...
    if (KuiNameplatesCore and KuiNameplatesCore.profile) then
      ...
      personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\PRDFrameKui");
    else
      ...
      -- personalRessourceDisplayFrame.texture:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\PRDFrame");
    end
...
local function tryAnchorAgain()
  ...
  for id, _ in pairs(delayed) do
    ...
    if (data and region) then
      ...
      local parentData
      if data.parent then
        parentData = M33Auras.GetData(data.parent)
...
local HiddenFrames = CreateFrame("Frame", "M33AurasHiddenFrames")
HiddenFrames:Hide()
M33Auras.HiddenFrames = HiddenFrames
...
local function GetAnchorFrame(data, region, parent)
  ...
  if (anchorFrameType == "NAMEPLATE") then
    ...
    if unit then
      local frame = unit and M33Auras.GetUnitNameplate(unit)
  ...
  if (anchorFrameType == "UNITFRAME") then
    local unit = region.state and region.state.unit
    if unit then
      local frame = M33Auras.GetUnitFrame(unit) or M33Auras.HiddenFrames
      if frame then
  ...
  if (anchorFrameType == "SELECTFRAME" and anchorFrameFrame) then
    if(anchorFrameFrame:sub(1, 10) == "WeakAuras:") then
      ...
      postponeAnchor(id);
    elseif (anchorFrameFrame:sub(1, 9) == "M33Auras:")  then
  ...
  if (anchorFrameType == "CUSTOM" and region.customAnchorFunc) then
    ...
    if ok and frame then
      return frame
    elseif M33Auras.IsOptionsOpen() then
      return parent
    else
      return HiddenFrames
    end
...
function Private.AnchorFrame(data, region, parent, force)
  ...
  and not M33Auras.IsLoginFinished()
  and not force
  then
    ...
    if (data.anchorFrameParent or data.anchorFrameParent == nil
      ...
      region:SetParent(parent or M33AurasFrame);
    end
...
function M33Auras.SetModel(frame, unused, model_fileId, isUnit, isDisplayInfo)
...
function M33Auras.SafeToNumber(input)
...
function M33Auras.ReplaceRaidMarkerSymbols(txt)
  local start = 1
...
do
  ...
  function M33Auras.IsUntrackableSoftTarget(unit)
  ...
  ---@param unit UnitToken
  ---@return boolean result
  function M33Auras.UntrackableUnit(unit)
...
do
  ...
  ---@return string name
  ---@return string realm
  function M33Auras.UnitNameWithRealm(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm = UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end

  function M33Auras.UnitNameWithRealmCustomName(unit)
    ownRealm = ownRealm or select(2, UnitFullName("player"))
    local name, realm =  M33Auras.UnitFullName(unit)
    return name or "", realm or ownRealm or ""
  end
end
...
function M33Auras.IsAuraLoaded(id)
...
-- are categorized
---@private
function M33Auras.GetTriggerCategoryFor(triggerType)
...
function Private.SortOrderForValues(values)
  ...
  table.sort(sortOrder, function(aKey, bKey)
    ...
    if type(aValue) == "string" and aValue:sub(1, #M33Auras.newFeatureString) == M33Auras.newFeatureString then
      aValue = aValue:sub(#M33Auras.newFeatureString + 1)
    end

    if type(bValue) == "string" and bValue:sub(1, #M33Auras.newFeatureString) == M33Auras.newFeatureString then
      bValue = bValue:sub(#M33Auras.newFeatureString + 1)
    end
...
do
  ...
  local function Traverse(data, includeSelf, includeGroups, includeLeafs)
    ...
    if data.controlledChildren then
      for _, child in ipairs(data.controlledChildren) do
        Traverse(M33Auras.GetData(child), true, includeGroups, includeLeafs)
      end
    end
  end
  ...
  local function TraverseParents(data)
    while data.parent do
      local parentData = M33Auras.GetData(data.parent)
