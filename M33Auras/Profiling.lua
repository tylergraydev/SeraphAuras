...
M33AurasProfilingReportMixin = {}
function M33AurasProfilingReportMixin:OnShow()
  ...
  self:SetTitle(L["M33Auras Profiling Report"])
  self:SetSize(500, 300)
end

function M33AurasProfilingReportMixin:ClearText()
  self.ScrollBox.messageFrame:SetText("")
end

function M33AurasProfilingReportMixin:AddText(v)
  if not v then
    return
  end
  --- @type string?
...
local RegisterProfile = function(startType)
  ...
  if startType == "encounter" then
    M33AurasProfilingFrame:UnregisterAllEvents()
    prettyPrint(L["Your next encounter will automatically be profiled."])
    M33AurasProfilingFrame:RegisterEvent("ENCOUNTER_START")
    M33AurasProfilingFrame:RegisterEvent("ENCOUNTER_END")
  ...
  elseif startType == "combat" then
    ...
    M33AurasProfilingFrame:RegisterEvent("PLAYER_REGEN_DISABLED")
    M33AurasProfilingFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
    currentProfileState = startType
    delayedStart = true
  ...
  elseif startType and startType:match("%d") then
    ...
    ProfilingTimer = M33Auras.timer:ScheduleTimer(M33Auras.StopProfile, time)
    currentProfileState = "profiling"
  else
    M33AurasProfilingFrame:UnregisterAllEvents()
  ...
  M33AurasProfilingFrame:UpdateButtons()
  return delayedStart
end

---@diagnostic disable-next-line: duplicate-set-field
function M33Auras.StartProfile(startType)
...
function M33Auras.StopProfile()
  ...
  if M33AurasProfilingFrame then
    M33AurasProfilingFrame:UnregisterAllEvents()
    M33AurasProfilingFrame:UpdateButtons()
  end

  if ProfilingTimer then
    M33Auras.timer:CancelTimer(ProfilingTimer)
    ProfilingTimer = nil
  end
end

function M33Auras.ToggleProfile()
  if not profileData.systems.time or profileData.systems.time.count ~= 1 then
    M33Auras.StartProfile()
  else
...
local function ColoredSpike(spike)
  local r, g, b
  if spike < 2 then
    r, g, b = M33Auras.GetHSVTransition(spike / 2, 0, 1, 0, 1, 1, 1, 0, 1)
  elseif spike < 2.5 then
    r, g, b = M33Auras.GetHSVTransition((spike - 2) * 2, 1, 1, 0, 1, 1, 0.65, 0, 1)
  elseif spike < 3 then
    r, g, b = M33Auras.GetHSVTransition((spike - 2.5) * 2, 1, 0.65, 0, 1, 1, 0, 0, 1)
  else
    r, g, b = 1, 0, 0
  end
...
function M33Auras.PrintProfile()
  local popup = M33AurasProfilingReport
  ...
  if M33AurasProfilingFrame and M33AurasProfilingFrame:IsShown() then
    popup:SetParent(M33AurasProfilingFrame)
    popup:SetPoint("TOPLEFT", M33AurasProfilingFrame, "TOPRIGHT", 5, 0)
  else
    popup:SetParent(UIParent)
    if M33AurasSaved.ProfilingWindow then
      popup:SetPoint("TOPLEFT", UIParent, "TOPLEFT", M33AurasSaved.ProfilingWindow.xOffset or 0, M33AurasSaved.ProfilingWindow.yOffset or 0)
    else
  ...
  popup:AddText("You can ask on our discord https://discord.gg/M33Auras for help interpreting this output.")
...
function M33AurasProfilingMixin:OnShow()
  ...
  ButtonFrameTemplate_HidePortrait(self)
  self:SetTitle(L["M33Auras Profiling"])
  ...
  self.buttons.report.tooltip = L["A detailed overview of your auras and M33Auras systems\nCopy the whole text to Weakaura's Discord if you need assistance."]
  ...
  view:SetElementInitializer("M33AurasProfilingLineTemplate", function(frame, elementData)
...
function M33AurasProfilingStopButton_OnClick(self)
  if currentProfileState == "profiling" then
    M33Auras.StopProfile()
  else
...
function M33AurasProfilingMixin:InitDropDown()
...
function M33AurasProfilingMixin:InitModeDropDown()
  local function Initializer(dropDown, level)
    for i = 1, 2 do
      local info = UIDropDownMenu_CreateInfo()
      info.text = modes[i]
...
local lastRefresh
function M33AurasProfilingMixin:RefreshBars(_, force)
...
function M33AurasProfilingMixin:ResetBars()
  self.bars:Flush()
end

function M33AurasProfilingMixin:UpdateBar(name, time, pct, spike)
  ...
  if elementData then
    ...
    local button = M33AurasProfilingFrame.ScrollBox:FindFrame(elementData)
    if button then
...
function M33AurasProfilingMixin:SortByColumnIndex(index)
...
function M33AurasProfilingMixin:UpdateButtons()
...
function M33AurasProfilingMixin:Start()
  self:Show()
end

function M33AurasProfilingMixin:Stop()
  M33Auras.StopProfile()
  self:UpdateButtons()
  self:ResetBars()
end

function M33AurasProfilingMixin:Toggle()
...
function M33AurasProfilingColumnDisplay_OnClick(self, columnIndex)
