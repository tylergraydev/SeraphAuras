...
function Private.Modernize(data, oldSnapshot)
  if not data.internalVersion or data.internalVersion < 2 then
    M33Auras.prettyPrint(string.format("Data for '%s' is too old, can't modernize.", data.id))
  ...
  -- Version 10 is skipped, due to a bad migration script (see https://github.com/m33shoq/M33Auras/pull/1091)
    ...
    for triggerId, triggerData in ipairs(data.triggers) do
      if triggerData.trigger.type == "status" or triggerData.trigger.type == "event" then
        ...
        if newType then
          ...
          M33Auras.prettyPrint("Unknown trigger type found in, please report: ", data.id, triggerData.trigger.event)
        end
      end
    end
  end
  ...
  if data.internalVersion == 49 then
    ...
    if broken then
      M33Auras.prettyPrint(L["Trying to repair broken conditions in %s likely caused by a M33Auras bug."]:format(data.id))
    end
  end
  ...
  if data.internalVersion < 75 then
    -- this commit from nov 2019 https://github.com/m33shoq/M33Auras/commit/6d8f11c17422aeffdb82a0aa05181edfdd137896
    ...
    -- this commit https://github.com/m33shoq/M33Auras/commit/dbcb70b1e4df262af82f63620b3b0d80741e6df2
    ...
    for _, when in ipairs({ "start", "finish" }) do
      if data.actions[when] then
        if type(data.actions[when].sound) == "string" then
          data.actions[when].sound = data.actions[when].sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\M33Auras\\")
        end
      end
    end
      ...
      for conditionIndex, condition in ipairs(data.conditions) do
        for changeIndex, change in ipairs(condition.changes) do
          if change.property == "sound" and type(change.value.sound) == "string" then
            change.value.sound = change.value.sound:gsub("^Interface\\Add[oO]ns\\WeakAuras\\", "Interface\\AddOns\\M33Auras\\")
          end
        end
      end
    end
  ...
  data.internalVersion = max(data.internalVersion or 0, M33Auras.InternalVersion())
end
