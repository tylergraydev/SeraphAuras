...
if not M33Auras.IsCataOrMistsOrRetail() then return end
---@type string
local AddonName = ...
---@class Private
...
if LibSpec then
  ...
  frame:SetScript("OnEvent", function()
    ...
    if IsInRaid() then
      ...
      for i = 1, max do
        local name = GetUnitName(M33Auras.raidUnits[i], true)
        nameToUnitMap[name] = M33Auras.raidUnits[i]
      end
    else
      local max = GetNumSubgroupMembers()
      for i = 1, max do
        local name = GetUnitName(M33Auras.partyUnits[i], true)
        nameToUnitMap[name] = M33Auras.partyUnits[i]
      end
    end
  ...
  if M33Auras.IsRetail() then
    function Private.LibSpecWrapper.CheckTalentForUnit(unit, talentId)
      if Private.ExecEnv.UnitIsUnit(unit, "player") then
        return select(4, M33Auras.GetTalentById(talentId))
      end
...
M33Auras.SpecForUnit = Private.LibSpecWrapper.SpecForUnit
M33Auras.SpecRolePositionForUnit = Private.LibSpecWrapper.SpecRolePositionForUnit
M33Auras.CheckTalentForUnit = Private.LibSpecWrapper.CheckTalentForUnit

if Private.LibSpecWrapper.CheckGlyphForUnit then
  M33Auras.CheckGlyphForUnit = Private.LibSpecWrapper.CheckGlyphForUnit
else
  M33Auras.CheckGlyphForUnit = function() return nil end
end
