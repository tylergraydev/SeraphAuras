...
local defaultFont = M33Auras.defaultFont
local defaultFontSize = M33Auras.defaultFontSize
...
local properties = {
  ...
  text_text = {
    ...
    setter = "ChangeText",
    type = "string",
    control = "M33AurasInputWithIndentation"
  },
  text_color = {
    display = L["Color"],
    setter = "Color",
...
local function modify(parent, region, parentData, data, first)
  ...
  if not text:GetFont() and fontPath then -- workaround font not loading correctly
    local objectName = "M33Auras-Font-" .. data.text_font
  ...
  if text:GetFont() then
    text:SetText(M33Auras.ReplaceRaidMarkerSymbols(data.text_text));
  end
  ...
  if first then
    ...
    if containsCustomText and parentData.customText and parentData.customText ~= "" then
      parent.customTextFunc = M33Auras.LoadFunction("return "..parentData.customText, parentData.id)
  ...
  function region:ConfigureTextUpdate()
    ...
    if region.text_text and Private.ContainsAnyPlaceHolders(region.text_text) then
      UpdateText = function()
        ...
        if text:GetFont() then
          text:SetText(M33Auras.ReplaceRaidMarkerSymbols(textStr))
        end
...
M33Auras.RegisterSubRegionType("subtext", L["Text"], supports, create, modify, onAcquire, onRelease,
