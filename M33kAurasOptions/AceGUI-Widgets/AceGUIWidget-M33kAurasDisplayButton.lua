...
local Type, Version = "M33kAurasDisplayButton", 60
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      elseif(IsShiftKeyDown()) then
        ...
        if(mouseButton == "RightButton") then
          ...
          if(OptionsPrivate.IsDisplayPicked(self.data.id) and OptionsPrivate.IsPickedMultiple()) then
            LibDD:EasyMenu(OptionsPrivate.MultipleDisplayTooltipMenu(), M33kAuras_DropDownMenu, self.frame, 0, 0, "MENU");
          else
            UpdateClipboardMenuEntry(self.data);
            LibDD:EasyMenu(self.menu, M33kAuras_DropDownMenu, self.frame, 0, 0, "MENU");
      ...
      if(self.data.parent) then
        ...
        if(index) then
          if(index <= 1) then
            ...
            OptionsPrivate.Private.Animate("button", M33kAuras.GetData(parentData.controlledChildren[index-1]).uid, "main", updata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", M33kAuras.GetData(parentData.controlledChildren[index]).uid, "main", downdata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            M33kAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
      ...
      if(self.data.parent) then
        ...
        if(index) then
          if(index >= #parentData.controlledChildren) then
            ...
            local updata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = -32};
            local downdata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = 32};
            OptionsPrivate.Private.Animate("button", M33kAuras.GetData(parentData.controlledChildren[index+1]).uid, "main", downdata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", M33kAuras.GetData(parentData.controlledChildren[index]).uid, "main", updata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            M33kAuras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
    ...
    function self.callbacks.OnRenameAction(newid)
      ...
      if not(newid == oldid) then
        M33kAuras.Rename(self.data, newid);
      end
      self:UpdateParentWarning()
    end
    ...
    -- tinsert(self.menu, {
    --   text = L["|cFF8855FFM33kAuras Sync|r"],
    --   notCheckable = true,
    --   func = function()
    ...
    if picked then
      ...
      if mode == "GROUP" then
        if self.data.regionType == "dynamicgroup" then
          ...
          local parentData = self.data.parent and M33kAuras.GetData(self.data.parent)
  ...
  ["UpdateOffset"] = function(self)
    ...
    if group then
      ...
      while(group) do
        depth = depth + 1
        group = M33kAuras.GetData(group).parent
      end
