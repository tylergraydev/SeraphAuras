...
local methods = {
  ...
  ["Initialize"] = function(self, id, companionData)
    ...
    function self.callbacks.OnUpdateClick()
      ...
      M33kAuras.Import(self.companionData.encoded, nil, nil, linkedAuras)
    end
        ...
        if not self.linkedChildren[auraId] then
          tinsert(
            self.menu,
            {
              text = auraId,
              notCheckable = true,
              func = function() M33kAuras.PickDisplay(auraId, "information") end
            }
          )
        end
      end
      LibDD:EasyMenu(self.menu, M33kAuras_DropDownMenu, self.followLink, 0, 0, "MENU", 5)
    end
...
local function Constructor()
  local name = "M33kAurasPendingUpdateButton" .. AceGUI:GetNextWidgetNum(Type)
