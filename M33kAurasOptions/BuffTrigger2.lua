...
local function getAuraMatchesLabel(name)
  local ids = M33kAuras.spellCache.GetSpellsMatching(name)
  if ids then
    local numMatches = 0
...
local function getAuraMatchesList(name, showSpellIdRecommendation)
  ...
  if ids then
    ...
    for id, _ in pairs(ids) do
      ...
      if M33kAuras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(id) then
    ...
    if showSpellIdRecommendation then
      local tip = L["M33kAuras recommends using spell ids instead of names. Spell ids are automatically localized."]
  ...
  for i = 1, size do
    ...
    if isExactSpellId then
      aura_options[iconOption].name = function()
        return OptionsPrivate.Private.ExecEnv.GetSpellName(M33kAuras.SafeToNumber(trigger[optionKey] and trigger[optionKey][i]) or "")
      end
      ...
      aura_options[iconOption].name = function()
        local spellId = trigger[optionKey] and trigger[optionKey][i] and M33kAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
      ...
      aura_options[iconOption].desc = function()
        ...
        local spellId = input and M33kAuras.SafeToNumber(input)
        if spellId then
      ...
      aura_options[iconOption].func = function()
        local input = trigger[optionKey] and trigger[optionKey][i]
        local spellId = input and M33kAuras.SafeToNumber(trigger[optionKey][i])
        if spellId then
          -- Do nothing
        elseif input and input ~= "" then
    ...
    aura_options[prefix .. i] = {
      ...
      get = function(info)
        ...
        local spellName, _, _, _, _, _, spellID = OptionsPrivate.Private.ExecEnv.GetSpellInfo(M33kAuras.SafeToNumber(rawString))
        if spellName and spellID then
          return ("%s (%s)"):format(spellID, spellName) .. "\0" .. rawString
        elseif M33kAuras.SafeToNumber(rawString) then
      ...
      set = function(info, v)
        ...
        if v == "" then
          ...
          if isExactSpellId then
            ...
            local _, spellId = M33kAuras.spellCache.CorrectAuraName(v)
            if spellId then
      ...
      validate = isExactSpellId and M33kAuras.ValidateNumeric or nil,
      control = "M33kAurasInputFocus",
    }
  end
  -- VALIDATE ?
end
...
local function GetBuffTriggerOptions(data, triggernum)
  ...
  local ValidateNumeric = M33kAuras.ValidateNumeric
  local aura_options = {
    useUnit = {
      type = "toggle",
      width = M33kAuras.normalWidth,
    ...
    warnSpecifcUnit = {
      ...
      order = 10.4,
      hidden = function() return not (trigger.type == "aura2" and trigger.unit == "member" and M33kAuras.UntrackableUnit(trigger.specificUnit)) end
    },
    warnSoftTarget = {
      type = "description",
      width = M33kAuras.doubleWidth,
      ...
      order = 10.4,
      hidden = function() return not M33kAuras.IsUntrackableSoftTarget(trigger.unit) end
    },
    useDebuffType = {
      type = "toggle",
      width = M33kAuras.normalWidth,
    ...
    useActualSpec = {
      ...
      desc = L["Requires LibSpecialization, that is e.g. a up-to date M33kAuras version"],
      order = 66.3,
      hidden = function() return
        not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        and M33kAuras.IsCataOrMistsOrRetail())
      end,
    },
    actualSpec = {
      type = "multiselect",
    ...
    useArenaSpec = {
      ...
      order = 67.8,
      hidden = function() return
        not (M33kAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena")
      end
    },
    arena_spec = {
      type = "multiselect",
      ...
      values = OptionsPrivate.Private.spec_types_all,
      hidden = function()
        return not (M33kAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and trigger.useArenaSpec)
      end,
      order = 67.9
    },
    arena_specSpace = {
      ...
      width = M33kAuras.normalWidth,
      hidden = function()
        return not (M33kAuras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and not trigger.useArenaSpec)
      end,
    },
...
M33kAuras.RegisterTriggerSystemOptions({"aura2"}, GetBuffTriggerOptions)
