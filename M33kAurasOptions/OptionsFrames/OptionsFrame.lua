...
function OptionsPrivate.CreateFrame()
  LibDD:Create_UIDropDownMenu("M33kAuras_DropDownMenu", nil)
  local frame
  local db = OptionsPrivate.savedVars.db
  local odb = OptionsPrivate.savedVars.odb

  frame = CreateFrame("Frame", "M33kAurasOptions", UIParent, "PortraitFrameTemplate")
  ...
  function OptionsPrivate.SetTitle(title)
    local text = "M33kAuras " .. M33kAuras.versionString
    if title and title ~= "" then
      text = ("%s - %s"):format(text, title)
    end
    M33kAurasOptionsTitleText:SetText(text)
  end
  ...
  M33kAurasOptionsPortrait:SetTexture([[Interface\AddOns\M33kAuras\Media\Textures\logo_256_round.tga]])
  ...
  frame.UpdateFrameVisible = function(self)
    ...
    if self.minimized then
      M33kAurasOptionsTitleText:Hide()
      ...
      M33kAurasOptionsTitleText:Show()
  ...
  minimizebutton:SetPoint("RIGHT", frame.CloseButton, "LEFT", M33kAuras.IsClassicOrWrathOrCataOrMists() and 10 or 0, 0)
  ...
  local tipPopup = CreateFrame("Frame", nil, frame, "BackdropTemplateM33kAuras")
  ...
  local reportbugButton = addFooter(L["Found a Bug?"], [[Interface\AddOns\M33kAuras\Media\Textures\bug_report.tga]], "https://github.com/m33shoq/M33kAuras/issues/new?template=bug_report.yml",
  ...
  local wagoButton = addFooter(L["Find Auras"], [[Interface\AddOns\M33kAuras\Media\Textures\wago.tga]], "https://wago.io",
  ...
  container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - M33kAuras.normalWidth * 340, 0)
  ...
  local undo = AceGUI:Create("M33kAurasToolbarButton")
  undo:SetText(L["Undo"])
  undo:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\upleft")
  ...
  local redo = AceGUI:Create("M33kAurasToolbarButton")
  redo:SetText(L["Redo"])
  redo:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\upright")
  ...
  local newButton = AceGUI:Create("M33kAurasToolbarButton")
  newButton:SetText(L["New Aura"])
  newButton:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\newaura")
  ...
  local importButton = AceGUI:Create("M33kAurasToolbarButton")
  importButton:SetText(L["Import"])
  importButton:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\importsmall")
  ...
  local lockButton = AceGUI:Create("M33kAurasToolbarButton")
  lockButton:SetText(L["Lock Positions"])
  lockButton:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\lockPosition")
  lockButton:SetCallback("OnClick", function(self)
    if M33kAurasOptionsSaved.lockPositions then
      lockButton:SetStrongHighlight(false)
      lockButton:UnlockHighlight()
      M33kAurasOptionsSaved.lockPositions = false
    else
      lockButton:SetStrongHighlight(true)
      lockButton:LockHighlight()
      M33kAurasOptionsSaved.lockPositions = true
    end
  end)
  if M33kAurasOptionsSaved.lockPositions then
    lockButton:LockHighlight()
  ...
  local magnetButton = AceGUI:Create("M33kAurasToolbarButton")
  magnetButton:SetText(L["Magnetically Align"])
  magnetButton:SetTexture("Interface\\AddOns\\M33kAuras\\Media\\Textures\\magnetic")
  magnetButton:SetCallback("OnClick", function(self)
    if M33kAurasOptionsSaved.magnetAlign then
      magnetButton:SetStrongHighlight(false)
      magnetButton:UnlockHighlight()
      M33kAurasOptionsSaved.magnetAlign = false
    else
      magnetButton:SetStrongHighlight(true)
      magnetButton:LockHighlight()
      M33kAurasOptionsSaved.magnetAlign = true
    end
  end)
  ...
  local pendingUpdateButton = AceGUI:Create("M33kAurasLoadedHeaderButton")
  ...
  -- Loaded section
  local loadedButton = AceGUI:Create("M33kAurasLoadedHeaderButton")
  ...
  -- Not Loaded section
  local unloadedButton = AceGUI:Create("M33kAurasLoadedHeaderButton")
  ...
  local sidegroup = AceGUI:Create("M33kAurasInlineGroup")
  ...
  local dynamicTextCodesFrame = CreateFrame("Frame", "M33kAurasTextReplacements", sidegroup.frame, "PortraitFrameTemplate")
  ...
  frame.UpdateOptions = function(self)
    ...
    if type(self.pickedDisplay) == "string" then
      data = M33kAuras.GetData(frame.pickedDisplay)
    elseif self.pickedDisplay then
      data = tempGroup
    end
    ...
    if optionTable then
      AceConfigRegistry:RegisterOptionsTable("M33kAuras", optionTable, true)
    end
  end
  ...
  frame.FillOptions = function(self)
    ...
    container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - M33kAuras.normalWidth * 340, -10)
    ...
    local group = AceGUI:Create("M33kAurasInlineGroup")
  ...
  frame.NewAura = function(self)
    ...
    if self.pickedDisplay then
      if type(self.pickedDisplay) == "table" and tempGroup.controlledChildren and tempGroup.controlledChildren[1] then
        ...
        M33kAuras.PickDisplay(targetId)
      elseif type(self.pickedDisplay) == "string" then
    ...
    if targetId then
      ...
      if pickedButton.data.controlledChildren then
        ...
        local parentData = parent and M33kAuras.GetData(parent)
    ...
    if C_AddOns.GetAddOnEnableState("M33kAurasTemplates") ~= Enum.AddOnEnableState.None then
      ...
      local button = AceGUI:Create("M33kAurasNewButton")
    ...
    for index, regionType in ipairs(regionTypesSorted) do
      if (targetIsDynamicGroup and (regionType == "group" or regionType == "dynamicgroup")) then
        ...
        button:SetClick(function()
          M33kAuras.NewAura(nil, regionType, self:GetTargetAura())
        end)
        containerScroll:AddChild(button)
      end
    end
    ...
    -- Import
    local importButton = AceGUI:Create("M33kAurasNewButton")
  ...
  frame.PickDisplayMultiple = function(self, id)
    if not self.pickedDisplay then
      ...
      if type(self.pickedDisplay) == "string" then
        if M33kAuras.GetData(self.pickedDisplay).controlledChildren or M33kAuras.GetData(id).controlledChildren then
          wasGroup = true
  ...
  frame.GetPickedDisplay = function(self)
    if type(self.pickedDisplay) == "string" then
      return M33kAuras.GetData(self.pickedDisplay)
    end
    return self.pickedDisplay
  end
