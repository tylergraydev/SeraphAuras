...
local function ConstructMover(frame)
  ...
  arrowTexture:SetTexture("Interface\\Addons\\M33kAuras\\Media\\Textures\\offscreen.tga")
  ...
  UpdateHighlight = function(self)
    if self.highlight then
      for _, data in ipairs(self.references) do
        local region = M33kAuras.GetRegion(data.id)
...
AlignmentLines.CreateLines = function(self, data, sizerPoint)
  ...
  local align = (M33kAurasOptionsSaved.magnetAlign and not IsShiftKeyDown())
                    or (not M33kAurasOptionsSaved.magnetAlign and IsShiftKeyDown())
  if align then
...
local function ConstructMoverSizer(parent)
  local frame = CreateFrame("Frame", nil, parent, "BackdropTemplateM33kAuras")
  frame:SetBackdrop({
