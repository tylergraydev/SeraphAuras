...
local function Diff(ours, theirs)
  ...
  -- generates a diff which M33kAuras.Update can use
  local debug = false
  ...
  uidMap.EnsureUniqueIdOfUnmatched = function(self, uid, IncProgress)
    ...
    if not self:GetUIDMatch(uid) then
      if OptionsPrivate.Private.GetDataByUID(uid) then
        local newUid = M33kAuras.GenerateUniqueID()
...
local function AddAuraList(container, uidMap, list, categories, expandText)
  local expand = AceGUI:Create("M33kAurasExpand")
  local collapsed = true
  local image = collapsed and "Interface\\AddOns\\M33kAuras\\Media\\Textures\\expand"
                           or "Interface\\AddOns\\M33kAuras\\Media\\Textures\\collapse"
  ...
  local auraLabelContainer = AceGUI:Create("M33kAurasInlineGroup")
...
local methods = {
  Open = function(self, data, children, target, linkedAuras, sender, callbackFunc)
    ...
    do
      ...
      if (highestVersion > M33kAuras.InternalVersion()) then
        ...
        highestVersionWarning:SetText(L["This aura was created with a newer version of M33kAuras.\nUpgrade your version of M33kAuras or wait for next release before installing this aura."])
    ...
    if matchInfo ~= nil then
      if not hasChanges(matchInfo) then
        ...
        self:AddChild(AceGUI:Create("M33kAurasSpacer"))
        ...
        local updateUiArea = AceGUI:Create("M33kAurasInlineGroup")
    ...
    local currentBuild = floor(M33kAuras.BuildInfo / 10000)
  ...
  CreateUpdateArea = function(self, area, matchInfo)
    area:AddChild(AceGUI:Create("M33kAurasSpacer"))
  ...
  Import = function(self)
    if M33kAuras.IsClassicEra() and C_GameRules.IsHardcoreActive() then
      StaticPopupDialogs["M33kAuras_CONFIRM_IMPORT_HARDCORE"] = {
  ...
  ImportImpl = function(self)
    ...
    if pendingPickData then
      ...
      M33kAuras.PickDisplay(pendingPickData.id, pendingPickData.tabToShow)
    end
  ...
  -- Note: There's a final renaming via M33kAuras.Rename at the end of the update process
  FixUpNames = function(self, uidMap, uid)
    ...
    if existingData then
      ...
      if M33kAuras.GetData(uidMap:GetIdFor(uid)) then
  ...
  UpdatePhase1 = function(self, structureUidMap, uid, GetPhase1Data, phase2Order)
    ...
    M33kAuras.Add(data)
    M33kAuras.NewDisplayButton(data, true)
    self:IncProgress10()
  ...
  UpdatePhase2 = function(self, structureUidMap, GetPhase2Data, phase2Order, copies)
    for i = #phase2Order, 1, -1 do
      ...
      M33kAuras.UpdateGroupOrders(data)
  ...
  AddProgressWidgets = function(self)
    ...
    local progress = AceGUI:Create("M33kAurasProgressBar")
