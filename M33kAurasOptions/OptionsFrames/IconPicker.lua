...
local function ConstructIconPicker(frame)
  ...
  local function iconPickerFill(subname, doSort)
    ...
    local AddButton = function(name, icon)
      local button = AceGUI:Create("M33kAurasIconButton");
      button:SetName(name);
  ...
  local input = CreateFrame("EditBox", "M33kAurasFilterInput", group.frame, "SearchBoxTemplate")
  ...
  local icon = AceGUI:Create("M33kAurasIconButton");
  icon.frame:Disable();
  ...
  function group.Pick(self, texturePath)
    ...
    if self.groupIcon then
      ...
      M33kAuras.Add(self.baseObject)
      M33kAuras.ClearAndUpdateOptions(self.baseObject.id)
      M33kAuras.UpdateThumbnail(self.baseObject)
    else
      for child in OptionsPrivate.Private.TraverseLeafsOrAura(self.baseObject) do
        ...
        M33kAuras.UpdateThumbnail(child);
      end
    end
  ...
  function group.CancelClose()
    ...
    if group.groupIcon then
      ...
      M33kAuras.Add(group.baseObject)
      M33kAuras.ClearAndUpdateOptions(group.baseObject.id)
      M33kAuras.UpdateThumbnail(group.baseObject)
    else
