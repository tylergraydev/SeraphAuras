...
function OptionsPrivate.GetInformationOptions(data)
  ...
  if not isTmpGroup then
    args.name = {
      ...
      set = function(info, newid)
        if data.id ~= newid and not M33kAuras.GetData(newid) then
          local oldid = data.id
  ...
  if OptionsPrivate.HasWagoUrl(data.id) then
    args.ignoreWagoUpdate = {
      ...
      set = function(info, v)
          local auraData = M33kAuras.GetData(data.id)
          if auraData then
            local ignoreUpdate
    ...
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(data) do
      if child.information.saved then
        args["savedData." .. child.uid] = {
          ...
          width = savedDataCount > 1 and M33kAuras.doubleWidth or M33kAuras.normalWidth,
          order = order,
        }
        order = order + 1
      end
  ...
  if not sameDebugLog or commonDebugLog then
    args.debugLogShow = {
      ...
      func = function()
        --- @type string
        local fullMessage = L["M33kAuras %s on WoW %s"]:format(M33kAuras.versionString, M33kAuras.BuildInfo) .. "\n\n"
        --- @type boolean
