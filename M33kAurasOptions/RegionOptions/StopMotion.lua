...
local texture_types = M33kAuras.StopMotion.texture_types;
local texture_data = M33kAuras.StopMotion.texture_data;
local animation_types = M33kAuras.StopMotion.animation_types;
...
local function modifyThumbnail(parent, region, data, fullModify, size)
    ...
    local texture = data.foregroundTexture or "Interface\\AddOns\\M33kAuras\\Media\\Textures\\stopmotion";
    ...
    if (region.foreground.rows and region.foreground.columns) then
      ...
      M33kAuras.setTile(region.texture, frame, region.foreground.rows, region.foreground.columns, frameScaleW, frameScaleH);

      region.SetValue = function(self, percent)
        local frame = floor(percent * (region.endFrame - region.startFrame) + region.startFrame);
        M33kAuras.setTile(self.texture, frame, region.foreground.rows, region.foreground.columns, frameScaleW, frameScaleH);
      end
    else
...
local function createIcon()
    local data = {
        height = 30,
        width = 30,
        foregroundTexture = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\stopmotion",
