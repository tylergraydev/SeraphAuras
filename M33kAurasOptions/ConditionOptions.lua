...
local function addControlsForChange(args, order, data, conditionVariable, totalAuraCount, conditions, i, j, allProperties, usedProperties, conditionTemplates)
      ...
      elseif (property == "DELETE") then
        if (data.controlledChildren) then
          for id, reference in pairs(conditions[i].changes[j].references) do
            local auraData = M33kAuras.GetData(id);
  ...
  elseif (propertyType == "bool" or propertyType == "number") then
    ...
    if (propertyType == "number") then
      ...
      if (properties and (properties.min or properties.softMin)) and (properties.max or properties.softMax) then
        ...
        args["condition" .. i .. "value" .. j].control = "M33kAurasSpinBox"
        ...
        args["condition" .. i .. "value" .. j].validate = M33kAuras.ValidateNumeric;
      end
    else
  ...
  elseif (propertyType == "string" or propertyType == "texture") then
    ...
    if propertyType == "texture" then
      args["condition" .. i .. "value" .. j].width =  M33kAuras.normalWidth - 0.15
  ...
  elseif (propertyType == "list" or propertyType == "progressSource" or propertyType == "textureLSM") then
    ...
    if propertyType == "progressSource" then
      args["condition" .. i .. "value" .. j].control = "M33kAurasTwoColumnDropdown"
  ...
  elseif (propertyType == "sound") then
    ...
    args["condition" .. i .. "value" .. j .. "sound"] = {
      ...
      itemControl = "M33kAurasMediaSound",
  ...
  elseif (propertyType == "chat") then
    ...
    args["condition" .. i .. "value" .. j .. "message dest_text_replacements_button"] = {
      ...
      imageHeight = 24,
      control = "M33kAurasIcon",
      image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\sidebar",
    }
    order = order + 1;
    ...
    args["condition" .. i .. "value" .. j .. "custom"] = {
      ...
      control = "M33kAurasMultiLineEditBox",
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              else
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, nil, nil, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              end
            end
          }
        }
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, true, true, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#run-custom-code");
              else
                data.conditions[i].changes[j].value = data.conditions[i].changes[j].value or {};
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, true, nil, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#run-custom-code");
              end
            end
          }
        }
...
local function addControlsForIfLine(args, order, data, conditionVariable, totalAuraCount, conditions, i, path, conditionTemplates, conditionTemplateWithoutCombinations, allProperties, parentType)
  ...
  local normalWidth = M33kAuras.normalWidth - indentWidth;
  ...
  if (indentWidth > 0) then
    ...
    args["space" .. order] = {
      ...
      image = function() return "", 0, 0 end,
      order = order,
      width = M33kAuras.doubleWidth * 1.5,
    }
    order = order + 1;
    ...
    args["condition" .. i .. tostring(path) .. "bullet"] = {
      ...
      name = "",
      order = order,
      image = "Interface\\Addons\\M33kAuras\\Media\\Textures\\bullet" .. indentDepth,
      imageWidth = 10,
      imageHeight = 10,
    }
    order = order + 1;
      ...
      if (conditionTemplatesToUse.indexToTrigger[v] == "REMOVE") then
        if (data.controlledChildren) then
          for id, reference in pairs(conditions[i].check.references) do
            ...
            M33kAuras.Add(auraData)
            M33kAuras.ClearAndUpdateOptions(auraData.id)
          end
        else
          ...
          extraFunctions = {
            {
              ...
              func = function()
                if (data.controlledChildren) then
                  ...
                  OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#custom-check");
                else
                  ...
                  OptionsPrivate.OpenTextEditor(data, fullPath, nil, nil, nil, nil, "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#custom-check");
                end
              end
            }
          }
...
local function addControlsForCondition(args, order, data, conditionVariable, totalAuraCount, conditions, i, conditionTemplates, conditionTemplateWithoutCombinations, allProperties)
  ...
  args["condition" .. i .. "header"] = {
    ...
    order = order,
    width = M33kAuras.doubleWidth - 0.6,
    func = function()
      if data.controlledChildren then
        for id, reference in pairs(conditions[i].check.references) do
          local index = reference.conditionIndex
    ...
    image = collapsed and "Interface\\AddOns\\M33kAuras\\Media\\Textures\\expand" or "Interface\\AddOns\\M33kAuras\\Media\\Textures\\collapse" ,
    imageWidth = 18,
    imageHeight = 18,
    control = "M33kAurasExpand",
  ...
  args["condition" .. i .. "up"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\moveup",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33kAurasIcon"
  };
  ...
  args["condition" .. i .. "down"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\movedown",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33kAurasIcon"
  };
  ...
  args["condition" .. i .. "duplicate"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\duplicate",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33kAurasIcon"
  };
  ...
  args["condition" .. i .. "delete"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\delete",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33kAurasIcon"
  };
