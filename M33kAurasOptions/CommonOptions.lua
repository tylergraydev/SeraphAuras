...
local function addCollapsibleHeader(options, key, input, order, isGroupTab)
  ...
  local titleWidth = M33kAuras.doubleWidth - (hasAdd and 0.15 or 0) - (hasDelete and 0.15 or 0)  - (hasUp and 0.15 or 0)
  ...
  if not withoutheader then
    options[key .. "collapseButton"] = {
      ...
      image = function()
        if notcollapsable then
          return "Interface\\AddOns\\M33kAuras\\Media\\Textures\\bullet1", 18, 18
        else
          return isCollapsed() and "Interface\\AddOns\\M33kAuras\\Media\\Textures\\expand"
    ...
    if hasApplyTemplate then
      options[key .. "applyTemplate"] = {
        ...
        order = order + 0.7,
        width = 0.15,
        image = "Interface\\AddOns\\M33kAuras\\Media\\Textures\\template",
        imageWidth = 24,
        imageHeight = 24,
        control = "M33kAurasIcon",
        hidden = hiddenFunc
...
local function ProgressOptions(data)
  ...
  options.adjustedMin = {
    type = "input",
    validate = M33kAuras.ValidateNumericOrPercent,
    width = M33kAuras.normalWidth,
    order = order + 2,
    name = L["Minimum"],
...
local function PositionOptions(id, data, _, hideWidthHeight, disableSelfPoint, group)
  ...
  local positionOptions = {
    ...
    selfPoint = {
      ...
      control = "M33kAurasAnchorButtons",
    },
    anchorPoint = {
      type = "select",
      width = M33kAuras.normalWidth,
  ...
  OptionsPrivate.commonOptions.AddCodeOption(positionOptions, data, L["Custom Anchor"], "custom_anchor",
                      "https://github.com/m33shoq/M33kAuras/wiki/Custom-Code-Blocks#custom-anchor-function",
...
local function AddCommonTriggerOptions(options, data, triggernum, doubleWidth)
  ...
  options.type = {
    type = "select",
    width = doubleWidth and M33kAuras.doubleWidth or M33kAuras.normalWidth,
    name = L["Type"],
