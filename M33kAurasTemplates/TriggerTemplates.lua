...
local function createDurationTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Combat Log"),
      event = "Combat Log",
...
local function createTotemTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Totem"),
      event = "Totem",
...
local function createPowerTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Power"),
      event = "Power",
      use_unit = true,
      unit = "player",
      use_powertype = true,
...
local function createHealthTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Health"),
      event = "Health",
      unit = "player",
      use_unit = true,
      use_absorbMode = true,
...
local function createCastTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Cast"),
      event = "Cast",
      use_unit = true,
...
local function createAbilityTrigger(triggers, position, item, genericShowOn)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Cooldown Progress (Spell)"),
...
local function createItemTrigger(triggers, position, item, genericShowOn)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Cooldown Progress (Item)"),
...
local function createOverlayGlowTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Spell Activation Overlay"),
...
local function createWeaponEnchantTrigger(triggers, position, item, showOn)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Weapon Enchant"),
...
local function createQueuedActionTrigger(triggers, position, item)
  triggers[position] = {
    trigger = {
      type = M33kAuras.GetTriggerCategoryFor("Queued Action"),
      event = "Queued Action",
      spellName = item.spell
    }
  }
...
local function subTypesFor(item, regionType)
  ...
  local icon = {
    ...
    glow = function()
      ...
      t1:SetTexture(134376);
      t1:SetAllPoints(thumbnail);
      M33kAuras.ShowOverlayGlow(thumbnail); -- where to call HideOverlayGlow() ?
      return thumbnail;
    end, -- 571554
    charges = function()
      local thumbnail = createThumbnail();
...
function M33kAuras.CreateTemplateView(Private, frame)
  ...
  local function createRegionFlyout(regionType, regionData)
    local group = AceGUI:Create("M33kAurasTemplateGroup");
    group:SetFullWidth(true);
    group:SetLayout("WATemplateTriggerLayoutFlyout");
    for _, item in ipairs(regionData.templates) do
      local templateButton = AceGUI:Create("M33kAurasNewButton");
      if (item.icon) then
      ...
      templateButton:SetClick(function()
        ...
        newView.data.internalVersion = M33kAuras.InternalVersion();
            ...
            if #subTypes < 2 then
              ...
              if (newView.existingAura) then
                ...
                if (subType.data) then
                  M33kAuras.DeepMixin(newView.data, subType.data)
                end
                newView:CancelClose();
                M33kAuras.NewAura(newView.data, newView.data.regionType, newView.targetId);
              end
            else
  ...
  local function createLastPage()
    local replaceButton = AceGUI:Create("M33kAurasNewButton");
    ...
    replaceButton:SetClick(function()
      ...
      if (newView.chosenItem.load) then
        M33kAuras.DeepMixin(newView.data.load, newView.chosenItem.load)
      end
    end);
    newViewScroll:AddChild(replaceButton);

    local addButton = AceGUI:Create("M33kAurasNewButton");
  ...
  createButtons = function(selectedItem) -- selectedItem is either a regionType or a trigger section
    ...
    elseif (newView.data and not newView.chosenItem) then
      ...
      -- Class
      local classSelector = createDropdown("class", M33kAuras.class_types);
      newViewScroll:AddChild(classSelector);

      if M33kAuras.IsRetail() then
        local specSelector = createDropdown("spec", M33kAuras.spec_types_specific[newView.class]);
      ...
      local raceSelector = createDropdown("race", M33kAuras.race_types);
