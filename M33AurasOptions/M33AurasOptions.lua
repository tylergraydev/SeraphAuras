...
local L = M33Auras.L
local ADDON_NAME = "M33AurasOptions";

local displayButtons = {};
OptionsPrivate.displayButtons = displayButtons;

local spellCache = M33Auras.spellCache;
local savedVars = {};
...
function OptionsPrivate.DuplicateAura(data, newParent, massEdit, targetIndex)
  ...
  while(M33Auras.GetData(new_id)) do
    new_id = base_id .. num
    num = num + 1
  end
  ...
  newData.id = new_id
  newData.parent = nil
  newData.uid = M33Auras.GenerateUniqueID()
  if newData.controlledChildren then
    newData.controlledChildren = {}
  end
  M33Auras.Add(newData)
  M33Auras.NewDisplayButton(newData, massEdit)
  if(newParent or data.parent) then
    local parentId = newParent or data.parent
    local parentData = M33Auras.GetData(parentId)
    local index
    if targetIndex then
      index = targetIndex
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if (event == "ADDON_LOADED") then
    if(addon == ADDON_NAME) then
      db = M33AurasSaved;
      M33AurasOptionsSaved = M33AurasOptionsSaved or {};

      odb = M33AurasOptionsSaved;
...
local function commonParent(controlledChildren)
  ...
  for index, id in ipairs(controlledChildren) do
    local childData = M33Auras.GetData(id);
    local childButton = OptionsPrivate.GetDisplayButton(id)
...
local function CreateNewGroupFromSelection(regionType, resetChildPositions)
  ...
  M33Auras.DeepMixin(data, OptionsPrivate.Private.data_stub)
  data.internalVersion = M33Auras.InternalVersion()
  ...
  if (parent) then
    ...
    M33Auras.Add(parentData);
    OptionsPrivate.Private.AddParents(parentData)
    M33Auras.NewDisplayButton(data);
    M33Auras.UpdateGroupOrders(parentData);
    OptionsPrivate.ClearOptions(parentData.id);
  ...
  for index, childId in pairs(tempGroup.controlledChildren) do
    ...
    local oldParentData = M33Auras.GetData(oldParent)
    if (oldParent) then
      local oldIndex = childButton:GetGroupOrder()

      tremove(oldParentData.controlledChildren, oldIndex)
      M33Auras.Add(oldParentData)
      OptionsPrivate.Private.AddParents(oldParentData)
      M33Auras.UpdateGroupOrders(oldParentData);
      M33Auras.ClearAndUpdateOptions(oldParent);
...
function OptionsPrivate.MultipleDisplayTooltipMenu()
  ...
  local menu = {
    ...
    },
    {
      text = L["Close"],
      notCheckable = 1,
      func = function() M33Auras_DropDownMenu:Hide() end
    }
  };
  ...
  if (allSameParent and commonParent) then
    local parentData = M33Auras.GetData(commonParent);
...
StaticPopupDialogs["M33Auras_CONFIRM_DELETE"] = {
  text = "",
  button1 = L["Delete"],
  button2 = L["Cancel"],
...
function OptionsPrivate.IsWagoUpdateIgnored(auraId)
    local auraData = M33Auras.GetData(auraId)
      if auraData then
...
function OptionsPrivate.ConfirmDelete(toDelete, parents)
  if toDelete then
    ...
    StaticPopupDialogs["M33Auras_CONFIRM_DELETE"].text = warningForm:format(#toDelete)
    StaticPopup_Show("M33Auras_CONFIRM_DELETE", "", "", {toDelete = toDelete, parents = parents})
  end
end
...
if not M33Auras.ToggleOptions then
  ---@type fun(msg: string, Private: Private)
  function M33Auras.ToggleOptions(msg, Private)
    if not Private then
      return
    end
    ...
    elseif (InCombatLockdown()) then
      M33Auras.prettyPrint(L["Options will open after combat ends."])
      reopenAfterCombat = true;
    else
      M33Auras.ShowOptions(msg);
    end
  end
end

function M33Auras.HideOptions()
  if(frame) then
    frame:Hide()
  end
end
...
local function EnsureDisplayButton(data)
  ...
  if not(displayButtons[id]) then
    displayButtons[id] = AceGUI:Create("M33AurasDisplayButton");
    if(displayButtons[id]) then
      displayButtons[id]:SetData(data);
      displayButtons[id]:Initialize();
      displayButtons[id]:UpdateWarning()
    else
      print("|cFF8800FFM33Auras|r: Error creating button for", id);
    end
  end
end
  ...
  local func1 = function()
    ...
    if parents then
      for id in pairs(parents) do
        local parentData = M33Auras.GetData(id)
...
function M33Auras.ShowOptions(msg)
  ...
  if (frame.pickedDisplay) then
    if (OptionsPrivate.IsPickedMultiple()) then
      ...
      M33Auras.PickDisplay(frame.pickedDisplay);
    end
  else
    frame:NewAura();
  end
...
function M33Auras.ClearAndUpdateOptions(id, clearChildren)
...
function M33Auras.NewDisplayButton(data, massEdit)
  local id = data.id;
  OptionsPrivate.Private.ScanForLoads({[id] = true});
  EnsureDisplayButton(db.displays[id]);
  M33Auras.UpdateThumbnail(db.displays[id]);
...
function M33Auras.UpdateGroupOrders(data)
  if(data.controlledChildren) then
...
function OptionsPrivate.SortDisplayButtons(filter, overrideReset, id)
  ...
  if OptionsPrivate.Private.CompanionData.stash then
    for id, companionData in pairs(OptionsPrivate.Private.CompanionData.stash) do
      ...
      if frame.pendingInstallButton:GetExpanded() then
        if not child then
          child = AceGUI:Create("M33AurasPendingInstallButton")
  ...
  if OptionsPrivate.Private.CompanionData.slugs then
    ...
    for id, aura in pairs(M33AurasSaved.displays) do
          ...
          if auraData and auraData.wagoVersion then
            if tonumber(auraData.wagoVersion) > tonumber(version) then
              ...
              if frame.pendingUpdateButton:GetExpanded() then
                ...
                if not child then
                  child = AceGUI:Create("M33AurasPendingUpdateButton")
  ...
  for _, id in ipairs(topLevelLoadedAuras) do
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(M33Auras.GetData(id)) do
...
function M33Auras.PickDisplay(id, tab, noHide)
...
function OptionsPrivate.PickDisplayMultipleShift(target)
  if (frame.pickedDisplay) then
    ...
    if (first and first ~= target) then
      ...
      local firstData = M33Auras.GetData(first);
      local targetData = M33Auras.GetData(target);
      if (firstData.parent == targetData.parent and not targetData.controlledChildren and not firstData.controlledChildren) then
        ...
        if (firstData.parent) then
          local group = M33Auras.GetData(targetData.parent);
          for index, child in ipairs(group.controlledChildren) do
            ...
            if (child == target or child == first) then
              ...
              for i = index + 1, #group.controlledChildren do
                ...
                if (M33Auras.GetData(current).controlledChildren) then
                  -- Skip sub groups
                else
        ...
        elseif (firstData.parent == nil and targetData.parent == nil) then
          ...
          for index, button in ipairs(frame.buttonsScroll.children) do
            if button.type == "M33AurasDisplayButton" then
...
local function CompareButtonOrder(a, b)
  ...
  while(aNode) do
    local parent = M33Auras.GetData(aNode).parent
    if (parent) then
  ...
  while(bNode) do
    local parent = M33Auras.GetData(bNode).parent
    if parent then
...
function OptionsPrivate.DropIndicator()
  ...
  if not indicator then
    ---@class Frame
    indicator = CreateFrame("Frame", "M33Auras_DropIndicator")
    ...
    groupTexture:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\Square_FullWhite")
...
function M33Auras.UpdateThumbnail(data)
  local id = data.id
...
function OptionsPrivate.OpenModelPicker(baseObject, path)
  if not(C_AddOns.IsAddOnLoaded("M33AurasModelPaths")) then
    local loaded, reason = C_AddOns.LoadAddOn("M33AurasModelPaths");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      M33Auras.prettyPrint(string.format(L["ModelPaths could not be loaded, the addon is %s"], reason));
      M33Auras.ModelPaths = {};
    end
    OptionsPrivate.ModelPicker(frame).modelTree:SetTree(M33Auras.ModelPaths)
  end
...
function OptionsPrivate.OpenTriggerTemplate(data, targetId)
  if not(C_AddOns.IsAddOnLoaded("M33AurasTemplates")) then
    local loaded, reason = C_AddOns.LoadAddOn("M33AurasTemplates");
    if not(loaded) then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      M33Auras.prettyPrint(string.format(L["Templates could not be loaded, the addon is %s"], reason));
      return;
    end
    frame.newView = M33Auras.CreateTemplateView(OptionsPrivate.Private, frame);
  end
...
function OptionsPrivate.UpdateTextReplacements(frame, data)
  ...
  -- Create a modified M33AurasSnippetButton for each property and add it to ScrollList
  ...
  for i, prop in ipairs(sortedProps) do
    if prop.type == "header" then
      ...
      local button = AceGUI:Create("M33AurasSnippetButton")
...
function M33Auras.SetMoverSizer(id)
...
function M33Auras.GetMoverSizerId()
...
function M33Auras.NewAura(sourceData, regionType, targetId)
  local function ensure(t, k, v)
    return t and k and v and t[k] == v
  end
  local new_id = OptionsPrivate.Private.FindUnusedId("New")
  local data = {id = new_id, regionType = regionType, uid = M33Auras.GenerateUniqueID()}
  M33Auras.DeepMixin(data, OptionsPrivate.Private.data_stub);
  if (sourceData) then
    M33Auras.DeepMixin(data, sourceData);
  end
  data.internalVersion = M33Auras.InternalVersion();
  OptionsPrivate.Private.validate(data, OptionsPrivate.Private.regionTypes[regionType].default);
  ...
  if targetId then
    ...
    if (target) then
      ...
      if (group) then
        ...
        M33Auras.Add(data);
        M33Auras.Add(group.data);
        OptionsPrivate.Private.AddParents(group.data)
        M33Auras.NewDisplayButton(data);
        M33Auras.UpdateGroupOrders(group.data);
      ...
      error(string.format("Calling 'M33Auras.NewAura' with invalid groupId %s. Reload your UI to fix the display list.", targetId))
    end
  else
    -- move source into the top-level list
    M33Auras.Add(data);
