...
typeControlAdders = {
  ...
  number = function(options, args, data, order, prefix, i)
    ...
    args[prefix .. "min"] = {
      type = "input",
      width = M33Auras.normalWidth * 2 / 3,
  ...
  multiselect = function(options, args, data, order, prefix, i)
    ...
    args[prefix .. "default"] = {
      type = "multiselect",
      width = M33Auras.normalWidth * 0.9,
      name = L["Default"],
      order = order(),
      values = defaultValues,
      get = function(_, k)
  ...
  group = function(options, args, data, order, prefix, i)
    ...
    args[prefix .. "noMerge"] = {
      type = "toggle",
      name = M33Auras.newFeatureString .. name(option, "noMerge", L["Prevent Merging"]),
      desc = desc(option, "noMerge", L["If checked, then this group will not merge with other group when selecting multiple auras."]),
      order = order(),
      width = option.groupType =="simple" and M33Auras.doubleWidth or M33Auras.normalWidth,
    ...
    if option.groupType ~="simple" then
      args[prefix .. "sortAlphabetically"] = {
        type = "toggle",
        name = M33Auras.newFeatureString .. name(option, "sortAlphabetically", L["Sort"]),
...
function addAuthorModeOption(options, args, data, order, prefix, i)
  ...
  args[prefix .. "collapse"] = {
    ...
    order = order(),
    width = M33Auras.doubleWidth - buttonWidth,
    func = function()
      for id, optionData in pairs(option.references) do
    ...
    image = collapsed and "Interface\\AddOns\\M33Auras\\Media\\Textures\\expand" or
      "Interface\\AddOns\\M33Auras\\Media\\Textures\\collapse",
    imageWidth = 18,
    imageHeight = 18,
  ...
  args[prefix .. "upAndIn"] = {
    ...
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\upright",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  }
  args[prefix .. "downAndIn"] = {
    ...
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\downright",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  }
  ...
  args[prefix .. "upAndOut"] = {
    ...
    func = function()
      for id, optionData in pairs(option.references) do
        ...
        M33Auras.Add(optionData.data)
      end
      M33Auras.ClearAndUpdateOptions(data.id, true)
    end,
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\upleft",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  }
  args[prefix .. "downAndOut"] = {
    ...
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\downleft",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  }
  ...
  if optionClass == "simple" then
    ...
    args[prefix .. "tooltip"] = {
      ...
      order = order(),
      width = M33Auras.normalWidth * 1.5,
...
local function addUserModeOption(options, args, data, order, prefix, i)
  ...
  if optionClass == "simple" then
    userOption = {
      ...
      width = (option.width or 1) * M33Auras.normalWidth,
      order = order(),
      get = getUser(option),
    ...
    if not collapsed then
      ...
      if option.groupType == "array" then
        ...
        args[prefix .. "resetEntry"] = {
          ...
          width = 0.15,
          image = "Interface\\Addons\\M33Auras\\Media\\Textures\\reset",
          imageWidth = 18,
          imageHeight = 18,
          control = "M33AurasIcon"
        }
        if option.limitType ~= "fixed" then
          args[prefix .. "createEntry"] = {
            ...
            width = 0.15,
            image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\add",
            imageWidth = 18,
            imageHeight = 18,
            control = "M33AurasIcon"
          }
  ...
  -- convert from M33Auras option type to ace option type
  if optionClass == "simple" then
