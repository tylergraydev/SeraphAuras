...
local function getAuraMatchesLabel(name)
  local ids = M33Auras.spellCache.GetSpellsMatching(name)
  if ids then
    local numMatches = 0
...
local function getAuraMatchesList(name, showSpellIdRecommendation)
  ...
  if ids then
    ...
    for id, _ in pairs(ids) do
      ...
      if M33Auras.IsPlayerSpellOrOverridesAndBaseIsPlayerSpell(id) then
    ...
    if showSpellIdRecommendation then
      local tip = L["M33Auras recommends using spell ids instead of names. Spell ids are automatically localized."]
  ...
  for i = 1, size do
    ...
    if i ~= 1 then
      aura_options[prefix .. "space" .. i] = {
        ...
        width = M33Auras.normalWidth - 0.2,
    ...
    if isExactSpellId then
      aura_options[iconOption].name = function()
        return OptionsPrivate.Private.ExecEnv.GetSpellName(M33Auras.SafeToNumber(trigger[optionKey] and trigger[optionKey][i]) or "")
      end
      ...
      aura_options[iconOption].name = function()
        local spellId = trigger[optionKey] and trigger[optionKey][i] and M33Auras.SafeToNumber(trigger[optionKey][i])
        if spellId then
      ...
      aura_options[iconOption].desc = function()
        ...
        local spellId = input and M33Auras.SafeToNumber(input)
        if spellId then
      ...
      aura_options[iconOption].image = function()
        ...
        elseif input and input ~= "" then
          icon = "Interface\\AddOns\\M33Auras\\Media\\Textures\\info"
        end
        return icon and tostring(icon) or "", 18, 18
      end
      ...
      aura_options[iconOption].func = function()
        local input = trigger[optionKey] and trigger[optionKey][i]
        local spellId = input and M33Auras.SafeToNumber(trigger[optionKey][i])
        if spellId then
          -- Do nothing
        elseif input and input ~= "" then
    ...
    aura_options[prefix .. i] = {
      ...
      get = function(info)
        ...
        local spellName, _, _, _, _, _, spellID = OptionsPrivate.Private.ExecEnv.GetSpellInfo(M33Auras.SafeToNumber(rawString))
        if spellName and spellID then
          return ("%s (%s)"):format(spellID, spellName) .. "\0" .. rawString
        elseif M33Auras.SafeToNumber(rawString) then
      ...
      set = function(info, v)
        ...
        if v == "" then
          ...
          if isExactSpellId then
            ...
            local _, spellId = M33Auras.spellCache.CorrectAuraName(v)
            if spellId then
      ...
      validate = isExactSpellId and M33Auras.ValidateNumeric or nil,
      control = "M33AurasInputFocus",
    }
  end
  -- VALIDATE ?
end
...
local function GetBuffTriggerOptions(data, triggernum)
  ...
  local ValidateNumeric = M33Auras.ValidateNumeric
  local aura_options = {
    useUnit = {
      type = "toggle",
      width = M33Auras.normalWidth,
    ...
    warnSpecifcUnit = {
      ...
      order = 10.4,
      hidden = function() return not (trigger.type == "aura2" and trigger.unit == "member" and M33Auras.UntrackableUnit(trigger.specificUnit)) end
    },
    warnSoftTarget = {
      type = "description",
      width = M33Auras.doubleWidth,
      ...
      order = 10.4,
      hidden = function() return not M33Auras.IsUntrackableSoftTarget(trigger.unit) end
    },
    useDebuffType = {
      type = "toggle",
      width = M33Auras.normalWidth,
    ...
    useActualSpec = {
      ...
      desc = L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"],
      order = 66.3,
      hidden = function() return
        not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party")
        and M33Auras.IsCataOrMistsOrRetail())
      end,
    },
    actualSpec = {
      type = "multiselect",
    ...
    useArenaSpec = {
      ...
      order = 67.8,
      hidden = function() return
        not (M33Auras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena")
      end
    },
    arena_spec = {
      type = "multiselect",
      ...
      values = OptionsPrivate.Private.spec_types_all,
      hidden = function()
        return not (M33Auras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and trigger.useArenaSpec)
      end,
      order = 67.9
    },
    arena_specSpace = {
      ...
      width = M33Auras.normalWidth,
      hidden = function()
        return not (M33Auras.IsRetail() and trigger.type == "aura2" and trigger.unit == "arena" and not trigger.useArenaSpec)
      end,
    },
    ...
    inRange = {
      ...
      order = 69.81,
      width = M33Auras.doubleWidth,
      hidden = function() return not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party") and M33Auras.IsRetail()) end
    },
    ignoreInvisible = {
      type = "toggle",
...
M33Auras.RegisterTriggerSystemOptions({"aura2"}, GetBuffTriggerOptions)
