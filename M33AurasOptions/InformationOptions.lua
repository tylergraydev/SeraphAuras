...
function OptionsPrivate.GetInformationOptions(data)
  ...
  if not isTmpGroup then
    args.name = {
      ...
      set = function(info, newid)
        if data.id ~= newid and not M33Auras.GetData(newid) then
          local oldid = data.id
  ...
  if OptionsPrivate.HasWagoUrl(data.id) then
    args.ignoreWagoUpdate = {
      ...
      set = function(info, v)
          local auraData = M33Auras.GetData(data.id)
          if auraData then
            local ignoreUpdate
    ...
    for child in OptionsPrivate.Private.TraverseLeafsOrAura(data) do
      if child.information.saved then
        args["savedData." .. child.uid] = {
          ...
          width = savedDataCount > 1 and M33Auras.doubleWidth or M33Auras.normalWidth,
          order = order,
        }
        order = order + 1
      end
  ...
  if not sameDebugLog or commonDebugLog then
    args.debugLogShow = {
      ...
      func = function()
        --- @type string
        local fullMessage = L["M33Auras %s on WoW %s"]:format(M33Auras.versionString, M33Auras.BuildInfo) .. "\n\n"
        --- @type boolean
