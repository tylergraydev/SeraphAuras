...
local function addCollapsibleHeader(options, key, input, order, isGroupTab)
  ...
  local titleWidth = M33Auras.doubleWidth - (hasAdd and 0.15 or 0) - (hasDelete and 0.15 or 0)  - (hasUp and 0.15 or 0)
  ...
  if not withoutheader then
    options[key .. "collapseButton"] = {
      ...
      image = function()
        if notcollapsable then
          return "Interface\\AddOns\\M33Auras\\Media\\Textures\\bullet1", 18, 18
        else
          return isCollapsed() and "Interface\\AddOns\\M33Auras\\Media\\Textures\\expand"
    ...
    if hasApplyTemplate then
      options[key .. "applyTemplate"] = {
        ...
        order = order + 0.7,
        width = 0.15,
        image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\template",
        imageWidth = 24,
        imageHeight = 24,
        control = "M33AurasIcon",
        hidden = hiddenFunc
...
local function ProgressOptions(data)
  ...
  options.adjustedMin = {
    type = "input",
    validate = M33Auras.ValidateNumericOrPercent,
    width = M33Auras.normalWidth,
    order = order + 2,
    name = L["Minimum"],
...
local function PositionOptions(id, data, _, hideWidthHeight, disableSelfPoint, group)
  ...
  OptionsPrivate.commonOptions.AddCodeOption(positionOptions, data, L["Custom Anchor"], "custom_anchor",
                      "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#custom-anchor-function",
...
local function AddCommonTriggerOptions(options, data, triggernum, doubleWidth)
  ...
  options.type = {
    type = "select",
    width = doubleWidth and M33Auras.doubleWidth or M33Auras.normalWidth,
    name = L["Type"],
