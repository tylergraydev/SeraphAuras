...
local spellCache = {}
M33Auras.spellCache = spellCache
...
function spellCache.Build()
  if not cache  then
    error("spellCache has not been loaded. Call M33Auras.spellCache.Load(...) first.")
  end
...
function spellCache.Load(data)
  ...
  local version = M33Auras.versionString
...
function spellCache.CorrectAuraName(input)
  ...
  local spellId = M33Auras.SafeToNumber(input)
  if type(input) == "string" and input:find("|", nil, true) then
    spellId = M33Auras.SafeToNumber(input:match("|Hspell:(%d+)"))
  end
  if(spellId) then
