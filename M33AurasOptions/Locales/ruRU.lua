...
local L = M33Auras.L
-- if M33AurasSaved.disableRuLocale then -- assume that M33AurasSaved is loaded by the time
--   return
-- end
-- M33Auras/Options
...
Enable this setting if you want this timer to be hidden, or when using a M33Auras text to display the timer]=] ] = [=[Отсчет времени будет отображаться в соответствии с настройками интерфейса (переопределено некоторыми аддонами).

Включите этот параметр, если вы хотите скрыть этот отсчет или использовать текст M33Auras для его отображения.]=]
...
L["Chat with M33Auras experts on our Discord server."] = "Общайтесь со знатоками M33Auras на нашем сервере Discord."
...
Поскольку это триггер статуса, указанные события могут быть переданы M33Auras без ожидаемых аргументов.
...
L["ModelPaths could not be loaded, the addon is %s"] = "Не удалось загрузить M33Auras Model Paths. Причина - %s"
...
L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"] = "Требуется LibSpecialization, т.е. актуальная версия M33Auras"
...
L["Templates could not be loaded, the addon is %s"] = "Не удалось загрузить M33Auras Templates. Причина - %s"
...
Upgrade your version of M33Auras or wait for next release before installing this aura.]=] ] = [=[Индикация была создана в новой версии M33Auras.
Прежде чем импортировать её, обновите вашу версию аддона или дождитесь следующего релиза M33Auras.]=]
...
L["Voice"] = "Голос"
L["M33Auras %s on WoW %s"] = "M33Auras: %s. Интерфейс: %s"
