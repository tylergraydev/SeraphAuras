...
Off Screen]=] ] = "El aura está fuera de la pantalla"
	...
	L["Chat with M33Auras experts on our Discord server."] = "Chatea con los expertos de M33Auras en nuestro servidor Discord."
	L["Check On..."] = "Chequear..."
...
every 3 events starting from 2nd and ending at 11th: 2-11/3]=] ] = "Ocurrencia del evento, se reinicia cuando se descarga la aura. Puede ser un rango de valores. Puede tener múltiples valores separados por una coma o un espacio. Ejemplos: 2.º 5.º y 6.º eventos: 2, 5, 6 2.º a 6.º: 2-6 cada 2 eventos : /2 cada 3 eventos comenzando desde el 2: 2/3 cada 3 eventos comenzando desde el 2 y terminando en el 11: 2-11/3"
	...
	L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"] = "Requiere LibSpecialization, es decir, una versión actualizada de M33Auras."
	...
	L["The M33Auras Options Addon version %s doesn't match the M33Auras version %s. If you updated the addon while the game was running, try restarting World of Warcraft. Otherwise try reinstalling M33Auras"] = "La versión del addon M33Auras Options %s no coincide con la versión de M33Auras %s. Si actualizaste el addon mientras el juego estaba en ejecución, intenta reiniciar World of Warcraft. De lo contrario, intenta reinstalar M33Auras."
	L["Then "] = "Entonces"
...
Upgrade your version of M33Auras or wait for next release before installing this aura.]=] ] = "Esta aura fue creada con una versión más nueva de M33Auras. Actualiza tu versión de M33Auras o espera la próxima versión antes de instalar esta aura."
	...
	L["M33Auras %s on WoW %s"] = "M33Auras %s en WoW %s"
	L["M33Auras recommends using spell ids instead of names. Spell ids are automatically localized."] = "M33Auras recomienda usar IDs de hechizos en lugar de nombres. Los IDs de hechizos se localizan automáticamente."
...
M33Auras will always run custom anchor code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "Puedes agregar aquí una lista de valores de estado separados por comas en los que (cuando se modifican) M33Auras también debería ejecutar el código anclaje. M33Auras siempre ejecutará el código de orden personalizado si incluye \"cambiado\" en esta lista, o cuando se agrega, se elimina, o se reordena una región"
	L[ [=[You can add a comma-separated list of state values here that (when changed) M33Auras should also run the Grow Code on.

M33Auras will always run custom grow code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "Puedes agregar aquí una lista de valores de estado separados por comas en los que (cuando se modifican) M33Auras también debería ejecutar el código crecimiento. M33Auras siempre ejecutará el código de orden personalizado si incluye \"cambiado\" en esta lista, o cuando se agrega, se elimina, o se reordena una región"
	L["You can add a comma-separated list of state values here that (when changed) M33Auras should also run the sort code on.M33Auras will always run custom sort code if you include 'changed' in this list, or when a region is added, removed."] = "Puedes agregar aquí una lista de valores de estado separados por comas en los que (cuando se modifican) M33Auras también debería ejecutar el código de orden. M33Auras siempre ejecutará el código de orden personalizado si incluye \"cambiado\" en esta lista, o cuando se agrega o se elimina una región"
