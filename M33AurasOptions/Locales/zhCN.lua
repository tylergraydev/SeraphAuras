...
Enable this setting if you want this timer to be hidden, or when using a M33Auras text to display the timer]=] ] = "冷却文本会根据原生界面设置（可能被某些插件改动）自动显示。当你想隐藏冷却文本时，或者使用M33Auras文本替代冷却文本时，启用此设置。"
...
Off Screen]=] ] = "光环在屏幕外"
	...
	L["Changelog"] = "更新日志"
	L["Chat with M33Auras experts on our Discord server."] = "在我们的Discord服务器上与M33Auras专家聊天。"
	L["Check On..."] = "检查..."
...
因为这一个是状态触发器, 指定的事件 可以被 M33Auras 调用, 而不需指定参数.
如果有多个事件,可以用逗号或空白分隔。
...
从第二到第十一次事件开始每三次事件：2-11/3]=]
	...
	L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"] = "需要LibSpecialization，可从最新的M33Auras版本中获取。"
	...
	L["The M33Auras Options Addon version %s doesn't match the M33Auras version %s. If you updated the addon while the game was running, try restarting World of Warcraft. Otherwise try reinstalling M33Auras"] = "M33Auras 选项插件版本 %s 与 M33Auras 本体版本 %s 不匹配。如果您在游戏运行时更新了插件，请尝试重新启动《魔兽世界》。否则请尝试重新安装 M33Auras"
	L["Then "] = "然后"
...
Upgrade your version of M33Auras or wait for next release before installing this aura.]=] ] = [=[此光环由更新版本的M33Auras创建。
在导入此光环前，请更新M33Auras版本或等待下次发布。]=]
	...
	L["M33Auras recommends using spell ids instead of names. Spell ids are automatically localized."] = "WA建议使用法术id而不是法术名称。法术id会自动本地化。"
...
M33Auras will always run custom anchor code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = [=[你可以添加一个由英文逗号分隔的状态键列表，当它对应的值被改变时，M33Auras 将运行自定义锚点代码。

如果你在此列表中包含 'changed'，或者一个框体被添加、移除、重新排序，M33Auras 始终会运行自定义锚点代码。]=]
	L[ [=[You can add a comma-separated list of state values here that (when changed) M33Auras should also run the Grow Code on.

M33Auras will always run custom grow code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = [=[你可以添加一个由英文逗号分隔的状态键列表，当它对应的值被改变时，M33Auras 将运行自定义生长代码。

如果你在此列表中包含 'changed'，或者一个框体被添加、移除、重新排序，M33Auras 始终会运行自定义生长代码。]=]
	L["You can add a comma-separated list of state values here that (when changed) M33Auras should also run the sort code on.M33Auras will always run custom sort code if you include 'changed' in this list, or when a region is added, removed."] = [=[你可以添加一个由英文逗号分隔的状态键列表，当它对应的值被改变时，M33Auras 将运行自定义排序代码。

如果你在此列表中包含 'changed'，或者一个框体被添加、移除、重新排序，M33Auras 始终会运行自定义排序代码。]=]
