...
Enable this setting if you want this timer to be hidden, or when using a M33Auras text to display the timer]=] ] = "時間數字會自動依照遊戲內建的選項 (或被其他插件取代) 來決定是否顯示。如果你想要隱藏這個時間數字，或是使用 M33Auras  的文字來顯示時間，請啟用此設定。"
...
跑出畫面]=]
	...
	L["Changelog"] = "更新紀錄"
	L["Chat with M33Auras experts on our Discord server."] = "在我們的 Discord 伺服器和 M33Auras 專家們聊天。"
	L["Check On..."] = "檢查..."
...
因為這是狀態類型的觸發，所指定的事件必須不用加參數就能夠被 M33Auras 呼叫。
可用逗號分隔多個事件。
...
從第二到第十一次事件開始每三次事件：2-11/3]=]
	...
	L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"] = "需要 LibSpecialization，也就是最新的 M33Auras 版本"
	...
	L["The M33Auras Options Addon version %s doesn't match the M33Auras version %s. If you updated the addon while the game was running, try restarting World of Warcraft. Otherwise try reinstalling M33Auras"] = "M33Auras 選項插件版本 %s 與 M33Auras 版本 %s 不符。如果您在遊戲運行時更新了插件，請嘗試重新啟動《魔獸世界》。否則嘗試重新安裝 M33Auras"
	L["Then "] = "(then) 則 "
...
Upgrade your version of M33Auras or wait for next release before installing this aura.]=] ] = [=[這個提醒效果是用較新版本的 M33Auras 建立的。
安裝這個提醒效果之前，請先升級你的 M33Auras 版本，或是等待下次發佈更新。]=]
	...
	L["We thank"] = "我們感謝"
	L["M33Auras %s on WoW %s"] = "M33Auras %s 在 WoW %s"
	L["M33Auras recommends using spell ids instead of names. Spell ids are automatically localized."] = "M33Auras 建議使用法術ID而不是名稱。法術ID會自動本地化。"
...
M33Auras will always run custom anchor code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "您可以在此處添加一個以逗號分隔的狀態值清單，當這些值發生變化時，M33Auras 也會運行錨點程式碼 (Anchor Code)。如果您在此清單中包含 'changed'，或者新增、刪除或重新排序某個區塊時，M33Auras 將會永遠運行自定錨點程式碼。"
	L[ [=[You can add a comma-separated list of state values here that (when changed) M33Auras should also run the Grow Code on.

M33Auras will always run custom grow code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "你可以在此處加入以逗號分隔的狀態值清單，(更改後) M33Auras 也會執行 '增長程式碼'。如果你更改了這個清單，或是加入、刪除或重新排序時，M33Auras 永遠都會執行自訂增長程式碼。"
	L["You can add a comma-separated list of state values here that (when changed) M33Auras should also run the sort code on.M33Auras will always run custom sort code if you include 'changed' in this list, or when a region is added, removed."] = "你可以在此處加入以逗號分隔的狀態值清單，(更改後) M33Auras 也會執行 '排序程式碼'。如果你更改了這個清單，或是加入、刪除或重新排序時，M33Auras 永遠都會執行自訂排序程式碼。"
