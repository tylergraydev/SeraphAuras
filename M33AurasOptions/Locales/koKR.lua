...
Enable this setting if you want this timer to be hidden, or when using a M33Auras text to display the timer]=] ] = "타이머가 기본 인터페이스 설정(일부 애드온에 의해 설정이 무시됨)에 따라 자동으로 표시됩니다. 이 타이머를 숨기거나 M33Auras의 텍스트를 사용해서 표시하고 싶으면 이 설정을 켜세요."
...
Off Screen]=] ] = "위크오라가 화면 밖에 있습니다"
	...
	L["Changelog"] = "업데이트 내역"
	L["Chat with M33Auras experts on our Discord server."] = "우리의 Discord 서버에서 M33Auras 전문가들과 이야기를 나누어 보세요."
	L["Check On..."] = "상태 확인 시점..."
...
이 활성 조건은 상태(Status) 유형이므로 지정된 이벤트가 인자가 없어도 M33Auras에 의해 호출될 수 있습니다.
...
2번부터 11번까지 이벤트 중 3개의 이벤트마다: 2-11/3]=]
	...
	L["Requires LibSpecialization, that is e.g. a up-to date M33Auras version"] = "LibSpecialization이 필요하며, M33Auras 버전을 최신으로 유지하면 됩니다."
...
Upgrade your version of M33Auras or wait for next release before installing this aura.]=] ] = "이 위크오라는 보다 최신 버전의 M33Auras로 제작되었습니다. M33Auras 버전을 업그레이드하거나 이 위크오라를 설치하지 말고 다음 릴리즈를 기다리세요."
	...
	L["We thank"] = "감사합니다"
	L["M33Auras %s on WoW %s"] = "M33Auras %s (WoW %s)"
	L["M33Auras recommends using spell ids instead of names. Spell ids are automatically localized."] = "M33Auras는 주문 이름 대신 주문 ID 사용을 권장합니다. 주문 ID는 언어에 맞게 자동 번역됩니다."
...
M33Auras will always run custom anchor code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "이곳에 State 테이블 값들을 쉼표로 구분해서 목록으로 만들어 넣을 수 있으며 (값이 바뀔 때) M33Auras가 위치 고정 코드를 실행하게 됩니다. 이 목록에 테이블 값 'changed'를 넣을 경우 또는 표시 영역(region)이 추가, 삭제, 재정렬 될 때 M33Auras는 반드시 사용자 정의 고정 코드를 실행합니다."
	L[ [=[You can add a comma-separated list of state values here that (when changed) M33Auras should also run the Grow Code on.

M33Auras will always run custom grow code if you include 'changed' in this list, or when a region is added, removed, or re-ordered.]=] ] = "이곳에 State 테이블 값들을 쉼표로 구분해서 목록으로 만들어 넣을 수 있으며 (값이 바뀔 때) M33Auras가 그룹 확장 코드를 실행하게 됩니다. 이 목록에 테이블 값 'changed'를 넣을 경우 또는 표시 영역(region)이 추가, 삭제, 재정렬 될 때 M33Auras는 반드시 사용자 정의 그룹 확장 코드를 실행합니다."
	L["You can add a comma-separated list of state values here that (when changed) M33Auras should also run the sort code on.M33Auras will always run custom sort code if you include 'changed' in this list, or when a region is added, removed."] = "이곳에 State 테이블 값들을 쉼표로 구분해서 목록으로 만들어 넣을 수 있으며 (값이 바뀔 때) M33Auras가 정렬 코드를 실행하게 됩니다. 이 목록에 테이블 값 'changed'를 넣을 경우 또는 표시 영역(region)이 추가, 삭제, 재정렬 될 때 M33Auras는 반드시 사용자 정의 정렬 코드를 실행합니다."
