...
local function addControlsForChange(args, order, data, conditionVariable, totalAuraCount, conditions, i, j, allProperties, usedProperties)
  ...
  if (propertyType == "bool" or propertyType == "number") then
    ...
    if (propertyType == "number") then
      ...
      if (properties.min or properties.softMin) and (properties.max or properties.softMax) then
        ...
        args["condition" .. i .. "value" .. j].validate = M33Auras.ValidateNumeric;
      end
    else
  ...
  elseif (propertyType == "string" or propertyType == "texture") then
    ...
    if propertyType == "texture" then
      args["condition" .. i .. "value" .. j].width =  M33Auras.normalWidth - 0.15
  ...
  elseif (propertyType == "icon") then
    args["condition" .. i .. "value" .. j] = {
      type = "input",
      width = M33Auras.normalWidth - 0.15,
  ...
  elseif (propertyType == "list" or propertyType == "progressSource" or propertyType == "textureLSM") then
    ...
    if propertyType == "progressSource" then
      args["condition" .. i .. "value" .. j].control = "M33AurasTwoColumnDropdown"
  ...
  elseif (propertyType == "sound") then
    ...
    args["condition" .. i .. "value" .. j .. "sound"] = {
      ...
      itemControl = "M33AurasMediaSound",
  ...
  elseif (propertyType == "chat") then
    ...
    args["condition" .. i .. "value" .. j .. "message dest_text_replacements_button"] = {
      ...
      imageHeight = 24,
      control = "M33AurasIcon",
      image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\sidebar",
    }
    order = order + 1;
    ...
    args["condition" .. i .. "value" .. j .. "custom"] = {
      ...
      control = "M33AurasMultiLineEditBox",
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              else
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, nil, nil, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#chat-message---custom-code-1");
              end
            end
          }
        }
        ...
        extraFunctions = {
          {
            ...
            func = function()
              if (data.controlledChildren) then
                ...
                OptionsPrivate.OpenTextEditor(data, multipath, true, true, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#run-custom-code");
              else
                data.conditions[i].changes[j].value = data.conditions[i].changes[j].value or {};
                OptionsPrivate.OpenTextEditor(data, {"conditions", i, "changes", j, "value", "custom"}, true, nil, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#run-custom-code");
              end
            end
          }
        }
...
local function addControlsForIfLine(args, order, data, conditionVariable, totalAuraCount, conditions, i, path, conditionTemplates, conditionTemplateWithoutCombinations, allProperties, parentType)
  ...
  local normalWidth = M33Auras.normalWidth - indentWidth;
  ...
  if (indentWidth > 0) then
    ...
    args["space" .. order] = {
      ...
      image = function() return "", 0, 0 end,
      order = order,
      width = M33Auras.doubleWidth * 1.5,
    }
    order = order + 1;
    ...
    args["condition" .. i .. tostring(path) .. "bullet"] = {
      ...
      name = "",
      order = order,
      image = "Interface\\Addons\\M33Auras\\Media\\Textures\\bullet" .. indentDepth,
      imageWidth = 10,
      imageHeight = 10,
    }
    order = order + 1;
      ...
      if (conditionTemplatesToUse.indexToTrigger[v] == "REMOVE") then
        if (data.controlledChildren) then
          for id, reference in pairs(conditions[i].check.references) do
            ...
            M33Auras.Add(auraData)
            M33Auras.ClearAndUpdateOptions(auraData.id)
          end
        else
          ...
          extraFunctions = {
            {
              ...
              func = function()
                if (data.controlledChildren) then
                  ...
                  OptionsPrivate.OpenTextEditor(data, multipath, nil, true, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#custom-check");
                else
                  ...
                  OptionsPrivate.OpenTextEditor(data, fullPath, nil, nil, nil, nil, "https://github.com/m33shoq/M33Auras/wiki/Custom-Code-Blocks#custom-check");
                end
              end
            }
          }
...
local function addControlsForCondition(args, order, data, conditionVariable, totalAuraCount, conditions, i, conditionTemplates, conditionTemplateWithoutCombinations, allProperties)
  ...
  args["condition" .. i .. "header"] = {
    ...
    order = order,
    width = M33Auras.doubleWidth - 0.6,
    func = function()
      if data.controlledChildren then
        for id, reference in pairs(conditions[i].check.references) do
          local index = reference.conditionIndex
    ...
    image = collapsed and "Interface\\AddOns\\M33Auras\\Media\\Textures\\expand" or "Interface\\AddOns\\M33Auras\\Media\\Textures\\collapse" ,
    imageWidth = 18,
    imageHeight = 18,
    control = "M33AurasExpand",
  ...
  args["condition" .. i .. "up"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\moveup",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  };
  ...
  args["condition" .. i .. "down"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\movedown",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  };
  ...
  args["condition" .. i .. "duplicate"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\duplicate",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  };
  ...
  args["condition" .. i .. "delete"] = {
    ...
    width = 0.15,
    image = "Interface\\AddOns\\M33Auras\\Media\\Textures\\delete",
    imageWidth = 24,
    imageHeight = 24,
    control = "M33AurasIcon"
  };
