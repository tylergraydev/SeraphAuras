...
local function CorrectSpellName(input)
  ...
  if(inputId) then
    return inputId
  elseif M33Auras.IsClassicEra() and input then
...
function OptionsPrivate.ConstructOptions(prototype, data, startorder, triggernum, triggertype)
  ...
  for index, arg in pairs(prototype.args) do
    ...
    elseif name and (hiddenProperty == "hidden" or not arg.hidden) then
      ...
      elseif (arg.type == "header") then
        ...
        options["use_"..name] = {
          type = "toggle",
          width = arg.width or M33Auras.normalWidth,
          name = arg.display,
          order = order,
          hidden = hidden,
          desc = arg.desc,
      ...
      if(arg.type == "toggle" or arg.type == "tristate") then
        options["use_"..name].width = arg.width or M33Auras.doubleWidth;
      end
      if(arg.type == "spell" or arg.type == "aura" or arg.type == "item") then
        if not arg.showExactOption then
          options["use_"..name].width = (arg.width or M33Auras.normalWidth) - 0.2;
        end
      end
      ...
      for entryNumber = 1, countEntries + 1 do
        ...
        if(arg.type == "number") then
          if entryNumber > 1 then
            options["spacer_"..name..suffix].width = M33Auras.normalWidth
          end
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.noOperator and M33Auras.normalWidth or M33Auras.halfWidth,
        ...
        elseif(arg.type == "string" or arg.type == "tristatestring") then
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.multiline and M33Auras.doubleWidth or M33Auras.normalWidth,
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
            ...
            control = arg.multiline and "M33Auras-MultiLineEditBoxWithEnter" or nil,
            get = function()
        ...
        elseif(arg.type == "longstring") then
          ...
          options[name..suffix] = {
            type = "input",
            width = arg.canBeCaseInsensitive and M33Auras.normalWidth or M33Auras.doubleWidth,
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
        ...
        elseif(arg.type == "spell" or arg.type == "aura" or arg.type == "item") then
          if entryNumber > 1 then
            options["spacer_"..name..suffix].width = M33Auras.normalWidth - (arg.showExactOption and 0 or 0.2)
          end
          local disabled = not trigger["use_"..realname]
          options[name..suffix.."dummy"] = {
            type = "description",
            ...
            image = function()
              ...
              if type(value) == "number" or type(value) == "string" then
                ...
                elseif(arg.type == "spell") then
                  if arg.negativeIsEJ and M33Auras.IsRetail() then
                    local key = M33Auras.SafeToNumber(value)
          ...
          options[name..suffix] = {
            type = "input",
            width = (arg.showExactOption and M33Auras.doubleWidth or M33Auras.normalWidth) - (arg.showExactOption and 0.2 or 0),
            name = arg.display,
            order = order,
            hidden = disabled or hidden,
            validate = validate,
            get = function()
              ...
              elseif(arg.type == "spell") then
                ...
                if value and value ~= "" and (type(value) == "number" or type(value) == "string") then
                  local spellID = M33Auras.SafeToNumber(value)
                  if spellID then
                    if arg.negativeIsEJ and M33Auras.IsRetail() and spellID < 0 then
            ...
            set = function(info, v)
              ...
              if not arg.noValidation then
                if(arg.type == "aura") then
                  fixedInput = M33Auras.spellCache.CorrectAuraName(v);
                elseif(arg.type == "spell") then
        ...
        elseif(arg.type == "select" or arg.type == "unit" or arg.type == "currency") then
          ...
          if(arg.type == "unit" or arg.type == "currency") then
            ...
            options["specific_"..name..suffix] = {
              ...
              desc = specificDesc,
              order = order,
              validate = arg.type == "currency" and M33Auras.ValidateNumeric or false,
  ...
  if prototype.countEvents then
    options.use_count = {
      type = "toggle",
      width = M33Auras.normalWidth,
      name = M33Auras.newFeatureString .. L["Count"],
      order = order,
      get = function()
  ...
  if prototype.delayEvents then
    options.use_delay = {
      type = "toggle",
      width = M33Auras.normalWidth,
      name = M33Auras.newFeatureString .. L["Delay"],
      order = order,
      get = function()
    ...
    if disabled then
      ...
      options.delay = {
        ...
        validate = M33Auras.ValidateTime,
        get = function()
          return OptionsPrivate.Private.tinySecondFormat(trigger.delay)
        end,
        set = function(info, v)
          trigger.delay = M33Auras.TimeToSeconds(v)
          M33Auras.Add(data)
        end
      };
      order = order + 1;
  ...
  if prototype.timedrequired then
    ...
    options.duration = {
      ...
      set = function(info, v)
        trigger.duration = tostring(M33Auras.TimeToSeconds(v))
        M33Auras.Add(data)
      end
    }
    order = order + 1;
