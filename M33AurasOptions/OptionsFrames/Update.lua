...
local function Diff(ours, theirs)
  ...
  -- generates a diff which M33Auras.Update can use
  local debug = false
...
local function BuildUidMap(data, children, type)
  ...
  uidMap.GetPhase2Data = function(self, uid, withAppliedPath, activeCategories)
    ...
    if self.map[uid].anchorFrameFrame then
      ...
      if target then
        data.anchorFrameFrame = "M33Auras:" .. target
      end
    end
  ...
  uidMap.EnsureUniqueIdOfUnmatched = function(self, uid, IncProgress)
    ...
    if not self:GetUIDMatch(uid) then
      if OptionsPrivate.Private.GetDataByUID(uid) then
        local newUid = M33Auras.GenerateUniqueID()
...
local function AddAuraList(container, uidMap, list, categories, expandText)
  local expand = AceGUI:Create("M33AurasExpand")
  local collapsed = true
  local image = collapsed and "Interface\\AddOns\\M33Auras\\Media\\Textures\\expand"
                           or "Interface\\AddOns\\M33Auras\\Media\\Textures\\collapse"
  ...
  local auraLabelContainer = AceGUI:Create("M33AurasInlineGroup")
...
local methods = {
  Open = function(self, data, children, target, linkedAuras, sender, callbackFunc)
    ...
    do
      ...
      if (highestVersion > M33Auras.InternalVersion()) then
        ...
        highestVersionWarning:SetText(L["This aura was created with a newer version of M33Auras.\nUpgrade your version of M33Auras or wait for next release before installing this aura."])
    ...
    if matchInfo ~= nil then
      if not hasChanges(matchInfo) then
        ...
        self:AddChild(AceGUI:Create("M33AurasSpacer"))
        ...
        local updateUiArea = AceGUI:Create("M33AurasInlineGroup")
    ...
    local currentBuild = floor(M33Auras.BuildInfo / 10000)
  ...
  CreateUpdateArea = function(self, area, matchInfo)
    area:AddChild(AceGUI:Create("M33AurasSpacer"))
  ...
  Import = function(self)
    if M33Auras.IsClassicEra() and C_GameRules.IsHardcoreActive() then
      StaticPopupDialogs["M33Auras_CONFIRM_IMPORT_HARDCORE"] = {
      ...
      StaticPopup_Show("M33Auras_CONFIRM_IMPORT_HARDCORE")
      return
    end
    OptionsPrivate.Private:Async({name = "import"}, function()
      self:ImportImpl()
  ...
  ImportImpl = function(self)
    ...
    if pendingPickData then
      ...
      M33Auras.PickDisplay(pendingPickData.id, pendingPickData.tabToShow)
    end
  ...
  -- Note: There's a final renaming via M33Auras.Rename at the end of the update process
  FixUpNames = function(self, uidMap, uid)
    ...
    if existingData then
      ...
      if M33Auras.GetData(uidMap:GetIdFor(uid)) then
  ...
  UpdatePhase1 = function(self, structureUidMap, uid, GetPhase1Data, phase2Order)
    ...
    M33Auras.Add(data)
    M33Auras.NewDisplayButton(data, true)
    self:IncProgress10()
  ...
  UpdatePhase2 = function(self, structureUidMap, GetPhase2Data, phase2Order, copies)
    for i = #phase2Order, 1, -1 do
      ...
      M33Auras.UpdateGroupOrders(data)
  ...
  AddProgressWidgets = function(self)
    ...
    local progress = AceGUI:Create("M33AurasProgressBar")
