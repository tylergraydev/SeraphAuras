...
if not M33AurasSaved.editor_tab_spaces then M33AurasSaved.editor_tab_spaces = 4 end
if not M33AurasSaved.editor_font_size then M33AurasSaved.editor_font_size = 12 end -- set default font size if missing
local color_scheme = {[0] = "|r"}
local function set_scheme()
  if not M33AurasSaved.editor_theme then
    M33AurasSaved.editor_theme = "Monokai"
  end
  local theme = editor_themes[M33AurasSaved.editor_theme]
...
local function ConstructTextEditor(frame)
  ...
  if (fontPath) then
    editor.editBox:SetFont(fontPath, M33AurasSaved.editor_font_size, "")
  end
  ...
  IndentationLib.enable(editor.editBox, color_scheme, M33AurasSaved.editor_tab_spaces)
    ...
    if level == 1 then
      for k, v in pairs(editor_themes) do
        local item = {
          ...
          isNotRadio = false,
          checked = function()
            return M33AurasSaved.editor_theme == k
          end,
          func = function()
            M33AurasSaved.editor_theme = k
            set_scheme()
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          ...
          isNotRadio = true,
          checked = function()
            return M33AurasSaved.editor_bracket_matching
          end,
          func = function()
            M33AurasSaved.editor_bracket_matching = not M33AurasSaved.editor_bracket_matching
          end
        },
      level)
      LibDD:UIDropDownMenu_AddButton(
      ...
      LibDD:UIDropDownMenu_AddButton(
        {
          text = M33Auras.newFeatureString .. L["Font Size"],
          hasArrow = true,
          notCheckable = true,
          menuList = "sizes"
        },
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return M33AurasSaved.editor_tab_spaces == i
            end,
            func = function()
              M33AurasSaved.editor_tab_spaces = i
              IndentationLib.enable(editor.editBox, color_scheme, M33AurasSaved.editor_tab_spaces)
              editor.editBox:SetText(editor.editBox:GetText().."\n")
        ...
        LibDD:UIDropDownMenu_AddButton(
          {
            ...
            isNotRadio = false,
            checked = function()
              return M33AurasSaved.editor_font_size == i
            end,
            func = function()
              M33AurasSaved.editor_font_size = i
              editor.editBox:SetFont(fontPath, M33AurasSaved.editor_font_size, "")
            end
          },
        level)
  ...
  M33AurasOptionsSaved.savedSnippets = M33AurasOptionsSaved.savedSnippets or {}
  local savedSnippets = M33AurasOptionsSaved.savedSnippets
  ...
  local snippetsFrame = CreateFrame("Frame", "M33AurasSnippets", group.frame, "PortraitFrameTemplate")
  ...
  apiSearchFrame = CreateFrame("Frame", "M33AurasAPISearchFrame", group.frame, "PortraitFrameTemplate")
  ...
  -- filter line
  local filterInput = CreateFrame("EditBox", "M33AurasAPISearchFilterInput", apiSearchFrame, "SearchBoxTemplate")
  ...
  editor.editBox:HookScript(
    "OnChar",
    function(_, char)
      if not IsControlKeyDown() and M33AurasSaved.editor_bracket_matching then
        if char == "(" then
