...
local function ConstructMover(frame)
  ...
  arrowTexture:SetTexture("Interface\\Addons\\M33Auras\\Media\\Textures\\offscreen.tga")
  ...
  UpdateHighlight = function(self)
    if self.highlight then
      for _, data in ipairs(self.references) do
        local region = M33Auras.GetRegion(data.id)
...
AlignmentLines.CreateLineInformation = function(self, data, sizerPoint)
  ...
  for id, v in pairs(OptionsPrivate.displayButtons) do
    local region = M33Auras.GetRegion(v.data.id)
    if not skipIds[id]
...
AlignmentLines.CreateLines = function(self, data, sizerPoint)
  ...
  local align = (M33AurasOptionsSaved.magnetAlign and not IsShiftKeyDown())
                    or (not M33AurasOptionsSaved.magnetAlign and IsShiftKeyDown())
  if align then
...
local function ConstructMoverSizer(parent)
  local frame = CreateFrame("Frame", nil, parent, "BackdropTemplateM33Auras")
  frame:SetBackdrop({
