...
local function ConstructIconPicker(frame)
  ...
  local function iconPickerFill(subname, doSort)
    ...
    local AddButton = function(name, icon)
      local button = AceGUI:Create("M33AurasIconButton");
      button:SetName(name);
  ...
  local input = CreateFrame("EditBox", "M33AurasFilterInput", group.frame, "SearchBoxTemplate")
  ...
  local icon = AceGUI:Create("M33AurasIconButton");
  icon.frame:Disable();
  ...
  function group.Pick(self, texturePath)
    ...
    if self.groupIcon then
      ...
      M33Auras.Add(self.baseObject)
      M33Auras.ClearAndUpdateOptions(self.baseObject.id)
      M33Auras.UpdateThumbnail(self.baseObject)
    else
      for child in OptionsPrivate.Private.TraverseLeafsOrAura(self.baseObject) do
        ...
        M33Auras.UpdateThumbnail(child);
      end
    end
  ...
  function group.CancelClose()
    ...
    if group.groupIcon then
      ...
      M33Auras.Add(group.baseObject)
      M33Auras.ClearAndUpdateOptions(group.baseObject.id)
      M33Auras.UpdateThumbnail(group.baseObject)
    else
