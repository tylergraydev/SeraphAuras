...
function OptionsPrivate.CreateFrame()
  LibDD:Create_UIDropDownMenu("M33Auras_DropDownMenu", nil)
  local frame
  local db = OptionsPrivate.savedVars.db
  local odb = OptionsPrivate.savedVars.odb

  frame = CreateFrame("Frame", "M33AurasOptions", UIParent, "PortraitFrameTemplate")
  ...
  function OptionsPrivate.SetTitle(title)
    local text = "M33Auras " .. M33Auras.versionString
    if title and title ~= "" then
      text = ("%s - %s"):format(text, title)
    end
    M33AurasOptionsTitleText:SetText(text)
  end
  ...
  M33AurasOptionsPortrait:SetTexture([[Interface\AddOns\M33Auras\Media\Textures\logo_256_round.tga]])
  ...
  minimizebutton:SetPoint("RIGHT", frame.CloseButton, "LEFT", M33Auras.IsClassicOrWrathOrCataOrMists() and 10 or 0, 0)
  ...
  local tipPopup = CreateFrame("Frame", nil, frame, "BackdropTemplateM33Auras")
  ...
  local addFooter = function(title, texture, url, description, descriptionCJ, descriptionK, rightAligned, width)
    local button = AceGUI:Create("M33AurasToolbarButton")
  ...
  local discordButton = addFooter(L["Discord"], [[Interface\AddOns\M33Auras\Media\Textures\discord.tga]], "https://discord.gg/M33Auras",
            L["Chat with M33Auras experts on our Discord server."])
  discordButton:SetParent(tipFrame)
  discordButton:SetPoint("LEFT", tipFrame, "LEFT")

  local documentationButton = addFooter(L["Documentation"], [[Interface\AddOns\M33Auras\Media\Textures\GitHub.tga]], "https://github.com/m33shoq/M33Auras/wiki",
  ...
  local thanksButton = addFooter(L["Thanks"], [[Interface\AddOns\M33Auras\Media\Textures\waheart.tga]],
                                 "https://www.patreon.com/M33Auras", thanksList, thanksListCJ, thanksListK, nil, 800)
  ...
  local reportbugButton = addFooter(L["Found a Bug?"], [[Interface\AddOns\M33Auras\Media\Textures\bug_report.tga]], "https://github.com/m33shoq/M33Auras/issues/new?template=bug_report.yml",
  ...
  local wagoButton = addFooter(L["Find Auras"], [[Interface\AddOns\M33Auras\Media\Textures\wago.tga]], "https://wago.io",
  ...
  if not OptionsPrivate.Private.CompanionData.slugs then
    companionButton = addFooter(L["Update Auras"], [[Interface\AddOns\M33Auras\Media\Textures\wagoupdate_refresh.tga]], "https://M33Auras.wtf",
  ...
  container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - M33Auras.normalWidth * 340, 0)
  ...
  -- filter line
  local filterInput = CreateFrame("EditBox", "M33AurasFilterInput", frame, "SearchBoxTemplate")
  ...
  local undo = AceGUI:Create("M33AurasToolbarButton")
  undo:SetText(L["Undo"])
  undo:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\upleft")
  ...
  local redo = AceGUI:Create("M33AurasToolbarButton")
  redo:SetText(L["Redo"])
  redo:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\upright")
  ...
  local newButton = AceGUI:Create("M33AurasToolbarButton")
  newButton:SetText(L["New Aura"])
  newButton:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\newaura")
  ...
  local importButton = AceGUI:Create("M33AurasToolbarButton")
  importButton:SetText(L["Import"])
  importButton:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\importsmall")
  ...
  local lockButton = AceGUI:Create("M33AurasToolbarButton")
  lockButton:SetText(L["Lock Positions"])
  lockButton:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\lockPosition")
  lockButton:SetCallback("OnClick", function(self)
    if M33AurasOptionsSaved.lockPositions then
      lockButton:SetStrongHighlight(false)
      lockButton:UnlockHighlight()
      M33AurasOptionsSaved.lockPositions = false
    else
      lockButton:SetStrongHighlight(true)
      lockButton:LockHighlight()
      M33AurasOptionsSaved.lockPositions = true
    end
  end)
  if M33AurasOptionsSaved.lockPositions then
    lockButton:LockHighlight()
  ...
  local magnetButton = AceGUI:Create("M33AurasToolbarButton")
  magnetButton:SetText(L["Magnetically Align"])
  magnetButton:SetTexture("Interface\\AddOns\\M33Auras\\Media\\Textures\\magnetic")
  magnetButton:SetCallback("OnClick", function(self)
    if M33AurasOptionsSaved.magnetAlign then
      magnetButton:SetStrongHighlight(false)
      magnetButton:UnlockHighlight()
      M33AurasOptionsSaved.magnetAlign = false
    else
      magnetButton:SetStrongHighlight(true)
      magnetButton:LockHighlight()
      M33AurasOptionsSaved.magnetAlign = true
    end
  end)
  ...
  local pendingInstallButton = AceGUI:Create("M33AurasLoadedHeaderButton")
  ...
  local pendingUpdateButton = AceGUI:Create("M33AurasLoadedHeaderButton")
  ...
  -- Loaded section
  local loadedButton = AceGUI:Create("M33AurasLoadedHeaderButton")
  ...
  -- Not Loaded section
  local unloadedButton = AceGUI:Create("M33AurasLoadedHeaderButton")
  ...
  local sidegroup = AceGUI:Create("M33AurasInlineGroup")
  ...
  local dynamicTextCodesFrame = CreateFrame("Frame", "M33AurasTextReplacements", sidegroup.frame, "PortraitFrameTemplate")
  ...
  frame.UpdateOptions = function(self)
    ...
    if type(self.pickedDisplay) == "string" then
      data = M33Auras.GetData(frame.pickedDisplay)
    elseif self.pickedDisplay then
      data = tempGroup
    end
    ...
    if optionTable then
      AceConfigRegistry:RegisterOptionsTable("M33Auras", optionTable, true)
    end
  end
  ...
  frame.FillOptions = function(self)
    ...
    container.frame:SetPoint("TOPLEFT", frame, "TOPRIGHT", -63 - M33Auras.normalWidth * 340, -10)
  ...
  frame.NewAura = function(self)
    ...
    if targetId then
      ...
      if pickedButton.data.controlledChildren then
        ...
        local parentData = parent and M33Auras.GetData(parent)
    ...
    if C_AddOns.GetAddOnEnableState("M33AurasTemplates") ~= Enum.AddOnEnableState.None then
      ...
      local button = AceGUI:Create("M33AurasNewButton")
    ...
    for index, regionType in ipairs(regionTypesSorted) do
      if (targetIsDynamicGroup and (regionType == "group" or regionType == "dynamicgroup")) then
        ...
        button:SetClick(function()
          M33Auras.NewAura(nil, regionType, self:GetTargetAura())
        end)
        containerScroll:AddChild(button)
      end
    end
    ...
    -- Import
    local importButton = AceGUI:Create("M33AurasNewButton")
  ...
  frame.PickDisplayMultiple = function(self, id)
    if not self.pickedDisplay then
      ...
      if type(self.pickedDisplay) == "string" then
        if M33Auras.GetData(self.pickedDisplay).controlledChildren or M33Auras.GetData(id).controlledChildren then
          wasGroup = true
  ...
  frame.GetPickedDisplay = function(self)
    if type(self.pickedDisplay) == "string" then
      return M33Auras.GetData(self.pickedDisplay)
    end
    return self.pickedDisplay
  end
