...
local Type, Version = "M33AurasPendingUpdateButton", 6
...
local methods = {
  ...
  ["Initialize"] = function(self, id, companionData)
    ...
    function self.callbacks.OnUpdateClick()
      ...
      M33Auras.Import(self.companionData.encoded, nil, nil, linkedAuras)
    end
        ...
        if not self.linkedChildren[auraId] then
          tinsert(
            self.menu,
            {
              text = auraId,
              notCheckable = true,
              func = function() M33Auras.PickDisplay(auraId, "information") end
            }
          )
        end
      end
      LibDD:EasyMenu(self.menu, M33Auras_DropDownMenu, self.followLink, 0, 0, "MENU", 5)
    end
...
local function Constructor()
  local name = "M33AurasPendingUpdateButton" .. AceGUI:GetNextWidgetNum(Type)
  ...
  update:SetNormalTexture([[Interface\AddOns\M33Auras\Media\Textures\wagoupdate_refresh.tga]])
  update:Disable()
  update:SetWidth(24)
  update:SetHeight(24)
  ...
  tex:SetTexture([[Interface\AddOns\M33Auras\Media\Textures\wagoupdate_logo.tga]])
  tex:SetAllPoints()
  updateLogo.tex = tex
