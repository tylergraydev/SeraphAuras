...
local Type, Version = "M33AurasDisplayButton", 60
  ...
  func = function()
    if (not IsRegionAGroup(clipboard.source) and IsRegionAGroup(clipboard.current)) then
      ...
      for child in OptionsPrivate.Private.TraverseLeafs(clipboard.current) do
        ...
        M33Auras.Add(child)
        M33Auras.ClearAndUpdateOptions(child.id)
      end
    else
      copyAuraPart(clipboard.source, clipboard.current, clipboard.part);
      M33Auras.Add(clipboard.current)
      M33Auras.ClearAndUpdateOptions(clipboard.current.id)
    end

    M33Auras.FillOptions()
    OptionsPrivate.Private.ScanForLoads({[clipboard.current.id] = true});
    OptionsPrivate.SortDisplayButtons(nil, true);
    M33Auras.PickDisplay(clipboard.current.id);
    M33Auras.UpdateThumbnail(clipboard.current.id);
    M33Auras.ClearAndUpdateOptions(clipboard.current.id);
  end
}
...
local Actions = {
    ...
    if source and not source.data.parent then
      if groupId then
        ...
        if group and group:IsGroup() then
          ...
          M33Auras.Add(source.data)
          M33Auras.Add(group.data)
          OptionsPrivate.Private.AddParents(group.data)
          M33Auras.UpdateGroupOrders(group.data)
          M33Auras.ClearAndUpdateOptions(group.data.id)
          M33Auras.ClearAndUpdateOptions(source.data.id)
  ...
  ["Ungroup"] =  function(source)
    if source and source.data.parent then
      local parent = M33Auras.GetData(source.data.parent)
      ...
      if ensure(children, index, source.data.id) then
        ...
        M33Auras.UpdateGroupOrders(parent);
        M33Auras.ClearAndUpdateOptions(parent.id);
...
local function IsParentRecursive(needle, parent)
  ...
  if needle.parent then
    local needleParent = M33Auras.GetData(needle.parent)
...
local methods = {
  ...
  ["Initialize"] = function(self)
      ...
      elseif(IsShiftKeyDown()) then
        ...
        if(editbox) then
          ...
          if not (GetCurrentRegion() == 5 or GetLocale() == "zhCN") then -- China region (5), and chinese locale profanity filter doesn't allow links in chat
            ...
            editbox:Insert("[M33Auras: "..fullName.." - "..self.data.id.."]"..url)
          else
            editbox:Insert("[M33Auras: "..fullName.." - "..self.data.id.."]")
          end
        ...
        if(mouseButton == "RightButton") then
          ...
          if(OptionsPrivate.IsDisplayPicked(self.data.id) and OptionsPrivate.IsPickedMultiple()) then
            LibDD:EasyMenu(OptionsPrivate.MultipleDisplayTooltipMenu(), M33Auras_DropDownMenu, self.frame, 0, 0, "MENU");
          else
            UpdateClipboardMenuEntry(self.data);
            LibDD:EasyMenu(self.menu, M33Auras_DropDownMenu, self.frame, 0, 0, "MENU");
            if not(OptionsPrivate.IsDisplayPicked(self.data.id)) then
              if self.data.controlledChildren then
                M33Auras.PickDisplay(self.data.id, "group")
              else
                M33Auras.PickDisplay(self.data.id);
              end
            end
          end
        else
    ...
    function self.callbacks.OnClickGrouping()
      if (M33Auras.IsImporting()) then return end;
      for index, selectedId in ipairs(self.grouping) do
        local selectedData = M33Auras.GetData(selectedId);
      ...
      M33Auras.FillOptions();
      M33Auras.UpdateGroupOrders(self.data);
    ...
    function self.callbacks.OnUpGroupClick()
      ...
      if(self.data.parent) then
        ...
        local parentData = M33Auras.GetData(self.data.parent);
        local index;
        ...
        if(index) then
          if(index <= 1) then
            ...
            OptionsPrivate.Private.Animate("button", M33Auras.GetData(parentData.controlledChildren[index-1]).uid, "main", updata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", M33Auras.GetData(parentData.controlledChildren[index]).uid, "main", downdata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            M33Auras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
      ...
      if(self.data.parent) then
        ...
        if(index) then
          if(index >= #parentData.controlledChildren) then
            ...
            local updata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = -32};
            local downdata = {duration = 0.15, type = "custom", use_translate = true, x = 0, y = 32};
            OptionsPrivate.Private.Animate("button", M33Auras.GetData(parentData.controlledChildren[index+1]).uid, "main", downdata, self.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            OptionsPrivate.Private.Animate("button", M33Auras.GetData(parentData.controlledChildren[index]).uid, "main", updata, otherbutton.frame, true, function() OptionsPrivate.SortDisplayButtons() end);
            M33Auras.FillOptions()
          end
        else
          error("Display thinks it is a member of a group which does not control it");
    ...
    function self.callbacks.OnRenameAction(newid)
      ...
      if not(newid == oldid) then
        M33Auras.Rename(self.data, newid);
      end
      self:UpdateParentWarning()
    end
    ...
    -- tinsert(self.menu, {
    --   text = L["|cFF8855FFM33Auras Sync|r"],
    --   notCheckable = true,
    --   func = function()
  ...
  ["Ungroup"] = function(self)
    ...
    local newParent = parentData.parent and M33Auras.GetData(parentData.parent)
    if newParent then
    ...
    self:UpdateIconsVisible()
    if newParent then
      M33Auras.Add(newParent)
      OptionsPrivate.Private.AddParents(newParent)
      M33Auras.ClearAndUpdateOptions(newParent.id)
      M33Auras.UpdateGroupOrders(newParent)
    end
    M33Auras.ClearAndUpdateOptions(self.data.id);
    ...
    if picked then
      ...
      if mode == "GROUP" then
        if self.data.regionType == "dynamicgroup" then
          ...
          local parentData = self.data.parent and M33Auras.GetData(self.data.parent)
  ...
  ["UpdateOffset"] = function(self)
    ...
    if group then
      ...
      while(group) do
        depth = depth + 1
        group = M33Auras.GetData(group).parent
      end
          ...
          onClick = function()
            MenuUtil.CreateContextMenu(UIParent, function(ownerRegion, root)
              root:CreateButton(soundText, function()
                M33Auras.PickDisplay(warning.auraId, tabsForWarning[warning.key] or "information")
              end)
  ...
  ["SetLoaded"] = function(self, prio, file, title, description)
    self:UpdateStatusIcon("load", prio, "Interface\\AddOns\\M33Auras\\Media\\Textures\\" .. file, title, description, nil)
    self:SortStatusIcons()
  end,
  ...
  ["OverrideIcon"] = function(self)
    self.icon:SetTexture("Interface\\Addons\\M33Auras\\Media\\Textures\\icon.blp")
    self.icon:Show()
    if(self.iconRegion and self.iconRegion.Hide) then
      self.iconRegion:Hide();
    end
...
local function Constructor()
  local name = "M33AurasDisplayButton"..AceGUI:GetNextWidgetNum(Type);
  ---@class Button
  ...
  renamebox:SetScript("OnEnterPressed", function()
    ...
    if(newid == "" or (newid ~= oldid and M33Auras.GetData(newid))) then
