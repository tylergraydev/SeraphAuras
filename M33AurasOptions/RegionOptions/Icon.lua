...
local function createOptions(id, data)
  ...
  local options = {
    ...
    cooldownSwipe = {
      ...
      name = L["Show \"Swipe\""],
      order = 11.3,
      desc = "|TInterface\\AddOns\\M33Auras\\Media\\Textures\\swipe-example:30|t\n"..L["Enable \"swipe\" part of the overlay"],
      hidden = function() return not data.cooldown end,
    },
    cooldownEdge = {
      type = "toggle",
      ...
      name = L["Show \"Edge\""],
      order = 11.4,
      desc = "|TInterface\\AddOns\\M33Auras\\Media\\Textures\\edge-example:30|t\n"..L["Enable \"Edge\" part of the overlay"],
      hidden = function() return not data.cooldown end,
    },
...
local function modifyThumbnail(parent, frame, data)
  ...
  if data then
    local name, icon = M33Auras.GetNameAndIcon(data);
    frame:SetIcon(icon)
  end
end
